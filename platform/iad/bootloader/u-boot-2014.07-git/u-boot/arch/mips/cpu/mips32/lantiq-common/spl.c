...
static int spl_load_nand_flash(struct spl_image *spl)
{
	 ...
	 *
	 * xRX200 or GRX330 SLC NAND bootstrap:
	 * - 20 byte non-volatile bootstrap header
	 * - SPL binary
	 * - 12 byte non-volatile bootstrap header
...
#if defined(CONFIG_SYS_NAND_BLOCK_SIZE)
...
static int spl_load_ubi(struct spl_image *spl)
{
	 ...
	 * Image format:
	 *
	 * MTD partition 'ubi' with volumes:
	 *  - uboot
	 *  - uboot_env
	 *  - kernel1
	 *  - kernel2
	 ...
	 *
	 * Image loading:
	 * - load volume 'uboot' (ID 0) to loadaddr
	 * - parse SPL infos from U-Boot mkimage header
	 * - skip U-Boot mkimage header (loadaddr += image_get_header_size())
	 * - uncompress or copy from loadaddr to entryaddr
	 */
	...
	info.vid_offset = CONFIG_SYS_NAND_PAGE_SIZE;
	info.leb_start = 2 * CONFIG_SYS_NAND_PAGE_SIZE;
	info.peb_count = CONFIG_SPL_UBI_MAX_PEBS;

	volume.vol_id = CONFIG_SPL_UBI_LOAD_MONITOR_ID;
	volume.load_addr = (void *)loadaddr;
#endif

	spl_debug("SPL: reading U-Boot image from UBI volume\n");
	ret = ubispl_load_volumes(&info, &volume, 1);
	if (ret) {
		spl_puts("SPL: UBI error\n");
		return ret;
	}
	...
	/* If data cannot be verified, don't start up */
	if (!spl_check_data(spl, loadaddr + image_get_header_size()))
		return -1;
...
#if spl_boot_hsnand_flash || spl_boot_ubi
	spl_debug("SPL: initializing NAND flash\n");
	nand_init();
#endif
