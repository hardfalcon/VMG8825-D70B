...
#if defined(CONFIG_LTQ_NAND_FORCE_NO_ECC)
#define hsnand_force_no_ecc		1
#else
#define hsnand_force_no_ecc		0
#endif
...
static void hsnand_select_chip(struct mtd_info *mtd, int chipnum)
{
	if (nand_cs1)
		...
		ltq_clrbits(&nand_regs->con, NAND_CON_OUT_CS1 | NAND_CON_IN_CS1);
...
static void hsnand_drv_setup(struct mtd_info *mtd)
{
	...
	if (boot_info & HSNAND_ECC_OFF || hsnand_force_no_ecc)
		drv->ecc_off = 1;
...
#if defined(CONFIG_SPL_UBI)
static uint8_t scratch_buf[CONFIG_SYS_NAND_PAGE_SIZE] __aligned(ARCH_DMA_MINALIGN);
...
int nand_spl_read_block(int block, int offset, int len, void *dst)
{
	...
	drv->hw_page = page + block * CONFIG_SYS_NAND_PAGE_COUNT;
