...
struct ltq_eth_priv {
	...
	u8 phy_prio[LTQ_ETHSW_MAX_GMAC];
	int rx_num;
	int tx_num;
};
...
static int ltq_eth_phy_startup(struct ltq_eth_priv *priv)
{
	...
	int i, j;
	unsigned int phy_prio, prio;

	for (prio = 0; prio < LTQ_ETHSW_MAX_GMAC; prio++) {
		printf("Waiting for PHY link (prio %u)\n", prio);
		for (j = 0; j < LTQ_PHY_ANEG_TIMEOUT; j++) {
					...
					const struct ltq_eth_port_config *port)
{
	...
	if (port->flags & LTQ_ETH_PORT_PHY) {
		...
		priv->phy_prio[port->num] = port->prio;
	}
}
