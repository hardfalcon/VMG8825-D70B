...
#define MACH_TYPE	CONFIG_MACH_TYPE
#else
#define MACH_TYPE	"GENERIC"
#endif
...
static void sas_env_console_init(ulong loadaddr)
{
	...
	if (ret == 0 && 0 == strncmp(img.name, "kernel#Linux-2.6.32", 19))
		consoledev = sas_consoledev_fixup();
	else
		consoledev = sas_consoledev_configured();

	printf("Set console device to %s\n", consoledev);
...
static void sas_env_bootargs_init(int ptest_mode)
{
	...
	sas_env_bootargs_add("machtype=" MACH_TYPE);
