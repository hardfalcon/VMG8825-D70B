...
dnl FILESYSTEM_CHECK(
dnl   [1-VARIABLE])
dnl ----------------------------------------------------------
dnl
dnl enable use of a filesystem
dnl specify --enable-filesystem (enabled by default)
dnl
AC_DEFUN([FILESYSTEM_CHECK],
[
   dnl enable use of filesystem
   AC_MSG_CHECKING(if filesystem operations should be used)
   AC_ARG_ENABLE(filesystem,
       AS_HELP_STRING(
           [--enable-filesystem],
           [enable the use of filesystem operations]
       ),
       [
           if test $enableval = 'yes'; then
               AC_MSG_RESULT(yes)
               AC_DEFINE([$1],[1],[enable the use of filesystem operations])
	   else
		AC_MSG_RESULT([no])
           fi
       ],
...
dnl
dnl Option used to enable IPv4 support
dnl -------------------------------------------------------
dnl IPV4_CHECK([DEFAULT-ACTION], [ACTION-IF-ENABLED], [ACTION-IF-DISABLED])
dnl
dnl available values are:
dnl		--enable-ipv4 (default)
dnl		--disable-ipv4
dnl
AC_DEFUN([IPV4_CHECK],
[
   ...
   AC_ARG_ENABLE(ipv4,
      ...
      [
         if test "x$enableval" = "xno"; then
            ...
            AC_MSG_RESULT([enabled])
            AC_DEFINE([IPV4_SUPPORT],[1],[enable IPv4 support])
         fi
      ],[
...
dnl
dnl Option used to enable IPv6 support
dnl -------------------------------------------------------
dnl IPV6_CHECK([DEFAULT-ACTION], [ACTION-IF-ENABLED], [ACTION-IF-DISABLED])
dnl
dnl available values are:
dnl		--enable-ipv6 (default)
dnl		--disable-ipv6
dnl
AC_DEFUN([IPV6_CHECK],
[
   ...
   AC_ARG_ENABLE(ipv6,
      ...
      [
         if test "x$enableval" = "xno"; then
            ...
            AC_MSG_RESULT([enabled])
            AC_DEFINE([IPV6_SUPPORT],[1],[enable IPv6 support])
         fi
      ],[
            AC_MSG_RESULT([enabled(default)])
            AC_DEFINE([IPV6_SUPPORT],[1],[enable IPv6 support])
