...
[
	AC_MSG_CHECKING(for Linux kernel 2.6 or later)

	if test "${cached_enable_linux26+set}" != set; then
		AC_ARG_ENABLE(linux-26,
			AS_HELP_STRING(
				[--enable-linux-26],
				[enable support for Linux kernel 2.6.x or later]
			),
		...
		if test "$__enable_linux26" = "yes" -o "$__enable_linux26" = "enable"; then
			...
			AC_DEFINE([LINUX_2_6],[1],[enable code for linux 2.6 or later])
			with_linux26=yes
...
dnl
dnl Option used to configure build as Linux platform driver with device tree support
dnl -------------------------------------------------
dnl WITH_KERNEL_MODULE_CHECK([DEFAULT-ACTION], [ACTION-IF-ENABLED], [ACTION-IF-DISABLED])
dnl
dnl available values are:
dnl		--enable-linux-platform-driver
dnl		--disable-linux-platform-driver (default)
dnl
AC_DEFUN([LINUX_PLATFORM_DRIVER_CHECK],
[
	AC_MSG_CHECKING(for build as Linux platform driver with device tree support)
        AC_ARG_ENABLE(linux-platform-driver,
                AS_HELP_STRING(
                        [--enable-linux-platform-driver],
                        [Enable build as Linux platform driver with device tree support]
                ),
                [__enable_linux_platform_driver=$enableval],
                [__enable_linux_platform_driver=ifelse([$1],,[no],[$1])]
        )

        if test "$__enable_linux_platform_driver" = "yes" -o "$__enable_linux_platform_driver" = "enable"; then
                AC_DEFINE([LINUX_PLATFORM_DRIVER],[1],[enable build as Linux platform driver with device tree support])
                AC_MSG_RESULT([enabled])
        ...
        unset __enable_linux_platform_driver
])
