...
dnl Option used to enable TAPI legacy FAX Data Pump
dnl -------------------------------------------------
dnl TAPI_FAX_CHECK([DEFAULT-ACTION], [ACTION-IF-ENABLED], [ACTION-IF-DISABLED])
dnl
dnl available values are:
dnl		--enable-fdp (default)
dnl		--disable-fdp
dnl
AC_DEFUN([TAPI_FAX_CHECK],
[
	AC_MSG_CHECKING(for TAPI legacy FAX Data Pump support)
	AC_ARG_ENABLE(fdp,
		AS_HELP_STRING(
			[--enable-fdp],
			[Enable TAPI FAX Data Pump (FDP) support (T.38 is not supported by voice DSP)]
		),
		[__enable_tapi_fdp=$enableval],
		[__enable_tapi_fdp=ifelse([$1],,[yes],[$1])]
	)

	if test "$__enable_tapi_fdp" = "yes" -o "$__enable_tapi_fdp" = "enable"; then
		AC_DEFINE([TAPI_FAX_T38],[1],[enable TAPI legacy FAX Data Pump support])
		AC_MSG_RESULT([enabled])
	...
	unset __enable_tapi_fdp
])

dnl Option used to enable TAPI FAX T.38 Stack
dnl -------------------------------------------------
...
[
	AC_MSG_CHECKING(for TAPI FAX T.38 stack support)
	AC_ARG_ENABLE(t38,
		AS_HELP_STRING(
			[--enable-t38],
			[Enable TAPI FAX T.38 stack support (T.38 is supported by voice DSP)]
		),
	...
	if test "$__enable_tapi_faxstack" = "yes" -o "$__enable_tapi_faxstack" = "enable"; then
		AC_DEFINE([TAPI_FAX_T38_FW],[1],[enable TAPI FAX T.38 stack support])
		AC_MSG_RESULT([enabled])
...
dnl Option to select which interface type to use with PMC
dnl --------------------------------------------------------------
dnl TAPI_PMC_IF_CHECK()
dnl
dnl specify interface with --enable-pmc-if = <PMCU|CPUFREQ|NONE>
dnl
AC_DEFUN([TAPI_PMC_IF_CHECK],
[
	AC_MSG_CHECKING(for interface type with Power Management Control)
	# m4 definitions required to pass value in to the help message
	m4_define('PMC_IF_NAMES',"pmcu|cpufreq|none")
	AC_SUBST([PMC_IF_NAMES],[m4_defn('PMC_IF_NAMES')])

	AC_ARG_ENABLE(pmc-if,
		AS_HELP_STRING(
			[--enable-pmc-if@<:@=ARG@:>@],
			[Define which interface type to use with PMC.
			"pmcu": LANTIQ proprietary PMCU interface,
			"cpufreq": Linux CPU frequency interface]
		),
		[PMC_IF_NAME=$enableval],
		[PMC_IF_NAME="none"])

	case $PMC_IF_NAME in
	pmcu|PMCU)
		AC_DEFINE([TAPI_PMC_IF_PMCU],[1],[Use LANTIQ PMCU interface])
         	;;
	cpufreq|CPUFREQ)
		AC_DEFINE([TAPI_PMC_IF_CPUFREQ],[1],[Use Linux CPU frequency interface])
         	;;
      	*)
		PMC_IF_NAME="none"
	;;
...
[
	...
	if test "$__enable_tapi_srtp" = "yes" -o "$__enable_tapi_srtp" = "enable"; then
		...
		AM_CONDITIONAL(TAPI_SRTP, true)
		AC_MSG_RESULT([enabled])

		ifelse([$2],,[:],[$2])
	else
		AC_MSG_RESULT([disabled])
		AM_CONDITIONAL(TAPI_SRTP, false) 
		ifelse([$3],,[:],[$3])
	fi

	AM_CONDITIONAL(SRTP_LIB_INCL, false)
	dnl set SRTP library includes path
	AC_ARG_WITH(srtp-incl,
		AS_HELP_STRING(
			[--with-srtp-incl@<:@=DIR@:>@],
			[SRTP library includes path, applied only with option --enable-srtp]
		),
		[
			if test \( "$__enable_tapi_srtp" = "yes" -o "$__enable_tapi_srtp" = "enable" \) -a "$cached_tapi_version" = TAPI_VERSION3; then
				AC_MSG_CHECKING(for valid path to SRTP library includes)
				if test -e $withval/srtp.h; then
					AC_MSG_RESULT(yes)
					AC_SUBST([SRTP_INCL_PATH],[$withval])
					AM_CONDITIONAL(SRTP_LIB_INCL, true)
				else
					AC_MSG_RESULT(no)
					AC_MSG_ERROR([SRTP library includes path '$withval' is invalid!])
				fi
			fi
		],
		[
			if test \( "$__enable_tapi_srtp" = "yes" -o "$__enable_tapi_srtp" = "enable" \) -a "$cached_tapi_version" = TAPI_VERSION3; then
				AC_MSG_ERROR([Please provide a path to SRTP library includes with --with-srtp-incl]);
			fi
		]
	)
...
dnl Option used to enable RTCP XR support
dnl -------------------------------------------------
dnl TAPI_RTCP_XR_CHECK([DEFAULT-ACTION], [ACTION-IF-ENABLED], [ACTION-IF-DISABLED])
dnl
dnl available values are:
dnl		--enable-rtcp-xr
dnl		--disable-rtcp-xr (default)
dnl
AC_DEFUN([TAPI_RTCP_XR_CHECK],
[
	AC_MSG_CHECKING(for TAPI RTCP XR support)
	AC_ARG_ENABLE(rtcp-xr,
		AS_HELP_STRING(
			[--enable-rtcp-xr],
			[Enable RTCP XR support]
		),
		[__enable_tapi_rtcp_xr=$enableval],
		[__enable_tapi_rtcp_xr=ifelse([$1],,[no],[$1])]
	)

	if test "$__enable_tapi_rtcp_xr" = "yes" -o "$__enable_tapi_rtcp_xr" = "enable"; then
		AC_DEFINE([TAPI_RTCP_XR],[1],[enable TAPI RTCP XR support])
		AC_MSG_RESULT([enabled])
	...
	unset __enable_tapi_rtcp_xr
])

dnl Option used to enable tone generator support
dnl -------------------------------------------------
dnl TAPI_TONEGEN_CHECK([DEFAULT-ACTION], [ACTION-IF-ENABLED], [ACTION-IF-DISABLED])
dnl
dnl available values are:
dnl            --enable-tonegen (default)
dnl            --disable-tonegen
dnl
AC_DEFUN([TAPI_TONEGEN_CHECK],
[
	AC_MSG_CHECKING(for TAPI Tonegen support)
	AC_ARG_ENABLE(tonegen,
		AS_HELP_STRING(
			[--enable-tonegen],
			[Enable tonegen support]
		),
		[__enable_tapi_tonegen=$enableval],
		[__enable_tapi_tonegen=ifelse([$1],,[yes],[$1])]
	)

	if test "$__enable_tapi_tonegen" = "yes" -o "$__enable_tapi_tonegen" = "enable"; then
		AC_DEFINE([TAPI_TONEGEN],[1],[enable TAPI tone generator support])
		AC_MSG_RESULT([enabled])
	...
	unset __enable_tapi_tonegen
])

dnl Option used to enable CPTD support
dnl -------------------------------------------------
dnl TAPI_CPTD_CHECK([DEFAULT-ACTION], [ACTION-IF-ENABLED], [ACTION-IF-DISABLED])
dnl
dnl available values are:
dnl		--enable-cptd (default)
dnl		--disable-cptd
dnl
AC_DEFUN([TAPI_CPTD_CHECK],
[
	AC_MSG_CHECKING(for TAPI CPTD support)
	AC_ARG_ENABLE(cptd,
		AS_HELP_STRING(
			[--enable-cptd],
			[Enable CPTD support]
		),
		[__enable_tapi_cptd=$enableval],
		[__enable_tapi_cptd=ifelse([$1],,[yes],[$1])]
	)

	if test "$__enable_tapi_cptd" = "yes" -o "$__enable_tapi_cptd" = "enable"; then
		AC_DEFINE([TAPI_CPTD],[1],[enable TAPI CPTD support])
		AC_MSG_RESULT([enabled])
...
dnl Option used to enable P-Phone support
dnl -------------------------------------------------
dnl TAPI_PPHONE_CHECK([DEFAULT-ACTION], [ACTION-IF-ENABLED], [ACTION-IF-DISABLED])
dnl
dnl available values are:
dnl		--enable-pphone (default)
dnl		--disable-pphone
dnl
AC_DEFUN([TAPI_PPHONE_CHECK],
[
	AC_MSG_CHECKING(for TAPI P-Phone support)
	AC_ARG_ENABLE(pphone,
		AS_HELP_STRING(
			[--enable-pphone],
			[Enable P-Phone support]
		),
		[__enable_tapi_pphone=$enableval],
		[__enable_tapi_pphone=ifelse([$1],,[yes],[$1])]
	)

	if test "$__enable_tapi_pphone" = "yes" -o "$__enable_tapi_pphone" = "enable"; then
		AC_DEFINE([TAPI_PPHONE],[1],[enable TAPI P-Phone support])
		AC_MSG_RESULT([enabled])
	...
	unset __enable_tapi_pphone
])

dnl Option used to enable MOS-LQE calculation support
dnl -------------------------------------------------
dnl TAPI_MOSLQE_CHECK([DEFAULT-ACTION], [ACTION-IF-ENABLED], [ACTION-IF-DISABLED])
dnl
dnl available values are:
dnl		--enable-mos-lqe (default)
dnl		--disable-mos-lqe
dnl
AC_DEFUN([TAPI_MOSLQE_CHECK],
[
	AC_MSG_CHECKING(for TAPI MOS-LQE calculation support)
	AC_ARG_ENABLE(mos-lqe,
		AS_HELP_STRING(
			[--enable-mos-lqe],
			[Enable MOS-LQE calculation support]
		),
		[__enable_tapi_moslqe=$enableval],
		[__enable_tapi_moslqe=ifelse([$1],,[yes],[$1])]
	)

	if test "$__enable_tapi_moslqe" = "yes" -o "$__enable_tapi_moslqe" = "enable"; then
		AC_DEFINE([TAPI_MOS_LQE],[1],[enable TAPI MOS-LQE calculation support])
		AC_MSG_RESULT([enabled])
...
dnl Option used to enable the MFTD support
dnl -------------------------------------------------
dnl TAPI_MFTD_CHECK([DEFAULT-ACTION], [ACTION-IF-ENABLED], [ACTION-IF-DISABLED])
dnl
dnl available values are:
dnl		--enable-mftd (default)
dnl		--disable-mftd
dnl
AC_DEFUN([TAPI_MFTD_CHECK],
[
	AC_MSG_CHECKING(for TAPI MFTD support)
	AC_ARG_ENABLE(mftd,
		AS_HELP_STRING(
			[--enable-mftd],
			[Enable MFTD support]
		),
		[__enable_tapi_mftd=$enableval],
		[__enable_tapi_mftd=ifelse([$1],,[yes],[$1])]
	)

	if test "$__enable_tapi_mftd" = "yes" -o "$__enable_tapi_mftd" = "enable"; then
		AC_DEFINE([TAPI_MFTD_SUPPORT],[1],[enable TAPI MFTD support])
		AC_MSG_RESULT([enabled])
	...
	unset __enable_tapi_mftd
])

dnl Option used to enable the FXO support
dnl -------------------------------------------------
dnl TAPI_FXO_CHECK([DEFAULT-ACTION], [ACTION-IF-ENABLED], [ACTION-IF-DISABLED])
dnl
dnl available values are:
dnl		--enable-fxo (default)
dnl		--disable-fxo
dnl
AC_DEFUN([TAPI_FXO_CHECK],
[
	AC_MSG_CHECKING(for TAPI FXO support)
	AC_ARG_ENABLE(fxo,
		AS_HELP_STRING(
			[--enable-fxo],
			[Enable FXO support]
		),
		[__enable_tapi_fxo=$enableval],
		[__enable_tapi_fxo=ifelse([$1],,[yes],[$1])]
	)

	if test "$__enable_tapi_fxo" = "yes" -o "$__enable_tapi_fxo" = "enable"; then
		AC_DEFINE([TAPI_FXO_SUPPORT],[1],[enable TAPI FXO support])
		AC_MSG_RESULT([enabled])
	...
	unset __enable_tapi_fxo
])

dnl Option used to enable SmartSLIC support
dnl -------------------------------------------------
dnl TAPI_SSLIC_CHECK([DEFAULT-ACTION], [ACTION-IF-ENABLED], [ACTION-IF-DISABLED])
dnl
dnl available values are:
dnl		--enable-sslic (default)
dnl		--disable-sslic
dnl
AC_DEFUN([TAPI_SSLIC_CHECK],
[
	AC_MSG_CHECKING(for TAPI SmartSLIC support)
	AC_ARG_ENABLE(sslic,
		AS_HELP_STRING(
			[--enable-sslic],
			[Enable SmartSLIC support]
		),
		[__enable_tapi_sslic=$enableval],
		[__enable_tapi_sslic=ifelse([$1],,[yes],[$1])]
	)

	if test "$__enable_tapi_sslic" = "yes" -o "$__enable_tapi_sslic" = "enable"; then
		AC_DEFINE([TAPI_SSLIC_SUPPORT],[1],[enable TAPI SmartSLIC support])
		AC_MSG_RESULT([enabled])
	...
	unset __enable_tapi_sslic
])

dnl Option used to enable LEC support
dnl -------------------------------------------------
dnl TAPI_LEC_CHECK([DEFAULT-ACTION], [ACTION-IF-ENABLED], [ACTION-IF-DISABLED])
dnl
dnl available values are:
dnl		--enable-lec (default)
dnl		--disable-lec
dnl
AC_DEFUN([TAPI_LEC_CHECK],
[
	AC_MSG_CHECKING(for TAPI LEC support)
	AC_ARG_ENABLE(lec,
		AS_HELP_STRING(
			[--enable-lec],
			[Enable LEC support]
		),
		[__enable_tapi_lec=$enableval],
		[__enable_tapi_lec=ifelse([$1],,[yes],[$1])]
	)

	if test "$__enable_tapi_lec" = "yes" -o "$__enable_tapi_lec" = "enable"; then
		AC_DEFINE([TAPI_LEC_SUPPORT],[1],[enable TAPI LEC support])
		AC_MSG_RESULT([enabled])
...
[
	...
	AC_ARG_ENABLE(dial,
		AS_HELP_STRING(
			[--enable-dial],
			[Enable pulse dial/hook support]
		),
...
dnl Option used to select SLIC support
dnl -------------------------------------------------
dnl TAPI_SLIC_CHECK([DEFAULT-ACTION], [ACTION-IF-ENABLED], [ACTION-IF-DISABLED])
dnl
dnl available values are:
dnl		--enable-slic (default)
dnl		--disable-slic
dnl
AC_DEFUN([TAPI_SLIC_CHECK],
[
	AC_MSG_CHECKING(for SmartSLIC support)
	AC_ARG_ENABLE(slic,
		AS_HELP_STRING(
			[--disable-slic],
			[Disable SmartSLIC support]
		),
		[__enable_tapi_slic=$enableval],
		[__enable_tapi_slic=ifelse([$1],,[yes],[$1])]
	)

	if test "$__enable_tapi_slic" = "yes" -o "$__enable_tapi_slic" = "enable"; then
		AC_MSG_RESULT([enabled])

		ifelse([$2],,[:],[$2])
	else
		AC_DEFINE([TAPI_NO_SLIC],[1],[disable TAPI SLIC support])
		AC_MSG_RESULT([disabled])

		ifelse([$3],,[:],[$3])
	fi

	unset __enable_tapi_slic
])

dnl Option used to disable automatic data to data channel connection support
dnl ------------------------------------------------------------------------
dnl TAPI_AUTO_DATA_MAP_CHECK([DEFAULT-ACTION], [ACTION-IF-ENABLED], [ACTION-IF-DISABLED])
dnl
dnl available values are:
dnl		--enable-automatic-datachannel-mapping (default)
dnl		--disable-automatic-datachannel-mapping
dnl
AC_DEFUN([TAPI_AUTO_DATA_MAP_CHECK],
[
	AC_MSG_CHECKING(for automatic datachannel mapping)
	AC_ARG_ENABLE(automatic-datachannel-mapping,
		AS_HELP_STRING(
			[--disable-automatic-datachannel-mapping],
			[Disable automatic-datachannel-mapping]
		),
		[__enable_auto_d2d_map=$enableval],
		[__enable_auto_d2d_map=ifelse([$1],,[yes],[$1])]
	)

	if test "$__enable_auto_d2d_map" = "yes" -o "$__enable_auto_d2d_map" = "enable"; then
		AC_DEFINE([TAPI_AUTOMATIC_DATA_MAPPING],[1],[enable automatic data to data channel connection support])
		AC_MSG_RESULT([enabled])
