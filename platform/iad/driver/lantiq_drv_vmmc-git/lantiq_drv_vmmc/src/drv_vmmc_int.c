...
extern IFX_uint16_t VMMC_ALM_ElapsedTimeSinceLastHook (
                        VMMC_CHANNEL *pCh,
...
IFX_return_t VMMC_Register_Callback(VMMC_DEVICE *pDev)
{
   ...
   for (mpsCh = command;
        VMMC_SUCCESS(ret) && (mpsCh <= E_MPS_DEVICE_TYPE_VOICE_LAST); mpsCh++)
   {
      /** MPS driver channel numbering: 1 stands for command channel,
          2 stands for voice channel 0, 3 stands for voice channel 1, ... */

      ret = ifx_mps_register_data_callback(mpsCh,
                                           1, /* upstream */
                                           VMMC_Upstream_Callback);

      TRACE(VMMC, DBG_LEVEL_LOW,
            ("VMMC_Register_Callback(), registered for mpsCh=%d (%s) %s\n",
            mpsCh,
            mpsCh==command ? "command" : "voice",
            VMMC_SUCCESS(ret) ? "success":"error"));
...
IFX_return_t VMMC_UnRegister_Callback(void)
{
   ...
   if (!VMMC_SUCCESS(ret))
   {
      TRACE (VMMC, DBG_LEVEL_HIGH,("Disable MPS IRQs failed %d.\n", ret));
   }
        ...
        VMMC_SUCCESS(ret) && (mpsCh <= E_MPS_DEVICE_TYPE_VOICE_LAST); mpsCh++)
   {
      ...
      if (VMMC_SUCCESS(ret))
      {
         ret = ifx_mps_unregister_data_callback(mpsCh, 1 /* upstream */);
         if (!VMMC_SUCCESS(ret))
         {
            TRACE (VMMC, DBG_LEVEL_HIGH,
      ...
      TRACE(VMMC, DBG_LEVEL_LOW,
            ("VMMC_UnRegister_Callback(), unregistered for mpsCh=%d (%s) %s\n",
            mpsCh,
            mpsCh==command ? "command" : "voice",
            VMMC_SUCCESS(ret) ? "success":"error"));
      if (!VMMC_SUCCESS(ret))
...
static IFX_void_t VMMC_Upstream_Callback(mps_devices mpsChannel)
{
   ...
   else
   {
      ...
      */
      nCh = (IFX_uint8_t) mpsChannel - voice0;
   ...
   switch (pEvt->val[0] & VMMC_EVT_ID_MASK)
   {
      ...
      case VMMC_EVT_ID_ALI_RAW_HOOK:
         ...
         tapiEvent.data.hook_int.nTime =
            VMMC_ALM_ElapsedTimeSinceLastHook(pCh,
               pEvt->evt_ali_raw_hook.TIME_STAMP);
         IFX_TAPI_Event_Dispatch(pChannel, &tapiEvent);
         VMMC_EVAL_EVENT(pCh, tapiEvent);
         break;
         ...
         if (pCh->TapiFaxStatus.nStatus & IFX_TAPI_FAX_T38_DP_ON)
         {
            ...
            switch (pEvt->evt_cod_fax_req.FDP_EVT)
            {
               case EVT_COD_FAX_REQ_FDP_EVT_FDP_REQ:
                  IFX_TAPI_DownStream_RequestData(pChannel, IFX_TRUE);
                  pCh->nFdpReq++;
