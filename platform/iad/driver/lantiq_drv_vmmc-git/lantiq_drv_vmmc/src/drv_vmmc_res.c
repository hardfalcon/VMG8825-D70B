...
struct IFX_TAPI_HDLC_FIFO_ELEM
{
...
IFX_void_t VMMC_RES_StructuresInit (VMMC_DEVICE *pDev)
{
   ...
   for (i=0; i < pDev->caps.nHDLC; i++)
   {
      ...
      pDev->pResHdlc[i].pIngressFifo =
         fifoInit(VMMC_HDLC_MAX_FIFO_SIZE,
                  sizeof(struct IFX_TAPI_HDLC_FIFO_ELEM));
                                 ...
                                 IFX_int32_t nLen)
{
   ...
   {
      struct IFX_TAPI_HDLC_FIFO_ELEM sElem;

      sElem.pBuf = (IFX_void_t *)pBuf;
      sElem.nLength = nLen;

      if (IFX_SUCCESS != fifoPut (pResHdlc->pIngressFifo, &sElem))
      {
...
*/
IFX_int32_t VMMC_RES_HDLC_InfoGet (struct seq_file *s,
                                   VMMC_DEVICE *pDev,
                                   IFX_uint16_t nResNr)
{
   VMMC_RES_HDLC_t  *pResHdlc = IFX_NULL;
   ...
   seq_printf (s, "%3d %3d %3d "
                  ...
                  fifoSize(pResHdlc->pIngressFifo),
                  fifoElements(pResHdlc->pIngressFifo));
...
IFX_void_t irq_VMMC_RES_HDLC_DD_MBX_Handler (VMMC_DEVICE *pDev)
{
   ...
   for (i=0; i < pDev->caps.nHDLC; i++)
   {
      ...
      if (fifoPeek(pResHdlc->pIngressFifo, &sElem) == IFX_SUCCESS)
      {
         VMMC_CHANNEL   *pCh = IFX_NULL;
         mps_message    msg;
         ...
         memset (&msg, 0, sizeof (msg));
         msg.cmd_type = DAT_PAYL_PTR_MSG_HDLC_PACKET;
         msg.pData = sElem.pBuf;
         msg.nDataBytes = sElem.nLength;
         ...
         if (VMMC_SUCCESS (err))
         {
            ...
            (void)fifoGet(pResHdlc->pIngressFifo, &sElem);
