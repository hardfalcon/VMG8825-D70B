...
static void __init vpe1mem(void)
{
	...
	vpe1_mem = 1 * 1024 * 1024;

	addr = kmalloc(vpe1_mem, GFP_KERNEL);
	if (!addr)
		panic("failed to alloc VPE1 memory\n");

	vpe1_load_addr = (unsigned int)addr;

	snprintf(command_line, sizeof(command_line) - 1,
		"vpe1_mem=1M vpe1_load_addr=0x%p", addr);

	pr_info("VPE1 command line: %s\n", command_line);
}
