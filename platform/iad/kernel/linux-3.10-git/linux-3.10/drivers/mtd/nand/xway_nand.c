...
static void xway_poll_wr_c(void)
{
	while ((ltq_ebu_r32(EBU_NAND_WAIT) & NAND_WAIT_WR_C) == 0)
			cpu_relax();
}

static void xway_poll_wr_c_timeout(void)
{
	unsigned long timeout = jiffies + msecs_to_jiffies(20);

	do {
		if (ltq_ebu_r32(EBU_NAND_WAIT) & NAND_WAIT_WR_C)
			return;
		cond_resched();
...
static unsigned char xway_read_byte(struct mtd_info *mtd)
{
	...
	spin_lock_irqsave(&ebu_lock, flags);
	ret = readb((void __iomem *)(nandaddr | NAND_READ_DATA));
	spin_unlock_irqrestore(&ebu_lock, flags);
	xway_poll_wr_c();
...
static void xway_read_buf(struct mtd_info *mtd, u_char *buf, int len)
{
	...
	spin_lock_irqsave(&ebu_lock, flags);
	for (i = 0; i < len; i++)
		buf[i] = readb((void __iomem *)(nandaddr | NAND_READ_DATA));
	spin_unlock_irqrestore(&ebu_lock, flags);
	xway_poll_wr_c();
}
...
static void xway_write_buf(struct mtd_info *mtd, const u_char *buf, int len)
{
	...
	spin_lock_irqsave(&ebu_lock, flags);
	for (i = 0; i < len; i++)
		writeb(buf[i], (void __iomem *) (nandaddr | NAND_WRITE_DATA));
	spin_unlock_irqrestore(&ebu_lock, flags);
	xway_poll_wr_c();
}
...
static int xway_chip_fixup(struct mtd_info *mtd)
{
	...
	} else if (mtd_nand_has_bch() && chip->ecc_strength_ds > 1) {
