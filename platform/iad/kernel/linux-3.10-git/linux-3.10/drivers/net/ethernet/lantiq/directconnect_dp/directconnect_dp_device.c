                     ...
                     int32_t status, uint32_t flags)
{
    ...
    if (port_id >= DC_DP_HW_PORT_RANGE_START && port_id <= DC_DP_HW_PORT_RANGE_END) {
        dev_idx = port_id;
        if (g_dc_dp_hw_dev[dev_idx].flags == status) {
           if (pp_devinfo) {
               *pp_devinfo = &g_dc_dp_hw_dev[dev_idx];
               ret = 0;
           }
    ...
    {
        dev_idx = port_id - DC_DP_SW_PORT_RANGE_START;
        if (g_dc_dp_sw_dev[dev_idx].flags == status) {
            if (pp_devinfo) {
                *pp_devinfo = &g_dc_dp_sw_dev[dev_idx];
                ret = 0;
            }
...
static int32_t
__dc_dp_free_device(int32_t port_id, struct net_device *dev, int32_t t_status, uint32_t flags)
{
    int32_t ret = -1;
    ...
    if ( (ret = __dc_dp_find_device(port_id, dev, &p_devinfo)) == 0 ) {
        if (t_status == DC_DP_DEV_PORT_RESERVED) {
            tmp_spid = p_devinfo->start_port_id;
            memset(p_devinfo, 0, sizeof(struct dc_dp_priv_dev_info));
            p_devinfo->flags = DC_DP_DEV_PORT_RESERVED;
            p_devinfo->owner = THIS_MODULE;
            p_devinfo->port_id = port_id;
            p_devinfo->start_port_id = tmp_spid;
        } else
            memset(p_devinfo, 0, sizeof(struct dc_dp_priv_dev_info));
    }
                   ...
                   int32_t s_status, int32_t t_status, uint32_t flags)
{
    ...
    if ( (ret = __dc_dp_alloc_device(port_id, dev, &p_devinfo, s_status, flags)) == 0 )
    {
        p_devinfo->port_id = port_id;
        p_devinfo->dev = dev;
        p_devinfo->flags = t_status;
    }
...
dc_dp_free_device(int32_t port_id, struct net_device *dev, int32_t t_status, uint32_t flags)
{
    ...
    __dc_dp_free_device(port_id, dev, t_status, flags);
