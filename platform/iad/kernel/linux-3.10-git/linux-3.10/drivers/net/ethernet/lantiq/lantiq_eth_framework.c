...
static struct net_device *eth_alloc_netdev(int sizeof_priv, const char *name)
{
	...
	dev->priv_flags |= IFF_LTQ_ETH_FRAMEWORK;
...
const char *ifx_eth_fw_netdev_physname(struct net_device *dev)
{
    struct ifx_eth_fw_priv_data *priv;

    if (!(dev->priv_flags & IFF_LTQ_ETH_FRAMEWORK))
        return dev->name;
...
EXPORT_SYMBOL(ifx_eth_fw_netdev_physname);

const char *ifx_eth_fw_netdev_realname(struct net_device *dev)
{
    return dev->name;
}
EXPORT_SYMBOL(ifx_eth_fw_netdev_realname);

struct net_device *ifx_eth_fw_find_by_physname(const char *name)
{
    unsigned long sys_flags;
    struct list_head *list;
    struct ifx_eth_fw_priv_data *priv;
...
EXPORT_SYMBOL(ifx_eth_fw_find_by_physname);
