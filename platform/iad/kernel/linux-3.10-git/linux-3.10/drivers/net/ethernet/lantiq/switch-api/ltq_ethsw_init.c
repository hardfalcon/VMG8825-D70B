...
#if IS_ENABLED(CONFIG_LTQ_ETHSW_API_EXTERNAL_INIT)
int ltq_ethsw_api_init(void __iomem *regbase)
{
	ethsw_core_init_t core_init;
	ethsw_api_dev_t *api_dev;
	ioctl_wrapper_ctx_t *wrapper;
	ioctl_wrapper_init_t wrapper_init;
	int err;

	gsw_regbase = regbase;

	err = gsw_api_misc_register();
	if (err)
		return err;

	core_init.eDev = LTQ_FLOW_DEV_INT;
	api_dev = ethsw_api_core_init(&core_init);
	if (!api_dev) {
		err = -ENODEV;
		goto err_misc;
	}

	api_dev->nCPU_Port = LTQ_SOC_CPU_PORT;
	api_dev->regbase = regbase;
	wrapper_init.pLlTable = &ifx_flow_fkt_tbl;
	wrapper_init.default_handler = NULL;
	wrapper = ioctl_wrapper_init(&wrapper_init);
	if (!wrapper) {
		err = -ENODEV;
		goto err_cleanup;
	}

	err = ioctl_wrapper_dev_add(wrapper, api_dev, LTQ_INT_GSWITCH);
	if (err) {
		err = -ENODEV;
		goto err_wrapper;
	}
...
err_misc:
	gsw_api_misc_unregister();

	return err;
}
EXPORT_SYMBOL(ltq_ethsw_api_init);

void ltq_ethsw_api_exit(void)
{
	gsw_api_ioctl_wrapper_cleanup();
	gsw_CoreCleanUP();
	gsw_api_misc_unregister();
}
EXPORT_SYMBOL(ltq_ethsw_api_exit);
#else
int ltq_ethsw_api_register(struct platform_device *pdev)
{
	...
	if (result != 0) {
		pr_err("%s:%d (Register Char Device Failed) \n",	\
			__func__, __LINE__);
		return result;
	}
	/*Enable Switch Power  */
