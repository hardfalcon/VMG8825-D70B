...
#define ETHSW_API_DEV_NAME "switch_api!0"
...
int gsw_command_search(void *pHandle, int command, int arg, ethsw_api_type_t apiType)
{
	...
	/* Number out of range. No function available for	\
		this command number. */
			...
			if (fkt == NULL) {
				pr_debug("ERROR %s[%d]: cmdNr=%d, nNumFkts=%d \n",\
...
static struct miscdevice gsw_api_dev = {
	.minor = MISC_DYNAMIC_MINOR,
	.name = ETHSW_API_DEV_NAME,
	.fops = &swapi_fops,
};

int gsw_api_misc_register(void)
{
	return misc_register(&gsw_api_dev);
}
EXPORT_SYMBOL(gsw_api_misc_register);

void gsw_api_misc_unregister(void)
{
	misc_deregister(&gsw_api_dev);
}
EXPORT_SYMBOL(gsw_api_misc_unregister);
...
int ioctl_wrapper_dev_add(ioctl_wrapper_ctx_t *pIoctlDev, void *pCoreDev, u8 nMinorNum)
{
	if (nMinorNum > LTQ_GSW_DEV_MAX) {
		pr_err("(Device number: %d) %s:%d \n",	\
			nMinorNum, __func__, __LINE__);
		return -1;
	}
