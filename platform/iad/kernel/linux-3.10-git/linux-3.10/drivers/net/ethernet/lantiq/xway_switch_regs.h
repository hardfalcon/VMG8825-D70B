...
#ifndef __XWAY_SWITCH_REGS_H__
#define __XWAY_SWITCH_REGS_H__
...
struct vr9_switch_regs {
	...
	struct xrx_switch_bm_regs bm;
	struct xrx_switch_pce_regs pce;
	struct xrx_switch_mac_regs mac;
	struct xrx_switch_fdma_regs fdma;
	struct xrx_switch_sdma_regs sdma;
	struct xrx_switch_mdio_regs mdio;
	struct vr9_switch_mii_regs mii;
	struct xrx_switch_pmac_regs pmac;
};
...
static inline void *to_pce_tbl_key(struct xrx_switch_pce_regs *regs,
					unsigned int id)
{
	return &regs->core.tbl_key[to_pce_tbl_key_id(id)];
}

static inline void *to_pce_tbl_value(struct xrx_switch_pce_regs *regs,
						unsigned int id)
{
	return &regs->core.tbl_val[to_pce_tbl_value_id(id)];
}

static inline void *to_mac_ctrl(struct xrx_switch_mac_regs *regs,
					unsigned int id, unsigned int ctrl)
{
	struct mac_port *mac = &regs->port[id];
...
static inline void *to_mdio_phyaddr(struct xrx_switch_mdio_regs *regs,
					unsigned int id)
{
	return &regs->phy_addr[to_mdio_phyaddr_id(id)];
}

static inline void *vr9_to_mii_miicfg(struct vr9_switch_mii_regs *regs,
					unsigned int id)
{
	switch (id) {
	case 0:
		return &regs->mii_cfg0;
	case 1:
		return &regs->mii_cfg1;
	case 5:
		return &regs->mii_cfg5;
	default:
		BUG();
	}
}

static inline void *ar10_to_mii_miicfg(struct ar10_switch_mii_regs *regs,
					unsigned int id)
{
	switch (id) {
	case 0:
...
static inline void *vr9_to_mii_pcdu(struct vr9_switch_mii_regs *regs,
					unsigned int id)
{
	switch (id) {
	case 0:
...
static inline void *ar10_to_mii_pcdu(struct ar10_switch_mii_regs *regs,
					unsigned int id)
{
	switch (id) {
	case 0:
...
#define XWAY_SWITCH_REG_OFFSET(reg)	(4 * (reg))

#define CHECK_VR9_SWITCH_REG(name, offset)	\
	BUILD_BUG_ON(offsetof(struct vr9_switch_regs, name) != (4 * offset))

#define CHECK_AR10_SWITCH_REG(name, offset)	\
	BUILD_BUG_ON(offsetof(struct ar10_switch_regs, name) != (4 * offset))

static inline void build_check_registers(void)
{
	CHECK_VR9_SWITCH_REG(core, 0x0);
	CHECK_VR9_SWITCH_REG(bm.core, 0x40);
	CHECK_VR9_SWITCH_REG(bm.core.queue_gctrl, 0x4a);
	CHECK_VR9_SWITCH_REG(bm.port[0], 0x80);
	CHECK_VR9_SWITCH_REG(bm.queue, 0x100);
	CHECK_VR9_SWITCH_REG(bm.shaper, 0x140);
	CHECK_VR9_SWITCH_REG(pce.core, 0x438);
	CHECK_VR9_SWITCH_REG(pce.core.tbl_ctrl, 0x44f);
	CHECK_VR9_SWITCH_REG(pce.core.parser_stat, 0x469);
	CHECK_VR9_SWITCH_REG(pce.port[0], 0x480);
	CHECK_VR9_SWITCH_REG(pce.meter[0], 0x580);
	CHECK_VR9_SWITCH_REG(mac.core, 0x8c0);
	CHECK_VR9_SWITCH_REG(mac.port[0].pstat, 0x900);
	CHECK_VR9_SWITCH_REG(mac.port[0].ctrl_0, 0x903);
	CHECK_VR9_SWITCH_REG(mac.port[1].pstat, 0x90c);
	CHECK_VR9_SWITCH_REG(mac.port[1].ctrl_0, 0x90f);
	CHECK_VR9_SWITCH_REG(mac.port[2].pstat, 0x918);
	CHECK_VR9_SWITCH_REG(mac.port[2].ctrl_0, 0x91b);
	CHECK_VR9_SWITCH_REG(fdma.core, 0xa40);
	CHECK_VR9_SWITCH_REG(fdma.port[0], 0xa80);
	CHECK_VR9_SWITCH_REG(sdma.core, 0xb40);
	CHECK_VR9_SWITCH_REG(sdma.port[0], 0xbc0);
	CHECK_VR9_SWITCH_REG(mdio, 0xc40);
	CHECK_VR9_SWITCH_REG(mii.mii_cfg0, (0xc40 + 0x36));
	CHECK_VR9_SWITCH_REG(mii.pcdu0, (0xc40 + 0x37));
	CHECK_VR9_SWITCH_REG(mii.mii_cfg1, (0xc40 + 0x38));
	CHECK_VR9_SWITCH_REG(mii.pcdu1, (0xc40 + 0x39));
	CHECK_VR9_SWITCH_REG(mii.mii_cfg5, (0xc40 + 0x40));
	CHECK_VR9_SWITCH_REG(mii.pcdu5, (0xc40 + 0x41));
	CHECK_VR9_SWITCH_REG(pmac, (0xc40 + 0x82));

	CHECK_AR10_SWITCH_REG(mii.mii_cfg0, (0xc40 + 0x36));
	CHECK_AR10_SWITCH_REG(mii.pcdu0, (0xc40 + 0x37));
	CHECK_AR10_SWITCH_REG(mii.mii_cfg3, (0xc40 + 0x3c));
	CHECK_AR10_SWITCH_REG(mii.mii_cfg5, (0xc40 + 0x40));
	CHECK_AR10_SWITCH_REG(mii.pcdu5, (0xc40 + 0x41));
}
...
#define MAC_CTRL0_FCON_MASK	(0x3 << MAC_CTRL0_FCON_SHIFT)
...
#define MAC_CTRL0_FDUP_MASK	(0x3 << MAC_CTRL0_FDUP_SHIFT)
...
#define MAC_CTRL0_GMII_MASK     0x3
...
#define MII_CFG_MIIMODE_GMIIM	0x9
#define MII_CFG_MIIMODE_GMIIM_3	0x1
...
#define MDIO_GLOB_CTRL_SE	BIT(15)
#define MDIO_MDC_CFG1_RES	BIT(15)
#define MDIO_MDC_CFG1_MCEN	BIT(8)
