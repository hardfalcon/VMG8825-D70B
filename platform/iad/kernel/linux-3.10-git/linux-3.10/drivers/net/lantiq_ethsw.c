...
static int ethswitch_init(struct net_device *dev)
{
  ...
  /* Workaround : Set NETIF_F_SG as features, where base device does not support it */
  dev->features    |= NETIF_F_SG;
  dev->hw_features = basedev->hw_features & ETHSW_FEATURES;
  /* Workaround : Set NETIF_F_SG as hw_features, to be controlled by ethtool */
  dev->hw_features |= NETIF_F_SG;
  dev->vlan_features = basedev->vlan_features & ETHSW_FEATURES;
  /* Workaround : Set NETIF_F_SG as vlan_features, to be inherited by vlan device */
  dev->vlan_features |= NETIF_F_SG;
  dev->hw_enc_features = basedev->hw_enc_features & ETHSW_FEATURES;
  /* Workaround : Set NETIF_F_SG as hw_enc_features, to be inherited by tunnel device */
