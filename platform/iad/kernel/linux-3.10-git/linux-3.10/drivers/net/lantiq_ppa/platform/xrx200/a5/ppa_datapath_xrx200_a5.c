...
static uint32_t lan_port_seperate_enabled=1, wan_port_seperate_enabled=0;
...
#define ENABLE_CONFIGURABLE_DSL_VLAN            0
...
#define err(format, arg...)                     do { if ( (g_dbg_enable & DBG_ENABLE_MASK_ERR) ) pr_err("%s: " format "\n", __FUNCTION__, ##arg); } while ( 0 )

#if defined(ENABLE_DEBUG_PRINT) && ENABLE_DEBUG_PRINT
  #undef  dbg
  #define dbg(format, arg...)                   do { if ( (g_dbg_enable & DBG_ENABLE_MASK_DEBUG_PRINT) ) pr_info("%s: " format "\n", __FUNCTION__, ##arg); } while ( 0 )
#else
  #if !defined(dbg)
  ...
  #define ASSERT(cond, format, arg...)          do { if ( (g_dbg_enable & DBG_ENABLE_MASK_ASSERT) && !(cond) ) pr_err("%s: " format "\n", __FUNCTION__, ##arg); } while ( 0 )
#else
...
static int32_t ppa_route_start_iteration(uint32_t *ppos, struct rout_forward_compare_tbl **pcompare, uint32_t type);
static int32_t ppa_route_iterate_next(uint32_t *ppos, struct rout_forward_compare_tbl **pcompare, uint32_t type);
 ...
 *  External Functions
 */
extern void (*atm_hook_mpoa_setup)(struct atm_vcc *, int, int);
#if defined(CONFIG_LTQ_OAM) || defined(CONFIG_LTQ_OAM_MODULE)
  extern void ifx_push_oam(struct atm_vcc *atmvcc, unsigned char *);
#else
  static inline void ifx_push_oam(struct atm_vcc *atmvcc, unsigned char *dummy) {}
#endif
...
static inline void prio_qid_map_init(int port)
{
    ...
    pq_map_init(pq_map, 8);
    pq_map_map_iptos(pq_map);
}
...
#if (defined(ENABLE_DBG_PROC) && ENABLE_DBG_PROC) || (defined(DEBUG_SKB_SWAP) && DEBUG_SKB_SWAP)
static int g_dump_cnt = -1;
#endif

static struct clk *g_ppe_clk;
static struct clk *g_fpi_clk;
...
static struct ppe_directpath_data g_ppe_directpath_data[MAX_DIRECTPATH_NUM];    //  port 0 - eth0, 1 - eth1, 2 - cpu, 6 - dsl (PPPoA/IPoA), 7 - dsl (EoA)
...
{NULL, NULL, (void *)ltq_mei_atm_showtime_check_local};
 ...
 #endif
#else
static inline int ifx_mei_atm_led_blink(void)
{
    return 0;
}
#endif
    ...
    case PPE_ATM_MAP_PKT_PRIO_TO_Q:
        {
            ...
            return pq_map_set_queue(&g_atm_priv_data.connection[conn].prio_qid_map, cmd.pkt_prio, cmd.qid);
        }
        break;
    ...
    case PPE_ATM_GET_MAP_PKT_PRIO_TO_Q:
        {
            ...
            if ( conn >= 0 )
            {
                ...
                for ( i = 0; i < 8; i++ )
                {
                    qid = pq_map_get_queue(&g_atm_priv_data.connection[conn].prio_qid_map, i);
                    cmd.pkt_prio[i] = i;
                    cmd.qid[i] = sw_tx_queue_to_virt_queue_map[qid];
                }
...
static int ppe_open(struct atm_vcc *vcc)
{
    ...
    *WTX_QUEUE_CONFIG(sw_tx_queue) = wtx_queue_config;
    ...
    pq_map_init(&g_atm_priv_data.connection[conn].prio_qid_map, 0);
    pq_map_add_queue(&g_atm_priv_data.connection[conn].prio_qid_map, sw_tx_queue);
...
static void ppe_close(struct atm_vcc *vcc)
{
    ...
    pq_map_release(&connection->prio_qid_map);
...
static int ppe_send(struct atm_vcc *vcc, struct sk_buff *skb)
{
    ...
    qid = pq_map_get_queue(&g_atm_priv_data.connection[conn].prio_qid_map, skb->priority);
    ...
    g_atm_priv_data.connection[conn].prio_tx_packets[qid]++;

    dump_skb(skb, DUMP_SKB_LEN, "ppe_send", 7, qid, 1);
...
static int ppe_send_oam(struct atm_vcc *vcc, void *cell, int flags)
{
    ...
    qid = pq_map_get_queue(&g_atm_priv_data.connection[conn].prio_qid_map, 7);
    ...
    dump_skb(skb, DUMP_SKB_LEN, "ppe_send_oam", 7, qid, 1);
...
static int sw_tx_queue_add(int conn)
{
    ...
    *WTX_QUEUE_CONFIG(new_sw_tx_queue) = wtx_queue_config;
    ...
    pq_map_add_queue(&g_atm_priv_data.connection[conn].prio_qid_map, new_sw_tx_queue);
...
static int sw_tx_queue_del(int conn)
{
    ...
    pq_map_del_queue(&g_atm_priv_data.connection[conn].prio_qid_map, rem_sw_tx_queue);
...
#else
    u32 qsb_clk = clk_get_rate (g_fpi_clk);
    dbg("qsb_clk = %u\n", qsb_clk);
#endif
...
static unsigned int ppe_fw_mode        = 0; // 0: normal mode, 1: mix mode
static unsigned int ppe_wan_hash_algo  = 0; // 0: using algo 0, 1: using algo 1
static unsigned int acc_proto      = 0; // 0: UDP, 1:TCP

static void setup_acc_action_tbl(void)
{
    unsigned int idx;
...
static void init_acc_tables(void)
{
...
static INLINE void init_communication_data_structures(int fwcode)
{
...
#else
    ps_mc_gencfg3.time_tick = clk_get_rate (g_ppe_clk) / 125000 / 2;    //  8 * (cgu_get_pp32_clock() / 1000000), 8 microsecond, half of QoS time tick
#endif
    ...
    *CFG_QID2CLASS_MAP(1) = 0;
    ...
    for ( j = 0; j < 8; j++ )
    {
        *CFG_CLASS2QID_MAP(0) |= j << (j * 4);
        *CFG_CLASS2QID_MAP(1) |= (j | 8) << (j * 4);
    }
...
#else
        tx_qos_cfg.time_tick    = clk_get_rate (g_ppe_clk) / 62500; //  16 * (cgu_get_pp32_clock() / 1000000)
#endif
...
#else
    seq_printf(seq, "PS_MC_GENCFG3 (0x%08x): time_tick - %d, basic tick - %luus, enable - %s\n", (unsigned int)PS_MC_GENCFG3, PS_MC_GENCFG3->time_tick, PS_MC_GENCFG3->time_tick * 1000 / (clk_get_rate(g_ppe_clk) / 1000), PS_MC_GENCFG3->psave_en ? "yes" : "no");
#endif
...
#else
                PS_MC_GENCFG3->time_tick = (clk_get_rate(g_ppe_clk)) / 125000;
#endif
    ...
    for ( i = 0; i < ATM_QUEUE_NUMBER; i++ )
    {
        if ( g_atm_priv_data.connection[i].vcc != NULL )
        {
            seq_printf(seq,"  PVC %d.%d:\n",
                g_atm_priv_data.connection[i].vcc->vpi,
                g_atm_priv_data.connection[i].vcc->vci);
            pq_map_seq_show(&g_atm_priv_data.connection[i].prio_qid_map, seq);
        }
    }
...
static ssize_t proc_write_prio_seq(struct file *file, const char __user *buf, size_t count, loff_t *data)
{
    ...
    while ( get_token(&p1, &p2, &len, &colon) )
    {
        if ( stricmp(p1, "help") == 0 )
        {
            printk("echo <eth0/eth1/pvc vpi.vci> prio xx queue xx > /proc/eth/prio\n");
            printk("echo <eth0/eth1/pvc vpi.vci> flush > /proc/eth/prio\n");
        ...
        else if ( port != ~0 || conn >= 0 )
        {
            ...
            else if ( stricmp(p1, "q") == 0 || stricmp(p1, "queue") == 0 )
            {
                ...
                {
                    if (pq_map_set_queue(&g_atm_priv_data.connection[conn].prio_qid_map, prio, queue))
                        err("failed to set PVC%d prio %d queue %d", conn, prio, queue);
                }
            }
            else if (stricmp(p1, "flush") == 0)
            {
                if (port >= 0 && port <= 1)
                    ...
                    pq_map_flush(&g_atm_priv_data.connection[conn].prio_qid_map);
                break;
            }
...
#else
    seq_printf(seq, "  PPE clk=%lu MHz, basic tick=%lu\n", (clk_get_rate(g_ppe_clk) / 1000000), TX_QOS_CFG->time_tick / (clk_get_rate(g_ppe_clk) / 1000000));
#endif
...
static INLINE void dump_skb(struct sk_buff *skb, u32 len, char *title, int port, int ch, int is_tx)
{
    ...
    if ( ch >= 0 )
        ...
        printk("%s\n", title);
    printk("  skb->data %08X, skb->tail %08X, skb->len %u, skb->priority %u, skb->mark %u, headroom %u\n",
        (u32)skb->data, (u32)skb->tail, skb->len, skb->priority, skb->mark, skb_headroom(skb));
    for ( i = 1; i <= len; i++ )
    {
        if ( i % 16 == 1 )
            printk("  %4d:", i - 1);
...
static int atm_showtime_enter(const unsigned char line_idx, struct port_cell_info *port_cell, void *xdata_addr)
{
    ...
    *UTP_CFG = 0x0F;
    ...
    for (i = 0; i < ATM_PORT_NUMBER; i++)
        atm_dev_signal_change(g_atm_priv_data.port[i].dev, ATM_PHY_SIG_FOUND);
...
static int atm_showtime_exit(const unsigned char line_idx)
{
    ...
    for (i = 0; i < ATM_PORT_NUMBER; i++)
        atm_dev_signal_change(g_atm_priv_data.port[i].dev, ATM_PHY_SIG_LOST);
...
int get_netif_qid_with_pkt(struct sk_buff *skb, void *arg, int is_atm_vcc)
{
    if ( is_atm_vcc )
    {
        ...
        qid = pq_map_get_queue(&g_atm_priv_data.connection[conn].prio_qid_map,
            skb->priority);
...
static int ppe_directpath_send(uint32_t if_id, struct sk_buff *skb, int32_t len, uint32_t flags)
{
    int ret;
    uint32_t off;
    unsigned long sys_flag;
...
static int ppe_directpath_rx_stop(uint32_t if_id, uint32_t flags)
{
    return 0;
}

static int ppe_directpath_rx_start(uint32_t if_id, uint32_t flags)
{
    return 0;
}
#endif
...
static int ppe_pwm_change(unsigned int arg, unsigned int flags)
{
...
static int __init ppe_eth_init(void)
{
    ...
    pr_info("Loading A5 (MII0/1 + ATM) driver ...... ");

    g_ppe_clk = clk_get_ppe();
    g_fpi_clk = clk_get_fpi();
