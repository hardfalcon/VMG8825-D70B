...
#define BEARER_CHANNEL_PER_LINE                 2
  ...
  #define SAVE_AND_SWITCH_PPE_BASE_TO_SM(_SAVED_ADDR)   \
        static unsigned int _SAVED_ADDR;                       \
...
static struct pq_map g_prio_qid_map[3];
...
static struct ppe_directpath_data g_ppe_directpath_data[MAX_DIRECTPATH_NUM];
...
static int ptm_hard_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
    ...
    if ( skb->cb[13] == 0x5A )  //  magic number indicating forcing QId (e.g. directpath)
        ...
        qid = prio_qid_map_get(7, skb->priority);
    ...
    case IFX_PTM_MAP_PKT_PRIO_TO_Q:
        {
            ...
            if (prio_qid_map_set(7, cmd.pkt_prio, cmd.qid))
                return -EINVAL;
        }
        break;
    default:
...
static INLINE int dma_rx_int_handler(struct dma_device_info *dma_dev)
{
    ...
    skb->len  = len;
    dump_skb(skb, DUMP_SKB_LEN, "dma_rx_int_handler packet data", 0, dma_dev->current_rx_chan, 0, 0);
...
#ifdef CONFIG_LTQ_PORT_MIRROR
    if ( ifx_get_mirror_netdev && (7 == header->src_itf) && ifx_get_mirror_netdev() != NULL )
	{
		mirror_pkt_eth(skb, 0);
	}
#endif
...
static unsigned int ipv6_en            = 0; // 0: IPv6 disabled, 1: IPv6 enabled
...
#else
    ps_mc_gencfg3.time_tick = clk_get_rate (g_ppe_clk) / 125000;    //  8 * (cgu_get_pp32_clock() / 1000000), 8 microsecond, half of QoS time tick
#endif
...
#if 0
static int str2uint(char * p, unsigned int* res, int is_hex)
{
    int i = 0;
    unsigned int uint = 0;
...
static void init_pdma_pftest_pdma_cmd(PDMA_DIRA_CMD * p_cmd, unsigned int offset, unsigned int byte_cnt)
{
    // pdma_cmd0.bc        = 3
    // pdma_cmd0.sar       = 0
    // pdma_cmd0.pdma      = 1
...
static void issue_test_cmd(unsigned int test_type, unsigned int offset, unsigned int byte_cnt, unsigned int test_cnt)
{
    int i = 0;
...
static void check_pf_test_result(void)
{
    PDMA_PF_TEST_CMD test_cmd;
    PDMA_DIRA_CMD pdma_cmd;
    unsigned int cycles;
...
static void check_ds_ll_list(volatile struct DS_BOND_GIF_LL_CTXT * p_ll_ctxt, unsigned char * leading)
{
    ...
    while ( get_token(&p1, &p2, &len, &colon) )
    {
        ...
        else if ( port != ~0 )
        {
            ...
            else if ( stricmp(p1, "q") == 0 || stricmp(p1, "queue") == 0 )
            {
                ...
                {
                    if (prio_qid_map_set(7, prio, queue))
                        err("failed to set PTM prio %d queue %d", prio, queue);
                }
            }
...
#else
    seq_printf(seq, "  PPE clk=%lu MHz, basic tick=%lu\n", (clk_get_rate(g_ppe_clk) / 1000000), TX_QOS_CFG_DYNAMIC->time_tick / (clk_get_rate(g_ppe_clk) / 1000000));
#endif
...
static void mirror_pkt_eth(struct sk_buff *skb, int offset)
{
	...
	//if(skb_headroom(new_skb)>sizeof(struct ethhdr))
	if(0)
	{
		new_skb = skb_copy(skb, GFP_ATOMIC);
		skb_push(new_skb,sizeof(struct ethhdr));
	}
	else
	{
		new_skb = alloc_skb_tx(skb->len + sizeof(struct ethhdr));
		if ( !new_skb )
			return;
		else
		{
			skb_put(new_skb, skb->len + sizeof(struct ethhdr));
			memcpy(new_skb->data + sizeof(struct ethhdr), skb->data, skb->len);
		}
	}
...
static int dsl_showtime_enter(const unsigned char line_idx, struct port_cell_info *port_cell, void *xdata_addr)
{
    ...
    netif_carrier_on(g_ptm_net_dev[0]);
...
//    *UTP_CFG = 0x00;
    ...
    netif_carrier_off(g_ptm_net_dev[0]);
    ...
    else
    {
        ...
        else if ( g_eth_wan_mode == 0 )
        {
            if ( arg == (void *) g_ptm_net_dev[0] )
                return prio_qid_map_get(7, skb->priority);
        }
    }
...
#else
    TX_QOS_CFG_DYNAMIC->time_tick = clk_get_rate(g_ppe_clk) / 62500;
#endif
...
static int __init ppe_eth_init(void)
{
    ...
    pr_info("Loading E5 (MII0/1) driver ...... ");
