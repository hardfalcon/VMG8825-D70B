...
/*@}*/
  ...
  #define dbg(format, arg...)                   do { if ( ppa_hal_dbg_enable  ) pr_info("%s: " format "\n", __FUNCTION__, ##arg); } while ( 0 )
#else
  #if !defined(dbg)
  ...
  #define ASSERT(cond, format, arg...)          do { if ( !(cond) ) pr_err("%s: " format "\n", __FUNCTION__, ##arg); } while ( 0 )
#else
...
#else
    llen = sprintf(pstr, "  PPE clk=%u MHz, basic tick=%u\n",(u32)clk_get_rate(g_ppe_clk)/1000000, (u32)get_basic_time_tick());
#endif
    pstr += llen;
    len += llen;
#ifdef CONFIG_LTQ_PPA_QOS_WFQ
 ...
 */
#if 0
static void get_max_route_entries(uint32_t *p_entry,
                           uint32_t *p_mc_entry)
{
    if ( p_entry )
...
static void get_max_bridging_entries(uint32_t *p_entry)
{
    if ( p_entry )
...
//    bit 1: WAN
#if 0
static void set_if_type(uint32_t if_type,
                 uint32_t if_no)
{
 ...
 */
static uint32_t get_if_type(uint32_t if_no)
{
 ...
 */
#if 0
static void set_if_wfq(uint32_t if_wfq,
                uint32_t if_no)
{
}
...
/*!
  ...
  \return no return value
 */
static void set_dplus_wfq(uint32_t wfq)
{
}
...
/*!
  ...
  \return no return value
 */
static void set_fastpath_wfq(uint32_t wfq)
{
}
#endif
...
#if 0
static int32_t get_ipv6_ip_entry(uint32_t entry,
                          uint32_t ipv6_ip[4])
{
    int x = entry / MAX_IPV6_IP_ENTRIES_PER_BLOCK;
    int i = entry % MAX_IPV6_IP_ENTRIES_PER_BLOCK;
 ...
 */
#if 0
static int32_t get_dest_ip_from_wan_mc_entry(uint32_t entry,
                                      uint32_t *p_ip)
{
    if ( entry >= g_wan_mc_entries )
        return PPA_EINVAL;
...
static int32_t ppe_drv_del_multifield_entry_via_index(int32_t index, uint32_t flag);
...
*/
static uint8_t ppe_drv_multifield_control(uint8_t enable, uint32_t flag)
{
...
/*!
    ...
    \return uint8_t, The return value can be any one of the following:  \n
               ...
               PPA_FAILURE on error
*/
static uint8_t ppe_drv_get_multifield_status(uint32_t *enable, uint32_t flag)
{
...
*/
static int32_t ppe_drv_get_multifield_max_entry(uint32_t flag)
{
...
*/
static int32_t ppe_drv_add_multifield_entry(PPA_MULTIFIELD_FLOW_INFO *p_multifield_info, int32_t *index, uint32_t flag)
{
    int ret=PPA_FAILURE;
    int32_t entry, empty_entry = -1;
...
/*!
    ...
    \return int32_t, return the bytes in the compare table. It can be any one of the following now: \n
               ...
               PPA_FAILURE on error
*/
static int32_t ppe_drv_get_multifield_entry(int32_t index, PPA_MULTIFIELD_FLOW_INFO *p_multifield_info, uint32_t flag)
{
    int ret=PPA_SUCCESS;
    uint32_t compare[4] = {0};
    uint32_t mask[4] = {0};
...
*/
static int32_t ppe_drv_del_multifield_entry(PPA_MULTIFIELD_FLOW_INFO *p_multifield_info, uint32_t flag)
{
    int ret=PPA_FAILURE;
    int32_t entry;
    uint32_t compare[4] = {0};
 ...
 */
#if 0
static uint32_t test_and_clear_hit_stat_batch(uint32_t entry)
{
    if ( (entry & 0x7FFFFFFF) < MAX_ROUTING_ENTRIES )
    {
        uint32_t ret;
...
#if 0
static uint32_t test_and_clear_mc_hit_stat_batch(uint32_t entry)
{
    if ( entry < MAX_WAN_MC_ENTRIES )
    {
...
#if 0
static uint32_t test_and_clear_bridging_hit_stat_batch(uint32_t entry)
{
    return 0;
}
#endif
...
#if defined(MBR_CONFIG) && MBR_CONFIG
#if 1
         if((unsigned long)(WTX_QoS_Q_SHAPER_MAPPING_CFG_SB(queueid)->shaper_base_addr) == (unsigned long)WTX_EG_Q_DEF_SHAPING_CFG)
         {
            if ( rate )
               *rate = 0;
            if ( burst )
...
static int32_t ppa_drv_get_mib(PPA_DSL_QUEUE_MIB *buffer, uint32_t flag)
{
    return -1;
}

static int32_t ppa_drv_clear_mib(PPA_DSL_QUEUE_MIB *buffer, uint32_t flag)
{
    return -1;
}
