...
int atm_callback_set(e_ltq_mei_cb_type type, void *func)
{
    int ret = 0;
    if (func)
    {
...
void *atm_callback_get(e_ltq_mei_cb_type type)
{
    switch (type)
    {
        case LTQ_MEI_SHOWTIME_CHECK:
...
EXPORT_SYMBOL(atm_callback_get);
EXPORT_SYMBOL(atm_callback_set);
