 ...
 */
static void mpoa_setup(struct atm_vcc *vcc, int mpoa_type, int f_llc, struct net_device *dev);
...
#else
  static inline void ifx_push_oam(struct atm_vcc *atmvcc, unsigned char *cell)
  {
    unsigned long sys_flag;
    int i;

    local_irq_save(sys_flag);
    pr_debug("ifx_push_oam\n");
...
//    unsigned int value;
    ...
    memset(&mib_vcc, 0, sizeof(atm_aal5_vcc_x_t));
...
static int ppe_send(struct atm_vcc *vcc, struct sk_buff *skb)
{
    ...
    memset(&reg_desc, 0 , sizeof(struct tx_descriptor));
...
static int sw_tx_queue_add(int conn)
{
    ...
    memset(&wtx_queue_config, 0, sizeof(wtx_queue_config_t));
...
int proc_atm_read_pcie_rst(struct seq_file *seq, void *v)
{
    int len = 0;
...
int vrx218_atm_datapath_init(const ifx_pcie_ep_dev_t *p_vrx218_dev)
{
    int ret;
    int i;
...
#if IS_ENABLED(CONFIG_VRX318_DATAPATH) || IS_ENABLED(CONFIG_VRX518_TC) || IS_ENABLED(CONFIG_LTQ_PPA_A1)
...
void vrx218_atm_datapath_exit(void)
{
    int i;
...
#if IS_ENABLED(CONFIG_VRX518_TC) || IS_ENABLED(CONFIG_VRX518) || IS_ENABLED(CONFIG_VRX318_DATAPATH) || IS_ENABLED(CONFIG_LTQ_PPA_A1)
