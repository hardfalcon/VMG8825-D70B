...
extern int proc_atm_read_pcie_rst(struct seq_file *, void *);
...
extern unsigned int in_atm_sync(unsigned long base, u8 bc);
    ...
    *SOC_ACCESS_VRX218_SB(__DSL_LINK_DOWN,base) = 0;
    ...
    for ( i = 0; i < 1000 && !in_atm_sync(base, 0); i++ )
        udelay(10);
    pr_info(in_atm_sync(base, 0) ? "BC0 is in sync\n" : "BC0 is not in sync\n");
    for ( i = 0; i < 1000 && !in_atm_sync(base, 1); i++ )
        udelay(10);
    pr_info(in_atm_sync(base, 1) ? "BC1 is in sync\n" : "BC1 is not in sync\n");
...
extern void vrx218_ppe_atm_fw_exit(const ifx_pcie_ep_dev_t *dev);
extern void vrx218_ppe_atm_free_mem(const ifx_pcie_ep_dev_t *dev);
//  vrx218_datapath.c
extern int vrx218_atm_datapath_init(const ifx_pcie_ep_dev_t *p_vrx218_dev);
extern void vrx218_atm_datapath_exit(void);

static int init_vrx218_module(const ifx_pcie_ep_dev_t *dev)
{
...
static void exit_vrx218_module(const ifx_pcie_ep_dev_t *dev)
{
...
{
    return single_open(file, proc_atm_read_pcie_rst, NULL);
}
...
int load_atm_tc(void)
{
    int dev_num;
    int i;

    pr_info("VRX318: loading PPE driver and firmware (ATM)\n");
    ...
    pr_info("  %d device(s) are detected!\n", dev_num);

    vrx_tc_msg_send(0, ATM_TC, TC_LOADED, 0, 0);

    return 0;
}

int unload_atm_tc(void)
{
    printk("VRX318: Unloading PPE driver and firmware (ATM)\n");
    ...
    vrx_tc_msg_send(0, ATM_TC, TC_UNLOADED, 0, 0);
