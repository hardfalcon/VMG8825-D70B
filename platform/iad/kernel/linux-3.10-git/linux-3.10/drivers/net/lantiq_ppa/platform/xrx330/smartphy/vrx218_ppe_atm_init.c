...
static void free_dataptr_fragments(void)
{
    unsigned int i;
...
//--------------------
void dword_atm_mem_clear(void *__addr, unsigned int no_of_bytes) {
...
void dword_atm_mem_write(void *__dst_addr, void *__src_addr, unsigned int no_of_bytes) {
...
void vrx218_atm_edma_init(unsigned long base, int lle_in_sb, int cdma_write_data_en)
{
    ...
    dword_atm_mem_clear(&edma_ch_ctrl, sizeof(edma_ch_ctrl));
    ...
    if (cdma_write_data_en == 0) {
        ...
        *EDMA_CH_IDX(pcie_ep_cfg_addr) = EDMA_WRITE_CH;
        ...
        dword_atm_mem_write((unsigned int *)(EDMA_CH_CTRL(pcie_ep_cfg_addr)), (unsigned int *)(&edma_ch_ctrl), sizeof(edma_ch_ctrl));
        ...
        *EDMA_LL_PTR_HIGH(pcie_ep_cfg_addr)     = 0;
        ...
        dword_atm_mem_clear(&edma_lle_link, sizeof(edma_lle_link));
        edma_lle_link.tcb = 1;
        edma_lle_link.llp = 1;
        if (lle_in_sb == 1)
        ...
        dword_atm_mem_clear(dst_addr, EDMA_WCH_DATA_LLE_NUM * sizeof(edma_lle_data_t));
        dst_addr = (void *)((unsigned char *)(dst_addr) + (EDMA_WCH_DATA_LLE_NUM * sizeof(edma_lle_data_t)));
        dword_atm_mem_write(dst_addr, &edma_lle_link, sizeof(edma_lle_link));

        //Setup EDMA Write Channel Context
        dword_atm_mem_clear(&edma_ch_ctxt, sizeof(edma_ch_ctxt));
        ...
        dword_atm_mem_write(dst_addr, &edma_ch_ctxt, sizeof(edma_ch_ctxt));
    ...
    *EDMA_LL_PTR_HIGH(pcie_ep_cfg_addr)     = 0;
    ...
    dword_atm_mem_clear(dst_addr, EDMA_RCH_DATA_LLE_NUM * sizeof(edma_lle_data_t));
...
void vrx218_us_inq_des_init(unsigned long base, int is_bonding, unsigned int vrx218_id)
{
    ...
    dword_atm_mem_clear(&tx_descriptor, sizeof(tx_descriptor));
    ...
    for (i=0; i<64; i++) {
        ...
        dword_atm_mem_write(dst_addr, &tx_descriptor, sizeof(tx_descriptor));
    }
...
//  is_bonding = 1, vrx218_id = 0: bonding application, us_bonding_master
//  is_bonding = 1, vrx218_id = 1: bonding application, ds_bonding_master
void vrx218_atm_ds_soc_des_init(unsigned long base, int is_bonding, unsigned int vrx218_id)
{
    unsigned int i, *dst_addr;
    rx_descriptor_t rx_descriptor;

    dword_atm_mem_clear(&rx_descriptor, sizeof(rx_descriptor));
    ...
    if (is_bonding == 0) {
        ...
        for (i=0; i<SOC_DS_DES_NUM; i++) {
            ...
            dword_atm_mem_write(dst_addr, &rx_descriptor, sizeof(rx_descriptor));
        }
    }
...
void vrx218_atm_us_fastpath_desq_cfg_ctxt_init(unsigned long base)
{
    unsigned int i, *dst_addr;
    desq_cfg_ctxt_t us_fp_desq_cfg_ctxt;
    tx_descriptor_t *p_tx_descriptor;

    dword_atm_mem_clear((unsigned int *)(&us_fp_desq_cfg_ctxt), sizeof(us_fp_desq_cfg_ctxt));
    ...
    dword_atm_mem_write(dst_addr, (unsigned int *)(&us_fp_desq_cfg_ctxt), sizeof(desq_cfg_ctxt_t));
...
void vrx218_atm_us_cpupath_desq_cfg_ctxt_init(unsigned long base)
{
    unsigned int i, *dst_addr;
    desq_cfg_ctxt_t us_cp_desq_cfg_ctxt;
    tx_descriptor_t *p_tx_descriptor;

    dword_atm_mem_clear((unsigned int *)(&us_cp_desq_cfg_ctxt), sizeof(us_cp_desq_cfg_ctxt));
    ...
    dword_atm_mem_write(dst_addr, (unsigned int *)(&us_cp_desq_cfg_ctxt), sizeof(desq_cfg_ctxt_t));
...
void vrx218_atm_ds_desq_cfg_ctxt_init(unsigned long base, int is_bonding, unsigned int vrx218_id, int tc_mode)
{
    unsigned int i, *dst_addr;
    ...
    dword_atm_mem_clear((unsigned int *)(&ds_desq_cfg_ctxt), sizeof(ds_desq_cfg_ctxt));
    ...
    dword_atm_mem_write(dst_addr, (unsigned int *)(&ds_desq_cfg_ctxt), sizeof(desq_cfg_ctxt_t));
...
//  is_bonding = 1, vrx218_id = 1: bonding application, ds_bonding_master
//  tc_mode - 0: PTM Mode; 1: A1+ Mode
void vrx218_atm_cdma_init(unsigned long base, int is_bonding, unsigned int vrx218_id, int cdma_write_data_en, int tc_mode)
{
#ifdef VRX218_A1_DRIVER
...
void vrx218_atm_sb_clear(unsigned long base)
{
    dword_atm_mem_clear((unsigned int *)SOC_ACCESS_VRX218_SB(0x2000, base), 0x5000 * 4);
    dword_atm_mem_clear((unsigned int *)SOC_ACCESS_VRX218_SB(0x8000, base), 0x8000 * 4);
}
...
#define DS_FLOW_CTRL_CFG    0x2026
    *SOC_ACCESS_VRX218_SB(DS_FLOW_CTRL_CFG, base) = 0x3C30;
    ...
    dword_atm_mem_clear((unsigned int *)(&qos_cfg), sizeof(qos_cfg));
    ...
    dword_atm_mem_write(dst_addr, &qos_cfg, sizeof(qos_cfg));

    // to be changed to follow the local -> remote copy pattern
    dword_atm_mem_clear((unsigned int *)(&ps_cfg),sizeof(ps_cfg));
    ps_cfg.start_state   = 0;
    ps_cfg.sleep_en      = 1;    //  Enable sleep mode by default
    dst_addr = (void *)PSAVE_CFG(base);
    dword_atm_mem_write(dst_addr,&ps_cfg,sizeof(ps_cfg));
...
void vrx218_atm_wtx_port_cfg_init(unsigned long base, u8 qsb_en)
{
    ...
    dword_atm_mem_write(dst_addr, &wtx_port_config, sizeof(wtx_port_config));
...
void vrx218_atm_wtx_queue_cfg_init(unsigned long base, u8 qsb_en)
{
    ...
    for (i = 0; i < 16; i++) {
        ...
        dword_atm_mem_write(dst_addr, &wtx_queue_config, sizeof(wtx_queue_config_t));
    }
...
void vrx218_atm_wrx_queue_cfg_init(unsigned long base)
{
    ...
    for (i = 0; i < 2; i++) {
        ...
        dword_atm_mem_write(dst_addr, &wrx_queue_config, sizeof(wrx_queue_config_t));
    }
...
//    outq_qos_cfg_ctxt_t outq_qos_cfg_ctxt;
    ...
    dword_atm_mem_clear((unsigned int *)(&qosq_flow_ctrl_cfg), sizeof(qosq_flow_ctrl_cfg));
    ...
    dword_atm_mem_write(dst_addr, (unsigned int *)(&qosq_flow_ctrl_cfg), sizeof(qosq_flow_ctrl_cfg));

    //Initialize STD_DES_CFG
    dword_atm_mem_clear((unsigned int *)(&std_des_cfg), sizeof(std_des_cfg));
    std_des_cfg.byte_off = 0;
    std_des_cfg.data_len = DMA_PACKET_SIZE - 32;
    dst_addr = (void *)SOC_ACCESS_VRX218_SB(__STD_DES_CFG, base);
    dword_atm_mem_write(dst_addr, (unsigned int *)(&std_des_cfg), sizeof(std_des_cfg));

    //--------------------------------------
    //Setup INQ_QoS_CFG for Fast-Path & CPU-Path
    //--------------------------------------
    dword_atm_mem_clear((unsigned int *)(&fp_qos_cfg), sizeof(fp_qos_cfg));
    dword_atm_mem_clear((unsigned int *)(&cpu_qos_cfg), sizeof(cpu_qos_cfg));
    ...
    dword_atm_mem_write(dst_addr, (unsigned int *)(&fp_qos_cfg), sizeof(fp_qos_cfg));

    dst_addr = (void *)SOC_ACCESS_VRX218_SB((__INQ_QOS_CFG_BASE + __INQ_QOS_CFG_SIZE), base);
    dword_atm_mem_write(dst_addr, (unsigned int *)(&cpu_qos_cfg), sizeof(cpu_qos_cfg));

    //-----------------------------
    //Setup Fast-Path DESQ_CFG_CTXT
    //-----------------------------
    vrx218_atm_us_fastpath_desq_cfg_ctxt_init(base);

    //----------------------------
    //Setup CPU-Path DESQ_CFG_CTXT
    //----------------------------
    vrx218_atm_us_cpupath_desq_cfg_ctxt_init(base);

    //-------------------
    //Setup QoSQ_CFG_CTXT
    //-------------------
    dword_atm_mem_clear((unsigned int *)(&qosq_cfg_ctxt), sizeof(qosq_cfg_ctxt));
    ...
    for( i = 0; i < atm_txq_num(); i ++) {
        ...
        dword_atm_mem_write(dst_addr, (unsigned int *)(&qosq_cfg_ctxt), sizeof(qosq_cfg_ctxt));
    }

    //------------------------------
    //Setup QoSQ PSEUDO DES_CFG_CTXT
    //------------------------------
    dword_atm_mem_clear((unsigned int *)(&desq_cfg_ctxt), sizeof(desq_cfg_ctxt));
    ...
    dword_atm_mem_write(dst_addr, (unsigned int *)(&desq_cfg_ctxt), sizeof(desq_cfg_ctxt));
...
void vrx218_atm_des_sync_init(unsigned long base)
{
    ...
    dword_atm_mem_clear((unsigned int *)(&des_sync_cfg_ctxt), sizeof(des_sync_cfg_ctxt));
    ...
    dword_atm_mem_write(dst_addr, (unsigned int *)(&des_sync_cfg_ctxt), sizeof(des_sync_cfg_ctxt));
...
void vrx218_atm_edma_copy_ch_init(unsigned long base)
{
    ...
    dword_atm_mem_write(dst_addr, (unsigned int *)(&copy_ch_cfg), sizeof(copy_ch_cfg));
    ...
    dword_atm_mem_clear((unsigned int *)(&local_desq_cfg_ctxt), sizeof(local_desq_cfg_ctxt));
    ...
    for (i = 0; i < atm_txq_num(); i++) {
        ...
        dword_atm_mem_write(dst_addr, (unsigned int *)(&local_desq_cfg_ctxt), sizeof(desq_cfg_ctxt_t));
...
void vrx218_ppe_atm_init(const ifx_pcie_ep_dev_t *dev, int qsb_en)
{
    ...
    //A1+ ==> Single Line; no Bonding
    vrx218_atm_ds_soc_des_init(base, 0, 0);

    //Setup Downstream Packet/AAL5 DESQ_CFG_CTXT
    //A1+ ==> Single Line; no Bonding
    //tc_mode = 1
    vrx218_atm_ds_desq_cfg_ctxt_init(base, 0, 0, 1);
    ...
    *SOC_ACCESS_VRX218_SB(0x7DD0, base) = 0x00000007;
    ...
    vrx218_atm_cdma_init(base, 0, 0, 0, 1);
...
unsigned int in_atm_sync(unsigned long base, u8 bc)
{
...
void vrx218_ppe_atm_fw_exit(const ifx_pcie_ep_dev_t *dev)
{
...
void vrx218_ppe_atm_free_mem(const ifx_pcie_ep_dev_t *dev)
{
    unsigned int base = dev->phy_membase;
    unsigned int i, *dst_addr;
    tx_descriptor_t tx_descriptor;
    ...
    for (i=0; i<512; i++) {
        dst_addr = (void *)SOC_ACCESS_VRX218_SB((__US_QOSQ_DES_LIST_BASE + (i * 2)), base);
        dword_atm_mem_write(&tx_descriptor, dst_addr, sizeof(tx_descriptor));
