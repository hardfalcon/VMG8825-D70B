...
int vrx218_ptm_datapath_init(const ifx_pcie_ep_dev_t *p_vrx218_dev_us, const ifx_pcie_ep_dev_t *p_vrx218_dev_ds)
{
    int ret;
    ...
    if(g_dsl_bonding){
        ...
        if( ret != 0 ){
            pr_err("Failed to request PCIe MSI irq %s:%u\n", "vrx318 DS ",g_vrx218_dev_ds.irq);
...
REQUEST_IRQ_FAIL:
    pr_err("PTM datapath initialization fail!!!\n");
...
void vrx218_ptm_datapath_exit(void)
{
#ifdef CONFIG_IFX_LED
    del_timer(&g_dsl_led_polling_timer);
