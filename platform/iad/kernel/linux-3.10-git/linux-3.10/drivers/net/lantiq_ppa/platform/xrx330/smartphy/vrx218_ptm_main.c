...
#if 0
extern void ltq_vectoring_priority(uint32_t priority);
#endif

/*
 *  TC SwitchOver definitions
 */
static int dsl_req_tc_switch(const unsigned char line_no, mei_tc_request_type tc_type, int is_bonding);
static int load_ptm_tc(void);
static int unload_ptm_tc(void);
extern int load_atm_tc(void);
extern int unload_atm_tc(void);
...
extern unsigned int in_sync(unsigned long base, u8 bc);
    ...
    *SOC_ACCESS_VRX218_SB(__DSL_LINK_DOWN,base) = 0;
    ...
    for ( i = 0; i < 1000 && !in_sync(base, 0); i++ )
        udelay(10);
    pr_info(in_sync(base, 0) ? "BC0 is in sync\n" : "BC0 is not in sync\n");
    for ( i = 0; i < 1000 && !in_sync(base, 1); i++ )
        udelay(10);
    pr_info(in_sync(base, 1) ? "BC1 is in sync\n" : "BC1 is not in sync\n");
...
extern void vrx218_ppe_ptm_fw_exit(const ifx_pcie_ep_dev_t *dev, int is_bonding, unsigned int vrx218_id);
extern void vrx218_ppe_ptm_free_mem(const ifx_pcie_ep_dev_t *dev, int is_bonding, unsigned int vrx218_id);
//  vrx218_datapath.c
...
static void exit_ptm_vrx218_module(void)
{
...
static int init_vrx218_bonding_module(void)
{
    pr_info("VRX318_US_BONDING_MASTER\n");
    ...
    *MBOX_IGU1_IER(g_ds_vrx218_dev->phy_membase) = 0x00;
    ...
    pr_info("VRX318_DS_BONDING_MASTER\n");
...
static void exit_vrx218_bonding_module(void)
{
    vrx218_ptm_datapath_exit();
                        ...
                        size_t count, loff_t *data)
{
	...
	volatile unsigned int *ppe_tc_sync = xTM_MODE_TO_DSL(g_vrx218_dev->phy_membase);
	...
	if(strstr(str, "atmload") != NULL) {
		if (*ppe_tc_sync == 0x0) {
                        printk(KERN_INFO " Loading ATM\n");
                        load_atm_tc();
                } else if (*ppe_tc_sync == 0x1) {
                        printk(KERN_INFO " ATM TC is already loaded\n");
                } else if (*ppe_tc_sync == 0x2) {
                        printk(KERN_INFO " PTM TC is already loaded. Try unloading PTM and load ATM\n");
		}
	} else if (strstr(str, "atmunload") != NULL) {
		if (*ppe_tc_sync == 0x1) {
                        printk(KERN_INFO " Unloading ATM\n");
                        unload_atm_tc();
                } else {
                        printk(KERN_INFO " ATM TC is not loaded to unload\n");
                }
	} else if (strstr(str, "ptmload") != NULL) {
		if (*ppe_tc_sync == 0x0) {
			printk(KERN_INFO " Loading PTM\n");
			load_ptm_tc();
		} else if (*ppe_tc_sync == 0x2) {
			printk(KERN_INFO " PTM TC is already loaded\n");
		} else if (*ppe_tc_sync == 0x1) {
			printk(KERN_INFO " ATM TC is already loaded. Try unloading ATM and load PTM\n");
		}
        } else if (strstr(str, "ptmunload") != NULL) {
		if (*ppe_tc_sync == 0x2) {
			printk(KERN_INFO " Unloading PTM\n");
			unload_ptm_tc();
		} else {
			printk(KERN_INFO " PTM TC is not loaded to unload\n");
		}
        }
    ...
    for ( i = 0; i < NUM_ENTITY(g_proc_entry); i++ ) {
        ...
        if ( g_proc_entry[i].is_dir ) {
            ...
            if ( g_proc_entry[i].proc_entry == NULL )
                pr_err("%s: Create proc directory \"%s\" failed!\n", __FUNCTION__, g_proc_entry[i].name);
        }
    }
    for ( i = 0; i < ARRAY_SIZE(g_proc_entry); i++ ) {
...
static ssize_t proc_write_vector_prio(struct file *file, const char __user *buf, size_t count, loff_t *data)
{
	...
	if (str2uint(p,&temp,0) == 1) {
		g_vector_prio = temp;
		ltq_vectoring_priority(g_vector_prio);
	}
	return count;
}
#endif
...
static void dump_qosqctxt(int qid)
{
    ...
    if(qid < 0 || qid >= VRX218_QOSQ_NUM){
        pr_err("QoSQ id is out of range: %d\n", qid);
        return;
    }
...
static void fwdbg_bonding_mode_switch(char **tokens, int token_num)
{
    if(g_ds_vrx218_dev == g_us_vrx218_dev){
        pr_err("Non Bonding mode, quit switch\n");
        return;
    }
...
static void init_host_desc(void)
{
    volatile unsigned long *p;
    unsigned int i;
 ...
 * ln_no: VRX318 ep id
 */
int vrx_tc_msg_send(int pid, int tc_mode, int tc_action, int bnd_mode, int ln_no)
{
        struct sk_buff *skb;
        int ret;
        void *msg_head;
        ...
        if (ret != 0) {
                pr_err("Failed to send out the multicast message\n");
                goto out;
        }
        return 0;
out:
...
static int unload_ptm_tc(void)
{
    ...
    printk("VRX318: Unloading PPE driver and firmware (PTM)\n");
    ...
    vrx_tc_msg_send(0, PTM_TC, TC_UNLOADED, g_dsl_bonding, 1);
...
static int load_ptm_tc(void)
{
    ...
    pr_info("VRX318: loading PPE driver and firmware (PTM)\n");
    ...
    for ( i = 0, valid_dev_num = 0; i < dev_num; i++ ) {
        if ( ifx_pcie_ep_dev_info_req(i, IFX_PCIE_EP_INT_PPE, &g_vrx218_dev[valid_dev_num]) ) {
            pr_err("%s failed to get pcie ep %d information\n", __func__, i);
            continue;
        }
    ...
    pr_info("  total %d device(s) are detected!\n", dev_num);
    if ( valid_dev_num ) {
        if ( g_dsl_bonding )
            pr_info("  %d device(s) are initialized to run in bonding mode!\n", valid_dev_num);
        else
            pr_info("  %d device is initialized to run in single line mode!\n", valid_dev_num);
    }

    vrx_tc_msg_send(0, PTM_TC, TC_LOADED, g_dsl_bonding, 1);

    return 0;
}

static void dsl_req_tc_switch_wq(struct work_struct *wk)
{
	...
	volatile unsigned int *ppe_tc_sync = xTM_MODE_TO_DSL(g_vrx218_dev->phy_membase);
        ...
        switch (g_tc_mode) {
        case TC_ATM_SL_MODE:
		printk(KERN_INFO " TC Request for ATM\n");
		if(*ppe_tc_sync == 0x2) {
			unload_ptm_tc();
			load_atm_tc();
		} else if (*ppe_tc_sync == 0x0) {
			load_atm_tc();
		}
                break;
        case TC_PTM_SL_MODE:
		printk(KERN_INFO " TC Request for PTM\n");
		if(*ppe_tc_sync == 0x1) {
			unload_atm_tc();
			load_ptm_tc();
		} else if (*ppe_tc_sync == 0x0) {
			load_ptm_tc();
                ...
                break;
        default:
		printk(KERN_INFO " Invalid TC Request\n");
        }
}
...
static int dsl_req_tc_switch(const unsigned char line_no, mei_tc_request_type tc_type, int is_bonding)
{
	...
	switch (tc_type) {
        ...
        case MEI_TC_REQUEST_ATM:
                printk(KERN_INFO " TC Request for ATM\n");
		g_tc_mode = TC_ATM_SL_MODE;
                break;
        case MEI_TC_REQUEST_PTM:
                printk(KERN_INFO " TC Request for PTM\n");
		g_tc_mode = TC_PTM_SL_MODE;
                break;
        default:
                printk(KERN_INFO " Invalid TC Request\n");
		g_tc_mode = TC_NONE_MODE;
                return -EINVAL;
        }

	queue_work(system_wq, &g_work);
	return 0;
}
...
static int __init ppe_init(void)
{
    ...
    printk("VRX318: PPE DSL module init\n");
    ...
    if ( vrx218_get_desc_mem_base(&g_host_desc_base) ) {
        pr_err("%s failed to get host descriptor base\n", __func__);
        return -ENOMEM;
    }
    init_host_desc();
    ...
    INIT_WORK(&g_work, dsl_req_tc_switch_wq);
    ...
    proc_create("tc_switch", 0200, NULL, &vrx318_tc_switch_proc_fops);

    vrx318_genetlink_init();

    ppa_callback_set(LTQ_MEI_TC_REQUEST, (void *)dsl_req_tc_switch);
...
static void __exit ppe_exit(void)
{
    ...
    printk("VRX318: PPE DSL module uninit\n");
    ...
    remove_proc_entry("tc_switch", NULL);

    cancel_work_sync(&g_work);

    register_ppe_ext_generic_hook(NULL, NULL);

    ppa_callback_set(LTQ_MEI_TC_REQUEST, (void *)NULL);
...
#ifndef MODULE
static int queue_gamma_map_setup(char *line)
{
    char *p;
    int i;
...
static int qos_queue_len_setup(char *line)
{
	char *p;
	int i;
