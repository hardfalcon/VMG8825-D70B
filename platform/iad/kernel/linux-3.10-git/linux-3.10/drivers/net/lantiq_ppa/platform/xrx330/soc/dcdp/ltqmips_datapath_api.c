...
const char *dp_port_type_str[] = DP_F_FLAG_LIST;
#undef DP_F_ENUM_OR_STRING

#undef DP_F_ENUM_OR_STRING
#define DP_F_ENUM_OR_STRING(name,value, short_name) value
uint32_t dp_port_flag[] = DP_F_FLAG_LIST;
#undef DP_F_ENUM_OR_STRING

const char *dp_subif_status_str[] = {
	"SUBIF_FREE",
	"SUBIF_REGISTERED",
	"Invalid"
};

const char *dp_port_status_str[] = {
	"PORT_NOT_READY",
	"PORT_FREE",
	"PORT_ALLOCATED",
	"PORT_DEV_REGISTERED",
...
static spinlock_t dp_lock;
unsigned int dp_dbg_flag = 0;
static struct dc_dp_res g_dcdp_resources[MAX_DIRECTPATH_NUM];
...
int get_dp_subif_status_str_size(void)
{
...
static void dp_private_resource_free(int port_id) {
	...
	if (pool) {
		for (i = 0; i < dcdp_res->num_bufpools; i++) {
			BUG_ON(!pool->pool);
			dc_dp_free_dma((unsigned int)pool->pool);
			pool++;
		}
	}
	if (dcdp_res->buflist)
...
{
	int i;
	int32_t port_idx = DP_PORT_ID_TO_PORT_IDX(port_id);
	...
	if (flags & DP_F_DEREGISTER) {	/*De-register */
		...
		DP_DBG("de-alloc port %d(%d)\n", port_id, port_idx);

		memset(&dp_port_info[port_idx], 0, sizeof(dp_port_info[port_idx]));

		dp_port_info[port_idx].status = PORT_FREE;
	...
	if (port_id) {		/*with specified port_id */
		if (dp_port_info[port_idx].status != PORT_FREE) {
			DP_DEBUG_ASSERT(i > 0, "module %s (dev_port %d) failed to allocate port %d(%d) used by %s %d\n",
	...
	if (g_ppe_directpath_data[port_idx].flags & 0x80000000) {
		printk("%s: DSL port is already in use [%x]\n", __func__, flags);
		return -1;
	}
	g_ppe_directpath_data[port_idx].flags = (0x80000000 | flags);
	dp_port_info[port_idx].owner = owner;
	dp_port_info[port_idx].dev = dev;
	dp_port_info[port_idx].dev_port = dev_port;
	dp_port_info[port_idx].port_id = DSL_PORT;
	dp_port_info[port_idx].alloc_flags = flags;
			  ...
			  uint32_t flags)
{
	...
	DP_DBG("to cha no: ownner[%p]dev[%p]subif_id[%p]\n", owner, dev, subif_id);
...
//__skip_psi:
	...
	ret = dc_dp_ppe_start_xmit(skb, rx_subif, rx_if, flags);
...
int32_t dp_create_dcdp_res(uint32_t port_id, struct dc_dp_res *dcdp_res)
{
	...
	dev_aca_cnt.tx_in_deq_cnt = (unsigned int)SB_BUFFER(__TOTAL_DEV_ACA_CNT_BASE);
	dev_aca_cnt.tx_out_enq_cnt = (unsigned int)SB_BUFFER(__TOTAL_DEV_ACA_CNT_BASE + 1);
	dev_aca_cnt.rx_in_deq_cnt = (unsigned int)SB_BUFFER(__TOTAL_DEV_ACA_CNT_BASE + 2);
	dev_aca_cnt.rx_out_enq_cnt = (unsigned int)SB_BUFFER(__TOTAL_DEV_ACA_CNT_BASE + 3);
	dccntr->soc2dev_deq_phys_base = (void *)CPHYSADDR(dev_aca_cnt.tx_in_deq_cnt);
	dccntr->soc2dev_deq_base = (void *)dev_aca_cnt.tx_in_deq_cnt;
	dccntr->soc2dev_ret_enq_phys_base = (void *)CPHYSADDR(dev_aca_cnt.tx_out_enq_cnt);
	dccntr->soc2dev_ret_enq_base = (void *)dev_aca_cnt.tx_out_enq_cnt;
	dccntr->dev2soc_enq_phys_base = (void *)CPHYSADDR(dev_aca_cnt.rx_out_enq_cnt);
	dccntr->dev2soc_enq_base = (void *)dev_aca_cnt.rx_out_enq_cnt;
	dccntr->dev2soc_ret_deq_phys_base = (void *)CPHYSADDR(dev_aca_cnt.rx_in_deq_cnt);
	dccntr->dev2soc_ret_deq_base = (void *)dev_aca_cnt.rx_in_deq_cnt;
	...
	ring->base = (void *)SB_BUFFER(__SOC_TX_IN_DES_LIST_BASE);
	ring->phys_base = (void *)CPHYSADDR((SB_BUFFER(__SOC_TX_IN_DES_LIST_BASE)));
	ring->size = __SOC_TX_IN_DES_LIST_NUM;
	ring->desc_dwsz = 2;

	ring = &(dcdp_res->rings.soc2dev_ret);
	ring->base = (void *)SB_BUFFER(__SOC_TX_OUT_DES_LIST_BASE);
	ring->phys_base = (void *)CPHYSADDR(SB_BUFFER(__SOC_TX_OUT_DES_LIST_BASE));
	ring->size = __SOC_TX_OUT_DES_LIST_NUM;
	ring->desc_dwsz = 2;

	ring = &(dcdp_res->rings.dev2soc);
	ring->base = (void *)SB_BUFFER(__SOC_RX_OUT_DES_LIST_BASE);
	ring->phys_base = (void *)CPHYSADDR(SB_BUFFER(__SOC_RX_OUT_DES_LIST_BASE));
	ring->size = __SOC_RX_OUT_DES_LIST_NUM;
	ring->desc_dwsz = 2;

	ring = &(dcdp_res->rings.dev2soc_ret);
	ring->base = (void *)SB_BUFFER(__SOC_RX_IN_DES_LIST_BASE);
	ring->phys_base = (void *)CPHYSADDR(SB_BUFFER(__SOC_RX_IN_DES_LIST_BASE));
