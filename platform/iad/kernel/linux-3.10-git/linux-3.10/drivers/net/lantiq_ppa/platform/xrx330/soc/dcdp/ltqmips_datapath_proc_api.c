...
#undef local_dbg
#if 1
#define local_dbg(fmt, arg...) do { pr_debug(fmt, ##arg);} while ( 0 )
#else
...
static int dp_proc_open(struct inode *inode, struct file *file)
{
	...
	s = file->private_data;
	p = (struct dp_proc_file_entry *)kzalloc(sizeof(*p), GFP_KERNEL);
...
{
	if ( !proc_entry || !proc_entry->name ) return ;
