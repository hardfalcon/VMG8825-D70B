...
#ifdef CONFIG_LTQ_DATAPATH
#include "dcdp/datapath.h"
#endif
...
module_param_string(wlanm, wlanm, sizeof(wlanm),S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(wlanm, "WLAN memory address.");
...
#define err(format, arg...)                     do { if ( (g_dbg_enable & DBG_ENABLE_MASK_ERR) ) pr_err("%d:%s: " format "\n", __LINE__, __FUNCTION__, ##arg); } while ( 0 )

#if defined(ENABLE_DEBUG_PRINT) && ENABLE_DEBUG_PRINT
  #undef  dbg
  #define dbg(format, arg...)                   do { if ( (g_dbg_enable & DBG_ENABLE_MASK_DEBUG_PRINT) ) pr_info("%d:%s: " format "\n", __LINE__, __FUNCTION__, ##arg); } while ( 0 )
#else
  #if !defined(dbg)
  ...
  #define ASSERT(cond, format, arg...)          do { if ( (g_dbg_enable & DBG_ENABLE_MASK_ASSERT) && !(cond) ) pr_err("%d:%s: " format "\n", __LINE__, __FUNCTION__, ##arg); } while ( 0 )
#else
...
#define DMA_PACKET_SIZE                         2048    //  ((1518 + 8 <2 VLAN> + 62 <PPE FW> + 8 <SW Header>) + 31) & ~31
#else
...
#ifdef CONFIG_LTQ_DATAPATH
#define CPU_TO_WAN_518_TX_DESC_BASE	((volatile struct tx_descriptor *)SB_BUFFER(__DSL_CPU_DES_LIST_BASE))
#define GRX518_DMA_TX_CH1_DESC_BASE ((volatile struct rx_descriptor *)SB_BUFFER(__SOC_RX_IN_DES_LIST_BASE))      /*         32 each queue    */
static enum DSL_CHIPSET g_current_dsl_chipset = DSL_UNKNOWN;
static void dc_dp_grx518_free_dma(void);
#endif
...
static int eth_change_mtu_330(struct net_device *, int);
...
int __get_skb_from_dbg_pool(struct sk_buff *, const char *, unsigned int);
int __put_skb_to_dbg_pool(struct sk_buff *, const char *, unsigned int);
#else
static inline int __get_skb_from_dbg_pool(struct sk_buff *skb, const char *func_name, unsigned int line_num)
{
    return 0;
}
static inline int __put_skb_to_dbg_pool(struct sk_buff *skb, const char *func_name, unsigned int line_num)
{
    return 0;
}
#endif
...
#define get_skb_pointer_no_print(dataptr)    __get_skb_pointer(dataptr, __FUNCTION__, 0)
...
static uint32_t wlan_pltfrm_ppa_offload_get_wlanm_base(void);
static int gResetFirmware = 0;
#ifdef SUPPORT_UNLOAD_DTLK
void ppa_dp_dlrx_init(void);
void ppa_dp_dlrx_exit(void);
#endif //SUPPORT_UNLOAD_DTLK
...
static void ppa_route_stop_iteration(void);
...
#ifdef CONFIG_LTQ_DATAPATH
static int g_cpu_to_518_wan_tx_desc_pos = 0;
#endif
...
static inline int tx_queue_max(unsigned int port)
{
    return (g_wan_itf & (1 << port)) && g_wanqos_en ? __ETH_WAN_TX_QUEUE_NUM : __ETH_VIRTUAL_TX_QUEUE_NUM;
}

static struct pq_map g_prio_qid_map[2];

static inline struct pq_map *prio_qid_map_by_port(int port)
{
    switch (port) {
    case 0:
    case 1:
        return &g_prio_qid_map[port];
    default:
        break;
    }
...
static inline int prio_qid_map_get(int port, int priority)
{
    struct pq_map *pq_map = prio_qid_map_by_port(port);

    return pq_map_get_queue(pq_map, priority);
}

static inline int prio_qid_map_set(int port, int priority, int qid)
{
    struct pq_map *pq_map = prio_qid_map_by_port(port);

    return pq_map_set_queue(pq_map, priority, qid);
}

static inline void prio_qid_map_seq_show(int port, struct seq_file *seq)
{
    struct pq_map *pq_map = prio_qid_map_by_port(port);

    pq_map_seq_show(pq_map, seq);
}

static inline void prio_qid_map_init(int port)
{
    struct pq_map *pq_map = prio_qid_map_by_port(port);

    pq_map_init(pq_map, tx_queue_max(port));
    pq_map_map_linear(pq_map);
}

static inline void prio_qid_map_flush(int port)
{
    struct pq_map *pq_map = prio_qid_map_by_port(port);
...
static inline void prio_qid_map_cleanup(int port)
{
    struct pq_map *pq_map = prio_qid_map_by_port(port);

    pq_map_release(pq_map);
}
...
#if defined(DEBUG_SKB_SWAP) && DEBUG_SKB_SWAP
#define G_POOL_MAX 4096
int g_dbg_skb_swap_cnt = 0;
static struct sk_buff **g_dbg_skb_swap_pool = NULL;
#endif
...
#if (defined(ENABLE_DBG_PROC) && ENABLE_DBG_PROC) || (defined(DEBUG_DUMP_SKB) && DEBUG_DUMP_SKB)
...
static void eth_setup(struct net_device *dev)
{
    ...
    if (port == 0) {
        dev->needed_headroom = sizeof(struct sw_eg_pkt_header);
        dev->priv_flags |= IFF_LAN_DATAPATH;
    }

    if (port == 1)
        dev->priv_flags |= IFF_WANOE_DATAPATH;
}
...
#ifdef CONFIG_LANTIQ_ETH_FRAMEWORK
#define PPE_ETH_DATA_LEN   1570  /*1600 - 14( 12(Flag_header) + 2(space)) - 14  ( MAC header) - 2 Additional bytes*/
static int eth_change_mtu_330(struct net_device *dev, int new_mtu)
{
...
static int eth_hard_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
    ...
    qid = prio_qid_map_get(port, skb->priority);
...
static int eth_qos_hard_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	pr_err("ERROR: %s-%d: NULL SKB FOR [%x]\n", __func__, __LINE__, reg_desc.dataptr);
    ...
    case ETH_MAP_PKT_PRIO_TO_Q:
        {
            ...
            if (prio_qid_map_set(port, cmd.pkt_prio, cmd.qid))
                return -EINVAL;
        }
        break;
    default:
...
int eth_xmit(struct sk_buff *skb, unsigned int port, int ch, int spid, int class, int flags)
{
    ...
    pkth.spid           = spid;
    pkth.dpid           = port;
    pkth.lrn_dis        = (port == 7 && g_eth_wan_mode == 0) ? 1 : 0;
    ...
    if ( pkth.dpid < 2 )
        pkth.dpid_en    = g_egress_direct_forward_enable;
    else if ( pkth.dpid == 2 || (pkth.dpid == 7 && g_eth_wan_mode == 0))
        pkth.dpid_en    = 1;
...
#if defined(DEBUG_SKB_SWAP) && DEBUG_SKB_SWAP
int __get_skb_from_dbg_pool(struct sk_buff *skb, const char *func_name, unsigned int line_num)
{
    int i;

    for (i = 0; i < G_POOL_MAX && g_dbg_skb_swap_pool[i] != skb; i++);
    if (i == G_POOL_MAX || (unsigned int)skb < KSEG0) {
	if (line_num)
		err("%s:%d: TOTAL[%d] i(%d) skb (%x) < KSEG0(%x) not in g_dbg_skb_swap_pool",
				func_name, line_num, g_dbg_skb_swap_cnt, i, (unsigned int)skb, KSEG0);
    } else if (i < G_POOL_MAX) {
...
struct sk_buff *__get_skb_pointer(unsigned int dataptr, const char *func_name, unsigned int line_num)
{
    ...
    if (__get_skb_from_dbg_pool(skb, func_name, line_num) == -1) {
	if (line_num)
		err("%s:%d: dataptr[0x%08x]skb[0x%p]\n", func_name, line_num, dataptr, skb);
	skb = NULL;
    }
...
#if defined(DEBUG_SKB_SWAP) && DEBUG_SKB_SWAP
int __put_skb_to_dbg_pool(struct sk_buff *skb, const char *func_name, unsigned int line_num)
{
    int i;

    for ( i = 0; i < G_POOL_MAX && g_dbg_skb_swap_pool[i] != NULL; i++ );
    if (i == G_POOL_MAX) {
        err("%s:%d: TOTAL[%d] g_dbg_skb_swap_pool overrun",
		func_name, line_num, g_dbg_skb_swap_cnt);
    } else if (i < G_POOL_MAX) {
...
void dbg_pool_check(const char *str)
{
#if defined(DEBUG_SKB_SWAP) && DEBUG_SKB_SWAP
	dbg("%s: NUMBER OF SKB IN POOL: %d\n", str, g_dbg_skb_swap_cnt);
#endif
}
...
//    }
    ...
    if ( (mailbox_signal & CPU_TO_WAN_SWAP_SIG) )
    {
		...
		pr_err("ERROR: %s-%d: NULL SKB FOR [%x]\n",
			__func__, __LINE__, desc->dataptr);
...
static INLINE int dma_rx_int_handler(struct dma_device_info *dma_dev)
{
    ...
    if ( !skb )
        return 0;
    if (get_skb_from_dbg_pool(skb) == -1) {
	err("%s-%d: Invalid skb pointer[%p]\n", __func__, __LINE__, skb);
	return 0;
    }
    ...
    case 7:     //  ATM EoA or PTM
		...
		skb->cb[13] = 0x5A; //  magic number indicating directpath
		skb->cb[15] = header->qid & 0xf;
		#ifdef CONFIG_LTQ_DATAPATH
		off = 7; /* In DCDP, it only recognizes port 7 */
		/* ATM TC to get correct connection */
		skb->DW0 &= ~(0xf << 3);
		skb->DW0 |= (header->qid & 0xf) << 3;

		skb->DW1 &= ~(0xf << 8);
		skb->DW1 |= (off << 8 );
		//printk(" qid is %d  skb->DW0 is 0x%x \n",header->qid, skb->DW0);
		#endif
...
#ifdef SUPPORT_UNLOAD_DTLK
/*
* Function ppa_dp_dlrx_init, called by DirectLink driver to initilize RX
*/
void ppa_dp_dlrx_init(void)
{
    /*
       ...
       */
    unsigned int *ddr_base_reserved       = 0;
    dtlk_mem_base_get(NULL, (uint32_t *)&ddr_base_reserved);
    ddr_base_reserved = (unsigned int *)KSEG1ADDR(ddr_base_reserved);

    // reset reseved memory
    memset(ddr_base_reserved, 0, DTLK_TX_RSV_MEM_SIZE+DTLK_RX_RSV_MEM_SIZE);  // 8+4MB

    directlink_init();

}
EXPORT_SYMBOL(ppa_dp_dlrx_init);

/*
* Function ppa_dp_dlrx_exit, called by DirectLink driver to stop DirectLink RX
*/
void ppa_dp_dlrx_exit(void)
{
    enable_directlink_dma_rx(0);
}
EXPORT_SYMBOL(ppa_dp_dlrx_exit);
...
static INLINE void init_communication_data_structures(int fwcode)
{
    ...
    *CFG_QID2CLASS_MAP(1) = 0;
    ...
    for ( j = 0; j < 8; j++ )
    {
        *CFG_CLASS2QID_MAP(0) |= (7 - j) << (j * 4);
        *CFG_CLASS2QID_MAP(1) |= (15 - j) << (j * 4);
    }

    for ( j = 0; j < NUM_ENTITY(g_wan_queue_class_map); j++ )
        *CFG_QID2CLASS_MAP(0) |= g_wan_queue_class_map[j] << (j * 4);
...
#else
        tx_qos_cfg.time_tick    = clk_get_rate (g_ppe_clk) / 62500; //  16 * (lk_get_rate (clk)() / 1000000)
...
static INLINE int alloc_dma(void)
{
    ...
    for ( i = 0; i < pre_alloc_desc_total_num; i++ )
    {
        ...
        put_skb_to_dbg_pool(skb_pool[i]);
#ifndef CONFIG_MIPS_UNCACHED
        /*  invalidate cache    */
    ...
    if ( !g_dma_device )
    {
        pr_err("Reserve DMA for PPE failed\n");
        ret = -EIO;
...
void enable_vrx218_dma_tx(int enabled)
{
    if(enabled){//turn on vrx218 dma TX channel
		...
		//printk("%s: ===========TX CHAN 1 IS ON, DSL TYPE: %d===========\n", __func__, dsl_chipset_type_get());
		/*! to support 518 */
    #ifdef CONFIG_LTQ_DATAPATH
		if (dsl_chipset_type_get() == DSL_518) {
			//printk("%s: ===========SET ENABLE TX CHAN 1===========\n", __func__);
		    //g_dma_device->tx_chan[1]->desc_base = (int)GRX518_DMA_TX_CH1_DESC_BASE;
		    //g_dma_device->tx_chan[1]->desc_len  = __SOC_RX_IN_DES_LIST_NUM;
		} else /* only reset DMA1TX in 330-318 */
    #endif
		g_dma_device->tx_chan[1]->reset(g_dma_device->tx_chan[1]);
            g_dma_device->tx_chan[1]->open(g_dma_device->tx_chan[1]);
        } else {
		printk("%s: ===========TX CHAN 1 IS NOT ON===========\n", __func__);
        }
    }
    else{
         g_dma_device->tx_chan[1]->close(g_dma_device->tx_chan[1]);
...
void enable_vrx218_swap(int enabled, int is_atm, int is_bonding)
{
    ...
    if(enabled){
		#ifdef CONFIG_LTQ_DATAPATH
		g_cpu_to_518_wan_tx_desc_pos = 0;
		#endif
...
#ifdef CONFIG_LTQ_DATAPATH
#define ATM_QOS_MAPPING_TABLE_GET(addr, queue_id) (((*(u32 *)SB_BUFFER(addr)) >> (queue_id * 4)) & 0xf)
#define ATM_QOS_MAPPING_TABLE_SET(addr, queue_id, value) (*(u32 *)SB_BUFFER(addr) = ((u32)(value << (queue_id * 4)) | ((*(u32 *)SB_BUFFER(addr)) & ~(0xf << (queue_id * 4)))))
static ssize_t proc_write_atm_qos(struct file *file, const char __user  *buf, size_t count, loff_t *data)
{
    char local_buf[2048];
    int len;
    char *p1, *p2;
    ...
    while ( get_token(&p1, &p2, &len, &colon) )
    {
        ...
        {
            printk("echo dslqid xx queue xx > /proc/eth/atm_qos\n");
            break;
        }
        else if (stricmp(p1, "dslqid") == 0)
		{
			ignore_space(&p2, &len);
			dsl_qid = get_number(&p2, &len, 0);
			dbg("dsl_qid = %d", dsl_qid);
		}
		else if (stricmp(p1, "queue") == 0)
		{
			ignore_space(&p2, &len);
    ...
    if (dsl_qid != -1 && queue != -1) {
	if (dsl_qid > 7)
		ATM_QOS_MAPPING_TABLE_SET(__DSLQID2ETHQID_MAP1, (dsl_qid - 8), queue);
	else
		ATM_QOS_MAPPING_TABLE_SET(__DSLQID2ETHQID_MAP0, dsl_qid, queue);
    }
    return count;
}

static int proc_read_atm_qos(struct seq_file *seq, void *v)
{
	int i;
    seq_puts(seq, "ATM QOS Mapping Table - \n");
    seq_puts(seq, "DSL TX Queue - QoS Queue\n");
    for (i = 0; i < 16; i++) {
	if (i < 8)
		seq_printf(seq, "%d - %d\n", i, ATM_QOS_MAPPING_TABLE_GET(__DSLQID2ETHQID_MAP0, i));
	else
		seq_printf(seq, "%d - %d\n", i, ATM_QOS_MAPPING_TABLE_GET(__DSLQID2ETHQID_MAP1, (i - 8)));
    }
    return 0;
}
static int proc_read_atm_qos_seq_open(struct inode *inode, struct file *file)
{
    return single_open(file, proc_read_atm_qos, NULL);
}
static struct file_operations g_proc_file_atm_qos_seq_fops = {
    .owner      = THIS_MODULE,
    .open       = proc_read_atm_qos_seq_open,
    .read       = seq_read,
	.write      = proc_write_atm_qos,
    .llseek     = seq_lseek,
    .release    = single_release,
};
#endif
...
#ifdef CONFIG_LTQ_PPA_QOS
#ifndef CONFIG_330_QOS
    if(g_wanqos_en)
#endif
    {
	res  = proc_create("qos",S_IRUGO, proc_dir, &g_proc_file_qos_seq_fops);
#ifdef CONFIG_LTQ_DATAPATH
	res  = proc_create("atm_qos",S_IRUGO, proc_dir, &g_proc_file_atm_qos_seq_fops);
#endif
    }
#endif
#if defined(DEBUG_MIRROR_PROC) && DEBUG_MIRROR_PROC
...
static INLINE void proc_file_create(void)
{
    ...
    for(i = 0; ppa_proc_entry[i].name != NULL; i ++){
	   ...
	   0 &&
#endif
	   strcmp(ppa_proc_entry[i].name, "qos") == 0){
...
int32_t ppa_route_start_iteration(uint32_t *ppos, struct rout_forward_compare_tbl **pcompare, uint32_t type)
{
	struct rout_forward_compare_tbl *pcomp = NULL;
	uint32_t l;
...
static int proc_read_route_seq_show(struct seq_file *seq, void *v)
{
    ...
    struct rout_forward_compare_tbl *pcompare = NULL;
    struct rout_forward_action_tbl *pwaction = NULL;
...
static void fwdbg_read_ethbuf(char **tokens, int token_num)
{
    ...
    for(i = start_pg, num = 0 ; num < print_pg_num ; num ++, i = (i + 1) % pnum ) {
        ...
        if (cw == NULL || p_ctrl_dmrx == NULL || p_ctrl_dsrx == NULL)
            return;
...
static int proc_read_prio(struct seq_file *seq, void *v)
{
    ...
    seq_puts(seq, "Priority to Queue Map:\n");
    for (i = 0; i < 2; i++) {
        seq_printf(seq, "  eth%d:\n", i);
        prio_qid_map_seq_show(i, seq);
    }
    ...
    while ( get_token(&p1, &p2, &len, &colon) )
    {
        ...
        else if ( port != ~0 )
        {
            ...
            else if ( stricmp(p1, "q") == 0 || stricmp(p1, "queue") == 0 )
            {
                ...
                dbg("queue = %d", queue);
                if (port == 0 || port == 1) {
                    if (prio_qid_map_set(port, prio, queue))
                        err("failed to set ETH%d prio %d queue %d", port, prio, queue);
                }
            }
            else if (stricmp(p1, "flush") == 0)
            {
                if (port == 0 || port == 1)
                    prio_qid_map_flush(port);
                break;
            }
            else
            {
...
static ssize_t proc_write_ewan(struct file *file, const char __user *buf, size_t count, loff_t *data)
{
    ...
    while ( get_token(&p1, &p2, &len, &colon) )
    {
        if ( stricmp(p1, "help") == 0 )
        {
            printk("echo <null|p0|p1|p2|p3|p4|p5> > /proc/eth/ewan\n");
            break;
        }
        else if (stricmp(p1,"null") == 0)
        {
            ports = 0;
            got_ports++;
        }
...
#else
    seq_printf(seq, "  PPE clk=%u MHz, basic tick=%lu\n", (u32)(clk_get_rate(g_ppe_clk) / 1000000), (u32) TX_QOS_CFG->time_tick / (clk_get_rate(g_ppe_clk)  / 1000000));
...
static ssize_t proc_write_class(struct file *file, const char __user *buf, size_t count, loff_t *data)
{
    ...
    char *p1, *p2;
    int colon = 1;
    int i, j, tc, queue, tc2qid = 0, qid2tc = 0;
    ...
    while ( get_token(&p1, &p2, &len, &colon) )
    {
        ...
        else if (tc2qid || qid2tc)
        {
            if (stricmp(p1, "tc") == 0)
            {
                ignore_space(&p2, &len);
                tc = get_number(&p2, &len, 0) & 0xf;
                dbg("tc = %d", tc);
            }
            else if ( stricmp(p1, "queue") == 0 )
            {
                ignore_space(&p2, &len);
                queue = get_number(&p2, &len, 0) & 0x7;
                dbg("queue = %d", queue);
            }

            if (tc2qid) {
                *CFG_CLASS2QID_MAP(tc >> 3) &= ~(0xf << (tc * 4));
                *CFG_CLASS2QID_MAP(tc >> 3) |= queue << (tc * 4);
            }

            if (qid2tc) {
                g_wan_queue_class_map[queue] = tc;
                *CFG_QID2CLASS_MAP(tc >> 3) &= ~(0xf << (queue * 4));
                *CFG_QID2CLASS_MAP(tc >> 3) |= tc << (queue * 4);
            }
        }
        else
        {
            ...
            printk("echo <tc2qid | qid2tc> tc <TC> queue <queue> > /proc/eth/class\n");
            break;
        }
...
static INLINE void dump_skb(struct sk_buff *skb, u32 len, char *title, int port, int ch, int is_tx)
{
    ...
    {
        printk("too big data length: skb %08x, skb->data %08x, skb->len %u\n",
    ...
    if ( ch >= 0 )
        ...
        printk("%s\n", title);
    printk("  skb->data %08X, skb->tail %08X, skb->len %u, skb-priority %u (%x:%03x), skb->mark 0x%x, skb->tc_index 0x%x, headroom %u\n",
        (u32)skb->data, (u32)skb->tail, skb->len, skb->priority, (skb->priority >> 16) & 0xffff,
        skb->priority & 0xffff, skb->mark, skb->tc_index, skb_headroom(skb));
    for ( i = 1; i <= len; i++ )
    {
        if ( i % 16 == 1 )
            printk("  %4d:", i - 1);
...
static int get_netif_qid_with_pkt(struct sk_buff *skb, void *arg, int is_atm_vcc)
{
    ...
    if (port >= 0)
            return prio_qid_map_get(port, skb->priority);
...
{
    return prio_qid_map_get(port, priority);
}
...
int directlink_send(uint32_t *p_if_id, struct sk_buff *skb, int32_t len, uint32_t flags)
{
	...
	if((flags & FRAME_FROM_DL) && g_ppe_dtlk_data[dl_id].dp_if_id < 0){//directlink and not directpath
            ...
            if(eth_xmit(skb, 3 /* CPU or dummy */, g_directpath_dma_ch, 3 /* spid */, (int32_t)vap_id /* SW classification */, XMIT_DIRECTLINK_TO_WIFI) != PPA_SUCCESS){
...
#else
    TX_QOS_CFG->time_tick = clk_get_rate(g_ppe_clk) / 62500;
#endif
    return 0;
}
...
static uint32_t get_mailbox1_irq_num(void)
{
    ...
    dbg("CHIPID: %d, chipid address: 0x%x\n", chipid->version, (uint32_t)chipid);
    if(chipid->version > 1){
...
EXPORT_SYMBOL(ppa_dl_detect_11ac_card);

static void wlan_detect_ath_card()
{
	int index = 0;
	...
	for (index = 0; index < 2; index++) {
		if ((dev_tel = pci_get_subsys(PCI_VENDOR_ATHEROS, ath_dev_id,PCI_ANY_ID,PCI_ANY_ID, NULL)))
            found = 1;
		if (found)
			break;
		else {
	...
	if (!found)	{
		pr_err("Can not found any Atheros card\n");
		return ;
	}
	if (ath_dev_id == PCI_ATH_DEV_PEREGRINE) {
		pr_info("Found PEREGRINE_BOARD card\n");
		g11ACWirelessCardID = PEREGRINE_BOARD;
		*SB_BUFFER(__CFG_BOARD_TYPE) = PEREGRINE_BOARD;
	} else if (ath_dev_id == PCI_ATH_DEV_BEELINER) {
		pr_info("Found BEELINER_BOARD card\n");
		g11ACWirelessCardID = BEELINER_BOARD;
		//changed in firmware, using 4 instead of 1
		*SB_BUFFER(__CFG_BOARD_TYPE) = 4;//BEELINER_BOARD;
	}
}
#endif
#endif
...
/*static*/ int __init ppe_datapath_init(void)
{
...
#endif
#if defined(DEBUG_SKB_SWAP) && DEBUG_SKB_SWAP
    g_dbg_skb_swap_cnt = 0;
    if (g_dbg_skb_swap_pool == NULL)
	g_dbg_skb_swap_pool =
			(struct sk_buff **)kmalloc(G_POOL_MAX * sizeof(*g_dbg_skb_swap_pool), GFP_KERNEL);;
    memset(g_dbg_skb_swap_pool, 0, G_POOL_MAX * sizeof(*g_dbg_skb_swap_pool));
#endif
    pr_info("Loading D5 (MII0/1) driver ...... \n");

    g_ppe_clk = clk_get_ppe();

#if defined(CONFIG_ACCL_11AC) || defined(CONFIG_ACCL_11AC_MODULE)
    if (strlen(wlanm)>0)
    {
        dtlk_wlanm_setup(wlanm);
    }
    else
    {
        wlan_pltfrm_ppa_offload_get_wlanm_base();
    }
#endif
    ...
    *MBOX_IGU1_IER = g_mailbox_signal_mask;
    ...
    /* register PPA callback only for WANoE */
    if (g_eth_wan_mode == 3)
    ...
    pr_info("%s\n", buf);
#ifdef CONFIG_LTQ_DATAPATH
    //by default, supporting 318
    dsl_chipset_type_set(DSL_318);
    dp_init();
#endif
    return 0;
...
/*static*/ void ppe_datapath_exit(void)
{
   ...
   ppa_drv_datapath_mac_entry_setting = NULL;
    ...
    prio_qid_map_cleanup(0);
    prio_qid_map_cleanup(1);
...
#if defined(DEBUG_SKB_SWAP) && DEBUG_SKB_SWAP
    kfree(g_dbg_skb_swap_pool);
    g_dbg_skb_swap_pool = NULL;
#endif
}
...
{
	struct sk_buff *skb;
	skb = alloc_skb_tx(len);
	put_skb_to_dbg_pool(skb);
	return skb;
}
EXPORT_SYMBOL(dc_dp_alloc_dma);
void dc_dp_free_dma(unsigned int dataptr)
{
	struct sk_buff *skb =
			get_skb_pointer(dataptr);
	if (skb)
		dev_kfree_skb_any(skb);
}
EXPORT_SYMBOL(dc_dp_free_dma);

static void dc_dp_grx518_alloc_dma(void)
{
	/*! 518 uses firmware qos */
	struct tx_qos_cfg tx_qos_cfg = {0};
	struct cfg_std_data_len cfg_std_data_len = {0};
	int i;
	struct sk_buff **skb_pool;
    ...
    dbg("%s: %x g_wan_itf[%x]\n", __func__, LINUX_VERSION_CODE, g_wan_itf);
	g_wanqos_en = 8;
	g_wan_itf = 0;
	init_qos_queue();
	...
	*SB_BUFFER(__DSL_QOS_VALID) = 1;
	...
	num_alloc_pkts = (WAN_TX_DESC_NUM_TOTAL +
			CPU_TO_WAN_TX_DESC_NUM +
			CPU_TO_WAN_SWAP_DESC_NUM +
			DMA_TX_CH1_DESC_NUM);

	skb_pool = (struct sk_buff **)kmalloc(num_alloc_pkts * sizeof(*skb_pool), GFP_KERNEL);
	for ( i = 0; i < num_alloc_pkts; i++ )
	{
		skb_pool[i] = alloc_skb_rx();
		if ( !skb_pool[i] )
		{
	...
	pskb = skb_pool;
	p = (volatile u32 *)(CPU_TO_WAN_518_TX_DESC_BASE);
	for ( i = 0; i < CPU_TO_WAN_TX_DESC_NUM; i++ )
	{
		ASSERT(((u32)(*pskb)->data & (DMA_ALIGNMENT - 1)) == 0, "CPU to WAN TX data pointer (0x%#x) must be 8 DWORDS aligned", (u32)(*pskb)->data);
		//printk("%s: ALLOC p->dataptr[%x]skb_to_free[%x]\n", __func__, (*pskb)->data, *pskb);
		*p++ = 0x30000000;
		*p++ = (u32)(*pskb++)->data & 0x1FFFFFE0;
	}
	...
	g_dma_device->tx_chan[1]->desc_len  = __SOC_RX_IN_DES_LIST_NUM;
	g_dma_device->tx_chan[1]->reset(g_dma_device->tx_chan[1]);
	p = (volatile u32 *)GRX518_DMA_TX_CH1_DESC_BASE;
	for ( i = 0; i < __SOC_RX_IN_DES_LIST_NUM; i++ ) {
		*p++ = 0xF0000000 | DMA_PACKET_SIZE;  //  reserve descriptor (owned by VRX318), but no data pointer (which is maintained in VRX318)
		*p++;
	}
	dbg("%s: DMA_TX_CH1_DESC_BASE[%p]GRX518_DMA_TX_CH1_DESC_BASE[%p]\n", __func__, DMA_TX_CH1_DESC_BASE, GRX518_DMA_TX_CH1_DESC_BASE);
...
static void dc_dp_grx518_free_dma(void)
{
	...
	p = (volatile struct tx_descriptor *)(CPU_TO_WAN_518_TX_DESC_BASE);
	for ( i = 0; i < CPU_TO_WAN_TX_DESC_NUM; i++ )
	{
		skb_to_free = get_skb_pointer(p->dataptr);
		if (skb_to_free)
	...
	//clear owner bit from fast path
	p1 = (volatile u32 *)VRX218_SYN_DESC_BASE;
	for (i = 0; i < VRX218_SYN_DESC_NUM; i ++){
		*p1++ = 0x80000000 | DMA_PACKET_SIZE;
		p1++;
	}
	//clear QoS mib
#ifdef CONFIG_LTQ_PPA_QOS
...
void dsl_chipset_type_set(enum DSL_CHIPSET type)
{
	dbg("%s: SET DSL TYPE %d\n", __func__, type);
	g_current_dsl_chipset = type;
	if (type == DSL_518) {
		dbg("%s: SET 518\n", __func__);
		//g_cpu_to_518_wan_tx_desc_pos = 0;
		*SB_BUFFER(__IS_VRX518) = 1;
	}
}
int dsl_chipset_type_get()
{
	//printk("%s: return %d\n", __func__, g_current_dsl_chipset);
	return g_current_dsl_chipset;
}
int dc_dp_ppe_518_path_enable(int enable)
{
	//printk("%s: enable %d\n", __func__, enable);
	if (enable) {
		dc_dp_grx518_alloc_dma();
		*SB_BUFFER(__SOC_MODE_SWITCH) = 0;
	}
	else {
		*SB_BUFFER(__SOC_MODE_SWITCH) = 1;
		udelay(10000);
		dc_dp_grx518_free_dma();
	}
	return 0;
}
#define QID_FROM_SUBIF(subif) ((subif >> 3) & 0xf)
#define ATM_MPOA_TYPE_FROM_SUBIF(subif) ((subif) & 0x3)
#define ATM_MPOA_MODE_FROM_SUBIF(subif) ((subif >> 2) & 0x1)
int dc_dp_ppe_start_xmit(struct sk_buff *skb, dp_subif_t *dp_subif, struct net_device *dev, uint32_t flags)
{
    int port;
    unsigned int qid, mpoa_pt, mpoa_type;
    unsigned long sys_flag;
    volatile struct atm_tx_descriptor *desc;
    struct atm_tx_descriptor reg_desc;
    struct sk_buff *skb_to_free;
    int byteoff;
    int len;
    //struct sw_eg_pkt_header pkth = {0};
    ...
    if ( g_stop_datapath != 0 ) {
	pr_err("%s: port is stopped\n", __func__);
        goto ETH_XMIT_DROP;
    }

    port = dp_subif->port_id;

    //printk("%s: port[%d] flags[0x%x]\n", __func__, port, flags);
    if (flags & DP_TX_ATM || flags & DP_TX_OAM) {
        qid = QID_FROM_SUBIF(dp_subif->subif);
        mpoa_pt = ATM_MPOA_MODE_FROM_SUBIF(dp_subif->subif);
        mpoa_type = ATM_MPOA_TYPE_FROM_SUBIF(dp_subif->subif);

#if defined(ENABLE_DEBUG_PRINT) && ENABLE_DEBUG_PRINT
        if (g_dbg_enable & DBG_ENABLE_MASK_DUMP_SKB_TX)
            pr_info("%s: ATM port %d, flags 0x%x, qid %u, mpoa_pt %u, mpoa_type %u\n",
                    __func__, port, flags, qid, mpoa_pt, mpoa_type);
#endif
    } else {
        qid = QID_FROM_SUBIF(dp_subif->subif);
#if defined(ENABLE_DEBUG_PRINT) && ENABLE_DEBUG_PRINT
        if (g_dbg_enable & DBG_ENABLE_MASK_DUMP_SKB_TX)
            pr_info("%s: PTM port %d, flags 0x%x, qid %u\n", __func__, port, flags, qid);
#endif
    }
    ...
    dump_skb(skb, DUMP_SKB_LEN, "dc_dp_ppe_start_xmit", port, qid, 1);
	...
	if (1)
	{
		...
		skb = new_skb;
		byteoff = (u32)skb->data & (DMA_ALIGNMENT - 1);
		//err("%s: CREATE skb(0x%08x) skb->data[0x%08x]byteoff[0x%08x]\n", __func__, skb, skb->data, byteoff);
#ifndef CONFIG_MIPS_UNCACHED
		/*  write back to physical memory   */
		dma_cache_wback((u32)skb->data, skb->len);
#endif
	...
	{
		err("%s: REUSE skb(0x%p) skb->data[0x%p]byteoff[0x%08x]\n", __func__, skb, skb->data, byteoff);
		*(struct sk_buff **)((u32)skb->data - byteoff - sizeof(struct sk_buff *)) = skb;
#ifndef CONFIG_MIPS_UNCACHED
		/*  write back to physical memory   */
		dma_cache_wback((u32)skb->data - byteoff - sizeof(struct sk_buff *), skb->len + byteoff + sizeof(struct sk_buff *));
	...
	if (flags & DP_TX_BOND) {
		//printk("%s: BOND\n", __func__);
		return eth_xmit(skb, 7, 2, 2, qid, 0);  //  spid - CPU, class - 2 layer map, 1st: taken from prio_queue_map, 2nd: taken from wan_queue_class_map
	} else {
    /*  allocate descriptor */
    spin_lock_irqsave(&g_cpu_to_wan_tx_spinlock, sys_flag);
    desc = (volatile struct atm_tx_descriptor *)(CPU_TO_WAN_518_TX_DESC_BASE + g_cpu_to_518_wan_tx_desc_pos);
    if ( desc->own )    //  PPE hold
    {
        spin_unlock_irqrestore(&g_cpu_to_wan_tx_spinlock, sys_flag);
        err("PPE hold");
    ...
    if ( ++g_cpu_to_518_wan_tx_desc_pos == CPU_TO_WAN_TX_DESC_NUM )
	g_cpu_to_518_wan_tx_desc_pos = 0;
    spin_unlock_irqrestore(&g_cpu_to_wan_tx_spinlock, sys_flag);
    /*  load descriptor from memory */
    reg_desc = *desc;
    ...
    ASSERT((reg_desc.dataptr & (DMA_ALIGNMENT - 1)) == 0, "reg_desc.dataptr (0x%#x) must be 8 DWORDS aligned", reg_desc.dataptr);
    skb_to_free = get_skb_pointer(reg_desc.dataptr);
    //err("%s: reg_desc.dataptr[0x%08x]skb_to_free[0x%08x]\n", __func__, reg_desc.dataptr, skb_to_free);
    if (skb_to_free)
	dev_kfree_skb_any(skb_to_free);
    else
	pr_err("ERROR: %s-%d NO SKB FOR [%x]\n",
		__func__, __LINE__, reg_desc.dataptr);
    /*  detach from protocol    */
    skb_to_free = skb;
    skb = skb_break_away_from_protocol(skb);
    ...
    /*  update descriptor   */
    //reg_desc.dataptr    = (u32)skb->data & (0x1FFFFFFF ^ (DMA_ALIGNMENT - 1));  //  byte address
    reg_desc.dataptr    = (u32)skb->data & 0x1FFFFFFF;
    ASSERT((reg_desc.dataptr & (DMA_ALIGNMENT - 1)) == 0, "reg_desc.dataptr (0x%#x) must be 8 DWORDS aligned", reg_desc.dataptr);
    if (flags & DP_TX_OAM) {
	/* printk("%s: sending OAM\n", __func__); */
	reg_desc.pdu_type = 1;
    }
    ...
    reg_desc.qid        = qid;
	reg_desc.mpoa_pt	= mpoa_pt;
	reg_desc.mpoa_type	= mpoa_type;
    reg_desc.own        = 1;
    reg_desc.c          = 1;    //  ?
    reg_desc.sop = reg_desc.eop = 1;
    ...
    *((volatile u32 *)desc + 1) = *((u32 *)&reg_desc + 1);
    *(volatile u32 *)desc = *(u32 *)&reg_desc;
    //printk("%s: desc[%p]index[%d]\n", __func__, desc, g_cpu_to_518_wan_tx_desc_pos);
    return 0;
	}
NO_FREE_DESC:
ALLOC_SKB_TX_FAIL:
...
{
    ethwan = simple_strtoul(line, NULL, 0);

    if (ethwan < 0 || ethwan > 2)
        return -EINVAL;
...
static uint32_t wlan_pltfrm_ppa_offload_get_wlanm_base(void)
{
    ...
    //1M for 11N and 12M for DL
    iMem = (iMem + 13) * 1024 * 1024 ;
    dbg("%s: wlanm address is 0x%x\n",__func__,iMem);
    g_wlan_membase = iMem;
    return iMem;
}
#endif
...
uint32_t ppa_dl_qca_h2t_ring_init ( uint32_t h2tRingSize, uint32_t entrySize,uint32_t src_ring_base,uint32_t pcie_base, uint32_t flags )
{
    ...
    //only reset firmware at the first time. The firmware
    //pointer needs to sync with DMA pointer.
    if( gResetFirmware == 0 )
    {
	...
	for(i=0; i<NUM_TXPB ; i++) // 4096
	{
	  ...
	  *((uint32_t*)(bufptr + RESERVED_TXPB+PBPTR_TXPB+SIZE_TXDES-8)) = __bswap32(((uint32_t)bufptr)&PHYADDR_MASK);
	  ...
	  //BEELINERTODO
	  if(g11ACWirelessCardID == BEELINER_BOARD)
		{
		//printk("%s: PEE LINER \n",__func__);
	    *((uint32_t*)(bufptr)) = 0x0;
		}
	  else {// PEREGRINE_BOARD
			//printk("%s: PEREGRINE LINER \n",__func__);
	  ...
	  *((uint32_t*)(bufptr+12)) = 0x0;
	  ...
	  if(g11ACWirelessCardID == BEELINER_BOARD)
		{
		//printk("%s: PEE LINER 1\n",__func__);
	    *((uint32_t*)(bufptr+16)) = ((uint32_t)(bufptr+PBADDR_TXPB))&PHYADDR_MASK;
		}
	}
	//BEELINERTODO
	if(g11ACWirelessCardID == BEELINER_BOARD)
	{
	  *SB_BUFFER(__BEELINER_FRAG_DES_BASE) = (BADR_TXPB+2048*SIZE_TXPB)&PHYADDR_MASK;
	  bufptr = BADR_TXPB+2048*SIZE_TXPB;
	  memset(bufptr, 0, SIZE_BEELINER_FRAG_DESC*SIZE_BEELINER_MSDU_ID);
	  for(i=1; i<SIZE_BEELINER_MSDU_ID ; i++)
	  {
		bufptr = BADR_TXPB+2048*SIZE_TXPB+(i*SIZE_BEELINER_FRAG_DESC);
		*((uint32_t*)(bufptr+16)) = ((uint32_t)(BADR_TXPB+(i-1)*SIZE_TXPB+PBADDR_TXPB))&PHYADDR_MASK;
	  }
	}
#if 0
	*SB_BUFFER(__DIRLINK_ENABLE) = 0x1;
 ...
 */
int32_t ppa_dl_dre_txpkt_buf_release ( uint32_t num_msdus,uint32_t * msg, uint32_t flags )
{
    uint32_t bit01;
    int counter = 0;
	...
	{
	  pr_err("FATAL error:%s : %d BREAK wait IGU4, counter:%d\n",
