...
/*@}*/
  ...
  #define dbg(format, arg...)                   do { if ( ppa_hal_dbg_enable  ) pr_info("%d:%s: " format "\n", __LINE__, __FUNCTION__, ##arg); } while ( 0 )
#else
  #if !defined(dbg)
  ...
  #define ASSERT(cond, format, arg...)          do { if ( !(cond) ) pr_err("%d:%s: " format "\n", __LINE__, __FUNCTION__, ##arg); } while ( 0 )
#else
...
static void add_tunnel_6rd_header(struct net_device *dev, volatile struct iphdr *iph);
  ...
  #define PPE_MAX_ETH1_QOS_QUEUE 8
    ...
    static uint32_t set_qos_port_id(void)
    {
        ...
        else if ((wan_itf >> 6) & 0x3) qos_queue_portid = 7;
#endif
        else
        {
...
static const int dest_list_map[] = {0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x00};
...
#else
    llen = sprintf(pstr, "  PPE clk=%u MHz, basic tick=%u\n", (unsigned int)(clk_get_rate(g_ppe_clk) / 1000000), (unsigned int) get_basic_time_tick() );
#endif
    pstr += llen;
    len += llen;
#ifdef CONFIG_LTQ_PPA_QOS_WFQ
...
static void get_ppe_hal_id(uint32_t *p_family,
                    uint32_t *p_type,
                    uint32_t *p_if,
                    uint32_t *p_mode,
                    uint32_t *p_major,
 ...
 */
static uint32_t get_firmware_id(uint32_t pp32_index,
                     uint32_t *p_family,
                     uint32_t *p_type,
                     uint32_t *p_major,
                     uint32_t *p_mid,
...
static void get_phys_port_info(uint32_t port,
                        uint32_t *p_flags,
...
/*!
  ...
  \return no return value
 */
static INLINE void get_max_route_entries(uint32_t *p_entry,
                           uint32_t *p_mc_entry)
{
    if ( p_entry )
...
static INLINE void get_max_bridging_entries(uint32_t *p_entry)
{
    if ( p_entry )
...
/*!
  ...
  \return no return value
 */
static void set_wan_vlan_id(uint32_t vlan_id)
{
 ...
 */
static uint32_t get_wan_vlan_id(void)
{
...
//    bit 0: LAN
//    bit 1: WAN
static INLINE void set_if_type(uint32_t if_type,
                 uint32_t if_no)
{
 ...
 */
static INLINE uint32_t get_if_type(uint32_t if_no)
{
...
//    bit 5: f_drop_on_no_hit is valid
//    bit 6: f_mc_drop_on_no_hit is valid
static void set_route_cfg(uint32_t f_is_lan,
...
static void set_bridging_cfg(uint32_t entry_num,
...
/*!
  ...
  \return no return value
 */
static void set_fast_mode(uint32_t mode,
                   uint32_t flags)
{
...
/*!
  ...
  \return no return value
 */
static INLINE void set_if_wfq(uint32_t if_wfq,
                uint32_t if_no)
{
}
...
/*!
  ...
  \return no return value
 */
static INLINE void set_dplus_wfq(uint32_t wfq)
{
}
...
/*!
  ...
  \return no return value
 */
static INLINE void set_fastpath_wfq(uint32_t wfq)
{
}
...
/*!
  ...
  \return no return value
 */
static void get_acc_mode(uint32_t f_is_lan,
                  uint32_t *p_acc_mode)
{
    if ( p_acc_mode )
...
static void set_acc_mode(uint32_t f_is_lan,
                  uint32_t acc_mode)
{
    if ( f_is_lan )
...
/*!
  ...
  \return no return value
 */
static void set_mib_mode(uint8_t mib_mode)
{
...
/*!
  ...
  \return no return value
 */
static void get_mib_mode(uint8_t *mib_mode)
{
...
/*!
  ...
  \return no return value
 */
static void set_default_dest_list(uint32_t uc_dest_list,
                           uint32_t mc_dest_list,
                           uint32_t if_no)
{
}

static void set_bridge_if_vlan_config(uint32_t if_no,
...
static void get_bridge_if_vlan_config(uint32_t if_no,
                               uint32_t *f_eg_vlan_insert,
                               uint32_t *f_eg_vlan_remove,
                               uint32_t *f_ig_vlan_aware,
                               uint32_t *f_ig_src_ip_based,
...
static void del_vlan_map(uint32_t ig_criteria_type,
                  uint32_t ig_criteria)
{
}

static int32_t get_vlan_map(uint32_t ig_criteria_type,
                     uint32_t entry,
                     uint32_t *ig_criteria,
                     uint32_t *new_vci,
                     uint32_t *dest_qos,
...
static int32_t is_ipv6_enabled(void)
{
    return GEN_MODE_CFG1->ipv6_acc_en ? 1 : 0;
}

static int32_t add_ipv6_ip_entry(uint32_t ipv6_ip[4],
                          uint32_t *p_entry)
{
    int32_t ret = 0;
...
static void del_ipv6_ip_entry(uint32_t entry)
{
...
static INLINE int32_t get_ipv6_ip_entry(uint32_t entry,
                          uint32_t ipv6_ip[4])
{
    int x = entry / MAX_IPV6_IP_ENTRIES_PER_BLOCK;
    int i = entry % MAX_IPV6_IP_ENTRIES_PER_BLOCK;
 ...
 */
static int32_t add_routing_entry(uint32_t f_is_lan,
                          uint32_t src_ip,
                          uint32_t src_port,
                          uint32_t dst_ip,
                          uint32_t dst_port,
...
static int32_t add_ipv6_routing_entry(uint32_t f_is_lan,
                               uint32_t src_ip[4],
                               uint32_t src_port,
                               uint32_t dst_ip[4],
                               uint32_t dst_port,
...
/*!
  ...
  \return no return value
 */
static void del_routing_entry(uint32_t entry)
{
    PPE_LOCK *p_lock;
    volatile struct rout_forward_action_tbl *paction;
    volatile struct rout_forward_compare_tbl *pcompare;
...
/*!
  ...
  \return 0: OK, otherwise: fail
 */
static int32_t update_routing_entry(uint32_t entry,
                             uint32_t route_type,
                             uint32_t new_ip,
                             uint32_t new_port,
                             uint8_t  new_mac[PPA_ETH_ALEN],
                             ...
                             uint32_t flags)
{
    ...
    memset(&action, 0, sizeof(struct rout_forward_action_tbl));
...
/*!
  ...
  \return 0: OK, otherwise: fail
 */
static int32_t add_capwap_entry(uint8_t directpath_portid,
                        uint8_t f_ipv6,
                        uint8_t qid,
                        uint16_t dest_ifid,
                        uint8_t *dst_mac,
...
/*!
  ...
  \return no return value
 */
static void del_capwap_entry(uint32_t entry)
{
...
/*!
  ...
  \return no return value
 */
static void get_capwap_mib(uint32_t entry, uint32_t *ds_mib, uint32_t *us_mib)
{
    if ( entry < g_capwap_entries )
    {
        volatile uint32_t *p;
...
/*!
  ...
  \return 0: OK, otherwise: fail
 */
static int32_t add_wan_mc_entry(uint32_t dest_ip_compare,
                         uint32_t f_vlan_ins_enable,
                         uint32_t new_vci,
                         uint32_t f_vlan_rm_enable,
                         uint32_t f_src_mac_enable,
...
ADD_WAN_MC_ENTRY_GOON:
    ...
    if (ROUT_WAN_MC_CMP_TBL(entry) == NULL || ROUT_WAN_MC_ACT_TBL(entry) == NULL || ROUT_WAN_MC_HIT_STAT_TBL(entry >> 5) == NULL)
        return PPA_FAILURE;
...
/*!
  ...
  \return no return value
 */
static void del_wan_mc_entry(uint32_t entry)
{
    if ( entry < g_wan_mc_entries )
    {
        ...
        if (ROUT_WAN_MC_CNT(entry) == NULL || ROUT_WAN_MC_ACT_TBL(entry) == NULL || ROUT_WAN_MC_CMP_TBL(entry) == NULL)
            return;
...
/*!
  ...
  \return 0: OK, otherwise: fail
 */
static int32_t update_wan_mc_entry(uint32_t entry,
                            uint32_t f_vlan_ins_enable,
                            uint32_t new_vci,
                            uint32_t f_vlan_rm_enable,
                            uint32_t f_src_mac_enable,
...
static int32_t set_wan_mc_rtp(uint32_t entry,
                       uint8_t  sample_en)
{
...
static int32_t get_wan_mc_rtp_sampling_cnt(uint32_t entry,
                                    uint32_t *rtp_pkt_cnt,
                                    uint32_t *rtp_seq_num)
{
...
/*!
  ...
  \return 0: OK, otherwise: fail
 */
static INLINE int32_t get_dest_ip_from_wan_mc_entry(uint32_t entry,
                                      uint32_t *p_ip)
{
    if ( entry >= g_wan_mc_entries )
        return PPA_EINVAL;
...
static int32_t add_bridging_entry(uint32_t port,
...
static void del_bridging_entry(uint32_t entry)
{
    struct mac_tbl_item *p_item;
...
static uint32_t add_6rd_tunnel_entry(struct net_device *dev, uint32_t *tunnel_idx)
{
    uint32_t entry;
...
/*!
  ...
  \return no return value
 */
static void del_6rd_tunnel_entry(uint32_t entry)
{
    if ( entry >= MAX_6RD_TUNNEL_ENTRIES )
        return;
...
static uint32_t add_dslite_tunnel_entry(struct net_device *dev, uint32_t *tunnel_idx)
{
    uint32_t entry;
...
/*!
  ...
  \return no return value
 */
static void del_dslite_tunnel_entry(uint32_t entry)
{
    if ( entry >= MAX_DSLITE_TUNNEL_ENTRIES )
        return;
...
#if defined(L2TP_CONFIG) && L2TP_CONFIG
static void add_tunnel_l2tp_header(uint32_t entry, uint32_t tunnel_id, uint32_t session_id, uint32_t src_ip, uint32_t dst_ip, uint16_t ip_chksum, uint16_t udp_chksum)
{
...
/*
  ...
  \return 0: OK, otherwise: fail
 */
static int32_t add_l2tp_tunnel_entry(uint32_t tunnel_id,uint32_t session_id,uint32_t src_ip,uint32_t dst_ip,uint16_t ip_chksum,uint16_t udp_chksum,uint32_t *p_entry)
{
...
static void del_l2tp_tunnel_entry(uint32_t entry)
{
    volatile uint32_t *p;
...
/*
  ...
  \return 0: OK, otherwise: fail
 */
static int32_t add_pppoe_entry(uint32_t session_id,
                        uint32_t *p_entry)
{
...
/*!
  ...
  \return no return value
 */
static void del_pppoe_entry(uint32_t entry)
{
...
/*!
  ...
  \return 0: OK, otherwise: fail
 */
static int32_t get_pppoe_entry(uint32_t entry,
                        uint32_t *p_session_id)
{
    if ( entry >= MAX_PPPOE_ENTRIES )
        return PPA_EINVAL;
...
/*!
  ...
  \return 0: OK, otherwise: fail
 */
static int32_t add_mtu_entry(uint32_t mtu_size,
                      uint32_t *p_entry)
{
...
/*!
  ...
  \return no return value
 */
static void del_mtu_entry(uint32_t entry)
{
...
/*!
  ...
  \return 0: OK, otherwise: fail
 */
static int32_t get_mtu_entry(uint32_t entry,
                      uint32_t *p_mtu_size)
{
    if ( entry >= MAX_MTU_ENTRIES )
        return PPA_EINVAL;
 ...
 */
static uint32_t get_routing_entry_bytes(uint32_t entry, uint32_t reset_flag)
{
    PPE_LOCK *p_lock;
    volatile struct rout_forward_action_tbl *paction;
    uint16_t *p_occupation;
...
/*!
  ...
  \return byte counter value
 */
static uint32_t get_mc_entry_bytes(uint32_t entry, uint32_t reset_flag)
{
...
/*!
  ...
  \return 0: OK, otherwise: fail
 */
static int32_t add_mac_entry(uint8_t mac[PPA_ETH_ALEN],
                      uint32_t *p_entry)
{
...
/*!
  ...
  \return no return value
 */
static void del_mac_entry(uint32_t entry)
{
...
/*!
  ...
  \return 0: OK, otherwise: fail
 */
static int32_t get_mac_entry(uint32_t entry,
                      uint8_t mac[PPA_ETH_ALEN])
{
    if ( entry >= MAX_MAC_ENTRIES )
        return PPA_EINVAL;
...
/*!
  ...
  \return 0: OK, otherwise: fail
 */
static int32_t add_outer_vlan_entry(uint32_t new_tag,
                             uint32_t *p_entry)
{
...
/*!
  ...
  \return no return value
 */
static void del_outer_vlan_entry(uint32_t entry)
{
...
/*!
  ...
  \return 0: OK, otherwise: fail
 */
static int32_t get_outer_vlan_entry(uint32_t entry,
                             uint32_t *p_outer_vlan_tag)
{
    if ( entry >= MAX_OUTER_VLAN_ENTRIES )
        return PPA_EINVAL;
...
static int32_t ifx_ppe_drv_del_multifield_entry_via_index(int32_t index, uint32_t flag);
...
*/
static uint8_t ifx_ppe_drv_multifield_control(uint8_t enable, uint32_t flag)
{
...
/*!
    ...
    \return uint8_t, The return value can be any one of the following:  \n
               ...
               PPA_FAILURE on error
*/
static uint8_t ifx_ppe_drv_get_multifield_status(uint32_t *enable, uint32_t flag)
{
...
*/
static int32_t ifx_ppe_drv_get_multifield_max_entry(uint32_t flag)
{
    if( flag == SESSION_ADDED_IN_HW ) //get accelerated mf flow entries
    {
        ...
        pr_warn("get mf flow for SESSION_ADDED_IN_HW only\n");
        return num;
    }
...
*/
static int32_t ifx_ppe_drv_add_multifield_entry(PPA_MULTIFIELD_FLOW_INFO *p_multifield_info, int32_t *index, uint32_t flag)
{
    int ret=PPA_FAILURE;
    int32_t entry, empty_entry = -1;
...
/*!
    ...
    \return int32_t, return the bytes in the compare table. It can be any one of the following now: \n
               ...
               PPA_FAILURE on error
*/
static int32_t ifx_ppe_drv_get_multifield_entry(int32_t index, PPA_MULTIFIELD_FLOW_INFO *p_multifield_info, uint32_t flag)
{
    int ret=PPA_SUCCESS;
    uint32_t compare[4] = {0};
    uint32_t mask[4] = {0};
...
*/
static int32_t ifx_ppe_drv_del_multifield_entry(PPA_MULTIFIELD_FLOW_INFO *p_multifield_info, uint32_t flag)
{
    int ret=PPA_FAILURE;
    int32_t entry;
    uint32_t compare[4] = {0};
...
static void get_itf_mib(uint32_t itf, struct ppe_itf_mib *p)
{
    if ( p != NULL && itf < 8 )
...
/*!
  ...
  \return 0: OK, otherwise: fail
 */
static uint32_t test_and_clear_hit_stat(uint32_t entry)
{
...
/*!
  ...
  \return 0: OK, otherwise: fail
 */
static INLINE uint32_t test_and_clear_hit_stat_batch(uint32_t entry)
{
    if ( (entry & 0x7FFFFFFF) < MAX_ROUTING_ENTRIES )
    {
        uint32_t ret;
...
static uint32_t test_and_clear_mc_hit_stat(uint32_t entry)
{
...
static INLINE uint32_t test_and_clear_mc_hit_stat_batch(uint32_t entry)
{
    if ( entry < MAX_WAN_MC_ENTRIES )
    {
...
static uint32_t test_and_clear_bridging_hit_stat(uint32_t entry)
{
    return 0;
}

static INLINE uint32_t test_and_clear_bridging_hit_stat_batch(uint32_t entry)
{
    return 0;
}

static int32_t get_max_vfilter_entries(uint32_t vfilter_type)
{
    return 0;
}
...
static const struct bitrate_2_t_kbps bitrate_2_t_kbps_table[]={
    {  100000,   1},
    {   10000,  10},
    {    1000, 100},
    {       0, 250}
...
#else
    return TX_QOS_CFG->time_tick / (clk_get_rate(g_ppe_clk) /1000000);
#endif
}
...
#else
    stat->pp32_clk = clk_get_rate(g_ppe_clk);
...
#endif
{
    ...
    if(shaper_id == -1){
        ...
        if ( queueid < TX_QOS_CFG->eth1_eg_qnum ){
            if (WTX_EG_Q_SHAPER_MAPPING_CFG(queueid) == NULL)
                return PPA_FAILURE;
    ...
    else
    {
        ...
        if ( queueid < TX_QOS_CFG->eth1_eg_qnum ){
            *__QOS_NEW_FEATURE = 1;
#if 0
            if (WTX_EG_Q_SHAPER_ID(queueid) == NULL)
                return PPA_FAILURE;
#endif
            ...
            if(cur_shaperid < 0){//current shaper is default shaper
                *WTX_EG_Q_DEF_SHAPER_QUEUE_MAP &= ~(1 << queueid);
                ...
                if (WTX_EG_Q_DEF_SHAPER_QUEUE_MAP == NULL)
                    return PPA_FAILURE;
...
/*!
    ...
    \return The return value can be any one of the following:  \n
               ...
               - IFX_FAILURE on error \n
*/
static int32_t get_qos_shaper( int32_t shaperid, uint32_t *rate, uint32_t *burst, uint32_t flag)
{
    struct wtx_eg_q_shaping_cfg qos_cfg = {0};
