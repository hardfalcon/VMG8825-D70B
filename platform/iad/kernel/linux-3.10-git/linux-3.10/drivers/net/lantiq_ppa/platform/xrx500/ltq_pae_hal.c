...
#if defined(CONFIG_PAE_HW_METER) || defined(CONFIG_PAE_HW_METER_MODULE)
#define CONFIG_HW_METER
#endif
...
static int proc_read_pae_meter(struct seq_file *seq, void *v)
{
	...
	for_each_set_bit(tx_id, &txid_bitmap, PPA_MAX_IFS_NUM)
		seq_printf(seq,	"%16s:%-8llu", tx_meter[tx_id].ifname,
						tx_meter[tx_id].cnt);
	seq_printf(seq, "\n");
	for_each_set_bit(rx_id, &rxid_bitmap, PPA_MAX_IFS_NUM) {
		seq_printf(seq, "%16s:%-8llu", rx_meter[rx_id].ifname,
						rx_meter[rx_id].cnt);
		for_each_set_bit(tx_id, &txid_bitmap, PPA_MAX_IFS_NUM)
			seq_printf(seq,	"%12llu:%-12u",
                        ...
                        PPA_IFNAME ifname[PPA_IF_NAME_SIZE])
{
  ...
  if(p_flags)
    ...
    if ( ifname )
        snprintf(ifname, sizeof(ifname), "%s", str_ifname[port]);
}
...
uint8_t allocate_meter(uint8_t rx_id, uint8_t tx_id)
{
	...
	param.nMeterId = find_first_zero_bit(&meter_bitmap, 64);
	param.bEnable = true;

	param.eMtrType =  GSW_QOS_Meter_srTCM;
	/* Committed Burst Size (CBS [Bytes]). */
	param.nCbs = 0xFFFFFFFF;
	/* Excess Burst Size (EBS [Bytes]). */
	param.nEbs = 0xFFFFFFFF;
	/* Committed Information Rate (CIR [kbit/s]) */
	param.nRate = 0xF4240; /* 1GBps*/
 ...
 */
int32_t get_session_nMeterId(PPE_ROUTING_INFO *route,
			     GSW_ROUTE_Entry_t *rt_entry)
{
	...
	if (!pae_hal_hw_meter_enable)
		return PPA_SUCCESS;
	if (!route->is_rx_logical && !route->is_tx_logical)
		return PPA_SUCCESS;

	if (route->is_rx_logical)
		rx_id = get_iface_idx(route->rxifname, 0);
	if (route->is_tx_logical)
		tx_id = get_iface_idx(route->txifname, 1);
 ...
 */
int32_t put_session_nMeterId(PPE_ROUTING_INFO *route)
{
	uint8_t rx_id = 0, tx_id = 0;
	...
	if (route->is_rx_logical) {
		meter_rx = rx_meter;
		for_each_set_bit(rx_id, &rxid_bitmap, PPA_MAX_IFS_NUM)
			if(strncmp(meter_rx[rx_id].ifname, route->rxifname,
				   PPA_IF_NAME_SIZE) == 0)
				break;
	}
	...
	if (route->is_tx_logical) {
		meter_tx = tx_meter;
		for_each_set_bit(tx_id, &txid_bitmap, PPA_MAX_IFS_NUM)
			if(strncmp(meter_tx[tx_id].ifname, route->txifname,
				   PPA_IF_NAME_SIZE) == 0)
				break;
	}
...
/*This API programs the PCE Rule to look-up only  Mutlicast group address and not the source IP */
int32_t add_pce_rule_mc_anysrcip(PPE_MC_INFO *mc_route)
{
#if defined(PPA_CLASSIFICATION) && PPA_CLASSIFICATION
    PPA_CLASS_RULE rule;
#else
    ...
    rule.pattern.eDstIP_Select = 1;
    rule.pattern.nDstIP.nIPv4 = mc_route->dest_ip_compare;
    rule.pattern.nDstIP_Mask= 0xFFFFFF00;   // 8 bit mask for ipv4 each bit used to mask a nibble

    rule.action.fwd_action.rtdestportmaskcmp = 0;
    rule.action.fwd_action.rtsrcportmaskcmp = 0;
    rule.action.fwd_action.rtdstipmaskcmp = 1;
    rule.action.fwd_action.rtsrcipmaskcmp = 0;
    ...
    if(pae_hal_add_class_rule(&rule)!=PPA_SUCCESS) {
	dbg( "add pce rule returned failure\n");
	return PPA_FAILURE;
    }
    return PPA_SUCCESS;
}
...
int32_t add_wan_mc_entry(PPE_MC_INFO *mc_route)
{
	...
	if(rt_entry.routeEntry.pattern.nSrcIP.nIPv4 == 0xFFFFFFFF) {
		rt_entry.routeEntry.pattern.nSrcIP.nIPv4 = 0x0;
		if(add_pce_rule_mc_anysrcip(mc_route) != PPA_SUCCESS)
			return PPA_FAILURE;
	}
...
int32_t pae_hal_del_class_rule(PPA_CLASS_RULE* rule)
{
    ...
    if(open_switch_dev(rule->in_dev,&gsw_handle)!=PPA_SUCCESS || rule->subcategory >= SUBCAT_MAX)
	return PPA_FAILURE;
