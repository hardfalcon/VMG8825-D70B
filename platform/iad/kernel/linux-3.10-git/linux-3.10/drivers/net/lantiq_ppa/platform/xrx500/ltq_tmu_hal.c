...
int tmu_hal_update_queue(struct net_device *netdev, char *dev_name, uint32_t q_id, uint32_t priority, uint32_t weight, uint32_t flags);
    ...
    case PPA_GENERIC_HAL_QOS_ADDQUE_CFG:
	{
		...
		if((cfg->flags & PPA_QOS_Q_F_WFQ_UPDATE) == PPA_QOS_Q_F_WFQ_UPDATE)
		{
			tmp_qid = tmu_hal_update_queue(
					...
					cfg->weight,
					cfg->flags);
			TMU_HAL_DEBUG_MSG(TMU_DEBUG_TRACE," <%s>: q_id returned by tmu_hal_update_queue for WFQ scenario is: %d\n",__FUNCTION__, tmp_qid);
			q_param.index = tmp_qid;
			q_param.weight = cfg->weight;
			TMU_HAL_DEBUG_MSG(TMU_DEBUG_TRACE," <%s>: Weight= %d : qid = %d : for WFQ scenario \n",__FUNCTION__, q_param.weight, q_param.index);
...
int32_t tmu_hal_set_mpe_ingress_grp_qmap(struct tmu_hal_user_subif_abstract *subif_index, uint32_t prio_level, uint32_t flags, uint32_t q_index, char *tc , uint8_t no_of_tc)
{
	...
	{ //default Q of Physical interface
		...
		for(x=1; x<subif_index->queue_cnt && subif_index->queue_cnt<=TMU_HAL_MAX_QUEUE_PER_EGRESS_INTERFACE; x++)
		{
			q_map_mask = 0;
		...
		uint32_t flags)
{
	...
	if((flags & PPA_QOS_Q_F_INGRESS) == PPA_QOS_Q_F_INGRESS)
	{
		...
		if(!g_IngressQueueScheduler) {
			...
			if(tmu_hal_get_tmu_res_from_netdevice(netdev, netdev->name, schedid, &dp_subif, &nof_of_cpu_tmu_ports, &cpu_tmu_res, (flags | PPA_QOS_Q_F_INGRESS)) == TMU_HAL_STATUS_ERR) {
				TMU_HAL_DEBUG_MSG( TMU_DEBUG_TRACE,"%s:%d Error: Failed to get resources from Netdevice\n", __FUNCTION__, __LINE__);
				kfree(cpu_tmu_res);
				return TMU_HAL_STATUS_ERR;
			}
...
CBM_RESOURCES_CLEANUP:
	kfree(tmu_res);
	TMU_HAL_DEBUG_MSG( TMU_DEBUG_TRACE,"<%s> <-- Exit --> \n",__FUNCTION__);
	return subif_index->user_queue[index].queue_index;
