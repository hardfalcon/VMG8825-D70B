 ...
 * ####################################
 */
PPA_FILTER_STATS ppa_filter;
PPA_PORT_QUEUE_NUM_STATUS ppa_port_qnum[PPA_MAX_PORT_NUM];
...
static void ppa_exit_new(void)
{
	...
	pr_info("Acceleration module exited!!!\n");
}
...
int32_t ppa_init(PPA_INIT_INFO *p_info, uint32_t flags)
{
	...
	if ( (ret = ppa_init_new(p_info, flags)) == PPA_SUCCESS )
		pr_info("ppa_init - init succeeded\n");
	else
		pr_err("ppa_init - init failed (%d)\n", ret);
...
int32_t ppa_bridge_entry_get(uint8_t *mac_addr, PPA_NETIF *brif, PPA_CMD_MAC_ENTRY *mac_entry)
{
	struct bridging_session_list_item *p_item;
	int32_t ret = PPA_FAILURE;
	uint16_t fid=0;
	...
	ppa_br_get_htable_lock();
	if ( __ppa_bridging_lookup_session(mac_addr, fid, NULL, &p_item) != PPA_SESSION_EXISTS ){
		goto __BR_SESSION_GET_DONE;
	}

	dump_bridging_list_item(p_item, "ppa_bridge_entry_get");

	ppa_strncpy(mac_entry->ifname, ppa_get_netif_name(p_item->netif), sizeof(mac_entry->ifname));
	mac_entry->flags = p_item->flags;
	ret = PPA_SUCCESS;

__BR_SESSION_GET_DONE:
	ppa_br_release_htable_lock();
