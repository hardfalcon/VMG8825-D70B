		...
		)
{
	...
	return ret;
reg_err1:
	if (ppa_directpath_register_dev_ex((PPA_SUBIF *)&subIf,
				dtlk->dev,
				pDirectpathCb,
...
int32_t ppa_directlink_register_dev(int32_t *p_if_id, PPA_DTLK_T *dtlk, PPA_DIRECTPATH_CB *pDirectpathCb,uint32_t flags)
{
	...
	if(dtlk->flags & PPE_F_DTLK_REGISTER){//register directlink
		pr_debug("%s: register directlink\n", __func__);
	...
	if(dtlk->flags & PPE_F_DTLK_DP_REGISTER){//register to directpath
		if((ret=ppa_directpath_register_dev(&if_id,dtlk->dev,pDirectpathCb,flags)) == PPA_SUCCESS){
			pr_debug("%s: register directpath\n", __func__);
		...
		pr_debug("%s: de-register directpath\n", __func__);
	...
	if(dtlk->flags & PPE_F_DTLK_DEREGISTER){//deregister from directlink
		pr_debug("%s: de-register directlink\n", __func__);
...
{
	int32_t i=0, temp = -1;
	pr_debug("%s: get subif for port id:%d\n", __func__,portid);
	for(i=0; i<5 ;i++)
	{
		if(ifid_subif_map[i].port_id == portid) {
			...
			pr_debug("%s: last subif for port id:%d is %d\n", __func__,portid, temp);
...
void dump_ifid_subif_map_tbl(void)
{
	...
	for(i=0; i<5 ;i++)
	{
		pr_debug("<%s> i=%d port:%d subif:%d flags: %x\n",__func__,i,ifid_subif_map[i].port_id,ifid_subif_map[i].subif, ifid_subif_map[i].flags);
	}
}
#endif
...
int32_t ppa_directpath_send_ex(PPA_SUBIF *subif, PPA_SKBUF *skb, int32_t len, uint32_t flags)
{
	...
	if(flags & (PPA_F_DIRECTLINK_XMIT_TO_WIFI | PPA_F_DIRECTLINK_XMIT_FROM_WIFI)) { //directlink function call
		...
		if (!skb2) {
			pr_err("<%s>: cannot allocate bytes\n", __func__ );
			ret = PPA_ENOMEM;
			goto __DIRETPATH_TX_EXIT;
		}
		/* Copy */
...
int32_t ppa_directpath_recycle_skb(PPA_SUBIF *subif, PPA_SKBUF *skb, uint32_t flags)
{
	...
	ppa_lock_release(&g_directpath_port_lock);
#else
	pr_debug("ppa_directpath_recycle_skb not defined!!\n");
#endif
...
int32_t ppa_api_directpath_init(void)
{
	...
	pr_debug("directpath start id: %d, end id: %d\n", g_start_ifid, g_end_ifid);
