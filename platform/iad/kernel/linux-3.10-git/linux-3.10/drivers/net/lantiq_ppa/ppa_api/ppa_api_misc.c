...
#endif
{
	...
	{
		pr_err("ppa_ioc_type(%08X - %d) != PPA_IOC_MAGIC(%d)\n", cmd, _IOC_TYPE(cmd), PPA_IOC_MAGIC);
		goto EXIT_EIO;
	}
	else if( ppa_ioc_nr(cmd) >= PPA_IOC_MAXNR )
	{
		pr_err("Current cmd is %02x wrong, it should less than %02x\n", _IOC_NR(cmd), PPA_IOC_MAXNR );
		goto EXIT_EIO;
	}
	...
	{
		pr_err("access check: (%08X && %d) || (%08X && %d)\n",
		...
		case PPA_CMD_GET_MAC_ENTRY:
			{
				...
				if ( ppa_bridge_entry_get(cmd_info->mac_entry.mac_addr, brif, &cmd_info->mac_entry) != PPA_SUCCESS )
					goto EXIT_EIO;
				else if ( ppa_copy_to_user((void *)arg, &cmd_info->mac_entry, sizeof(cmd_info->mac_entry)) != 0 )
					goto EXIT_EFAULT;
				else
					goto EXIT_ZERO;
			}
					...
					for(j=0; j<vfilter_count.num; j++ )
					{
						...
						if( ppa_drv_get_vlan_map(&filter_map, 0 ) == 1 )
						{
							...
							if( vlan_filter_type[i] == PPA_F_VLAN_FILTER_IFNAME )
							{
								...
								{
									pr_err("Why ig_criteria (%d) big than %d for PPA_CMD_GET_ALL_VLAN_FILTER_CFG\n", filter_map.ig_criteria, PPA_MAX_IFS_NUM );
									continue;
								}
								if( filter_map.ig_criteria < PPA_MAX_IFS_NUM )
		...
		case PPA_CMD_ADD_MC:
			{
				...
				if ( cmd_info->mc_add_info.num_ifs > PPA_MAX_MC_IFS_NUM )
				{
					...
					pr_err("Error cmd_info->mc_add_info.num_ifs=%d is over max limit %d", cmd_info->mc_add_info.num_ifs, PPA_MAX_MC_IFS_NUM);
					goto EXIT_EIO;
				}
		...
		case PPA_CMD_ADD_CAPWAP:
			{
				...
				{
					pr_err("Error cmd_info->capwap_add_info.num_ifs=%d is over max limit %d", cmd_info->capwap_add_info.num_ifs, PPA_MAX_CAPWAP_IFS_NUM);
					goto EXIT_EIO;
				}
...
#endif
		default:
			pr_err("wrong cmd:0x%0x(its nr value=0x%0x)\n", cmd, ppa_ioc_nr(cmd) );
...
int32_t ppa_ioctl_add_session(unsigned int cmd, unsigned long arg, PPA_CMD_DATA *cmd_info)
{
	pr_err("This ioctl is no more supported. No Session will be added/deleted from user space\n");
	return PPA_FAILURE;
}
...
int __init ppa_driver_init(void)
{
	...
	if ( ret != 0 )
		pr_err("PPA API --- failed in register_chrdev(%d, %s, g_ppa_dev_ops), errno = %d\n", PPA_CHR_DEV_MAJOR, PPA_DEV_NAME, ret);

#if defined(CONFIG_LTQ_PPA_API_PROC) || defined(CONFIG_LTQ_PPA_API_PROC_MODULE)
	proc_file_create();
#endif
	pr_info("PPA API --- init successfully\n");
	return 0;
