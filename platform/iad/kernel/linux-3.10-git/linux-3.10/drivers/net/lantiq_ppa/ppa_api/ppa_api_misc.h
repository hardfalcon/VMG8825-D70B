...
#undef  ASSERT
#define ASSERT(cond, format, arg...)          do { if ( (g_ppa_dbg_enable & DBG_ENABLE_MASK_ASSERT) && !(cond) ) pr_err("%d:%s: " format "\n", __LINE__, __FUNCTION__, ##arg); } while ( 0 )
#else
...
#undef  err
#define err(format, arg...)                   do { if ( (g_ppa_dbg_enable & DBG_ENABLE_MASK_ERR) ) pr_err("%d:%s: " format "\n", __LINE__, __FUNCTION__, ##arg); } while ( 0 )
#else
...
#if defined(ENABLE_ERROR) && ENABLE_ERROR
#undef  critial_err
#define critial_err(format, arg...)           do { pr_emerg("%d:%s: " format "\n", __LINE__, __FUNCTION__, ##arg); } while ( 0 )
#else
...
#define ppa_debug(flag, fmt, arg...)  do { if ( (g_ppa_dbg_enable & flag) && max_print_num ) { pr_err(fmt, ##arg); if( max_print_num != ~0) max_print_num--; } } while ( 0 )
#else
#undef ppa_debug
