				    ...
				    struct phys_port_info **pp_info)
{
	...
	phys_port_name = ifx_eth_fw_netdev_physname(netif);

	ppa_debug(DBG_ENABLE_MASK_DEBUG_PRINT, "netif->name %s, phys_port_name %s",
		netif->name, phys_port_name);

	ppa_lock_get(&g_phys_port_lock);
	for (obj = g_phys_port_info; obj; obj = obj->next) {
		if (strcmp(obj->ifname, phys_port_name) == 0) {
			ret = PPA_SUCCESS;
			if (pp_info)
				*pp_info = obj;

			ppa_debug(DBG_ENABLE_MASK_DEBUG_PRINT, "found phys_port %s, port %d, type %d\n",
				obj->ifname, obj->port, obj->type);
			break;
		}
	}
	ppa_lock_release(&g_phys_port_lock);
...
static void ppa_register_new_netif(PPA_NETIF *netif)
{
	...
	if ((ppa_get_physical_if(netif, NULL, phy_ifname) == PPA_SUCCESS) &&
		...
		if ((ppa_get_netif_info(phy_ifname, &p_ifinfo) == PPA_SUCCESS)
			...
			err = ppa_netif_add(ifname, f_is_lan, NULL, NULL, force_wanitf);
			ppa_debug(DBG_ENABLE_MASK_DEBUG_PRINT,
				  "dynamically adding iface[%s], f_is_lan %d, force_wanitf %d\n",
				  ifname, f_is_lan, force_wanitf);
		} else if (netif->priv_flags & IFF_LAN_DATAPATH) {
			err = ppa_netif_add(ifname, 1, NULL, NULL, 0);
			ppa_debug(DBG_ENABLE_MASK_DEBUG_PRINT,
				  "dynamically adding LAN iface[%s]\n",
				  ifname);
		} else if (netif->priv_flags & IFF_PTM_DATAPATH) {
			err = ppa_netif_add(ifname, 0, NULL, NULL, 0);
			ppa_debug(DBG_ENABLE_MASK_DEBUG_PRINT,
				  "dynamically adding PTM WAN iface[%s]\n",
				  ifname);
		} else if (netif->priv_flags & IFF_WANOE_DATAPATH) {
			err = ppa_netif_add(ifname, 0, NULL, NULL, 0);
			ppa_debug(DBG_ENABLE_MASK_DEBUG_PRINT,
				  "dynamically adding ETH WAN iface[%s]\n",
				  ifname);
		} else if (ppa_if_is_br_if(netif, NULL)) {
			err = ppa_netif_add(ifname, NETIF_LAN_IF, NULL, NULL, 0);
			ppa_debug(DBG_ENABLE_MASK_DEBUG_PRINT,
				  "dynamically adding bridge iface[%s]\n",
				  ifname);
		} else if (ppa_if_is_br2684(netif, NULL)) {
			err = ppa_netif_add(ifname, f_is_lan, NULL, NULL, 0);
			ppa_debug(DBG_ENABLE_MASK_DEBUG_PRINT,
...
#else
	if (f_is_lan) {
		mcast_helper_register_module(p_ifinfo->netif, THIS_MODULE, NULL, (int32_t *)mcast_module_config_puma, MC_F_REGISTER | MC_F_DIRECTPATH);
	}
#endif
#endif
#if defined(CONFIG_PPA_PUMA7) && defined(CONFIG_TI_HIL_PROFILE_INTRUSIVE_P7)
...
#else
	if (f_is_lan) {
		mcast_helper_register_module(p_ifinfo->netif, THIS_MODULE, NULL, (int32_t *)mcast_module_config_puma, MC_F_DEREGISTER | MC_F_DIRECTPATH);
	}
#endif
#endif
		ppa_netif_put(p_ifinfo);
