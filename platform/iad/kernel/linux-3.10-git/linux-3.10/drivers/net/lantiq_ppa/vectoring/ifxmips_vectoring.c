...
#define err(format, arg...)                     do { pr_err("%d:%s: " format "\n", __LINE__, __FUNCTION__, ##arg); } while ( 0 )
#define dbg(format, arg...)                     do { if ( (g_dbg_enable & DBG_ENABLE_MASK_DEBUG_PRINT) ) pr_warning("%d:%s: " format "\n", __LINE__, __FUNCTION__, ##arg); } while ( 0 )
...
int32_t mei_dsm_cb_func(uint32_t *p_error_vector);

static int register_netdev_event_handler(void);
...
void ltq_vectoring_priority(uint32_t priority);
...
int mei_dsm_cb_func(unsigned int *p_error_vector)
{
    int rc, ret;
...
EXPORT_SYMBOL(mei_dsm_cb_func);

void ltq_vectoring_priority(uint32_t priority)
{
	vector_prio = priority;
	pr_info("Vectoring: New priority[%d]\n",vector_prio);
}
EXPORT_SYMBOL(ltq_vectoring_priority);

static int register_netdev_event_handler(void)
{
    g_netdev_event_handler_nb.notifier_call = netdev_event_handler;
    return register_netdevice_notifier(&g_netdev_event_handler_nb);
}
...
static int netdev_event_handler(struct notifier_block *nb, unsigned long event, void *netdev)
{
    ...
    pr_info("%s: event %ld, netdev %s\n", __func__, event, dev->name);

    if (event == NETDEV_REGISTER) {
        pr_info("register with device %s\n", dev->name);
        dev_hold(dev);
        g_ptm_net_dev = dev;
        return NOTIFY_OK;
    }
    ...
    if (event == NETDEV_UNREGISTER) {
        pr_info("unregister with device %s\n", dev->name);
        if (g_ptm_net_dev == dev) {
            dev_put(dev);
            g_ptm_net_dev = NULL;
        }
        return NOTIFY_OK;
...
static int __init vectoring_init(void)
{
    ...
    res = proc_create("vectoring", S_IRUGO|S_IWUSR, 0,
                        &g_proc_file_vectoring_dbg_seq_fops);
    if (!res)
        return -ENOMEM;

    err = register_netdev_event_handler();
    if (err) {
