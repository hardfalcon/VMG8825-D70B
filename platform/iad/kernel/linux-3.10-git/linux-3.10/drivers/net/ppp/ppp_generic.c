...
struct ppp_file {
	...
	int		priority;	/* send skb priority */
};
			 ...
			 size_t count, loff_t *ppos)
{
	...
	skb->priority = pf->priority;
#ifdef CONFIG_PPP_DEV_PRIORITY_DEBUGGING
	pr_crit("PPP: device %d, type %s - send frame with priority %d\n",
			pf->index, pf->kind == INTERFACE ? "i" : "c", skb->priority);
#endif
...
static long ppp_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	if (pf->kind == CHANNEL) {
		...
		case PPPIOCGETPRIORITY:
			if (put_user(pf->priority, p))
				break;
			err = 0;
			break;
...
{
	pf->kind = kind;
	pf->priority = CONFIG_PPP_DEV_DEFAULT_PRIORITY;
