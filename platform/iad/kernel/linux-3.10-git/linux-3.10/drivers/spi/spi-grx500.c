...
//#define CONFIG_SPI_GRX500_POLL

#define NO_TIMEOUT

/* you should turn this off when you are sure unalign load transparency fix is enable in your SOC*/
#define RX_UNALIGN_LOAD_FIX
#ifdef RX_UNALIGN_LOAD_FIX
...
static void ltq_spi_rxfifo_read(struct ltq_spi *hw)
{
	...
	while (hw->len - hw->rx_cnt >= 4 && fifo_fill) {
		/* you should turn this off when you are sure unalign load transparency fix is enable in your System*/
		#ifdef RX_UNALIGN_LOAD_FIX
		if ((((u32)rx32) % 4) != 0)
		{
			data = ltq_spi_reg_read(hw, LTQ_SPI_RB);

			put_unaligned(data, rx32);
			rx32++;
		}
		else
		#endif /* RX_UNALIGN_LOAD_FIX*/
