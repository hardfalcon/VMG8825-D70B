...
static struct sk_buff_head oam_skb_queue;

static void koam_rx_task(unsigned long data);

static DECLARE_TASKLET(koam_rx_tasklet, koam_rx_task, 0);
...
static void koam_rcv_skb(struct sk_buff *skb)
{
	...
	koam_upid = pCell->pid;
	if (vpi == 0 && vci == 0) {
		printk(KERN_ERR "userspace registering PID %d\n", koam_upid);
		goto SEND_OAM_END;
	}
...
static void koam_rx_task(unsigned long data)
{
	...
	while (!skb_queue_empty(&oam_skb_queue)) {
		skb = skb_dequeue(&oam_skb_queue);
...
static int push_oam_internal(struct atm_vcc *atmvcc, void *cell)
{
	...
	skb_queue_tail(&oam_skb_queue, skb);
	tasklet_schedule( &koam_rx_tasklet);
...
static __init int koam_init(void)
{
	...
	skb_queue_head_init(&oam_skb_queue);
