...
static void tcp_v6_send_response(struct sk_buff *skb, u32 seq,
...
void tcp_v6_send_reset(struct sock *sk, struct sk_buff *skb)
{
	...
	tcp_v6_send_response(skb, seq, ack_seq, 0, 0, 0, 0, oif, key, 1, 0, 0);
...
int tcp_v6_conn_request(struct sock *sk, struct sk_buff *skb)
{
	...
	return tcp_conn_request(1, &tcp6_request_sock_ops, &tcp_request_sock_ipv6_ops, sk, skb);
drop:
	NET_INC_STATS_BH(sock_net(sk), LINUX_MIB_LISTENDROPS);
 ...
 * In 0.90 this initialization is done inside tcp_fill_cb()
 * but we don't have this function in 3.10.
 */
#ifdef CONFIG_MPTCP
