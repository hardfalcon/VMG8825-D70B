...
static void mptcp_balia_recalc_ai(const struct sock *sk)
{
	...
	mptcp_for_each_sk(mpcb, sub_sk) {
		...
		/* replaced srrt_us with srtt
		 * however the formula uses USEC_PER_SEC so we should
		 * revisit this part
		 */
		tmp = div_u64((u64)tp->mss_cache * sub_tp->snd_cwnd
				* (USEC_PER_SEC << 3), sub_tp->srtt);
		sum_rate += tmp;
	...
	if (num_scale_down) {
		...
		mptcp_for_each_sk(mpcb, sub_sk) {
			...
			/* not sure that replacing srtt_us with srtt is safe
			 * because the formulae uses div_u64
			 */
...
static void mptcp_balia_cong_avoid(struct sock *sk, u32 ack, u32 in_flight)
{
