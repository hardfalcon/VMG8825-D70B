...
#ifdef SAS_EVENT_SCRIPT
typedef enum
{
    DSL_LINESTATE_SIMPLE_NOTINITIALIZED,
    DSL_LINESTATE_SIMPLE_RESET,
    DSL_LINESTATE_SIMPLE_READY,
    DSL_LINESTATE_SIMPLE_SYNCING,
    DSL_LINESTATE_SIMPLE_SHOWTIME,
    DSL_LINESTATE_SIMPLE_ERROR,
    DSL_LINESTATE_SIMPLE_UNKNOWN
} DSL_LineStateSimple_t;
static DSL_LineStateSimple_t g_nPrevLineStateSimple = DSL_LINESTATE_SIMPLE_NOTINITIALIZED;
static const char* DSL_CPE_LineStateSimpleDescription (DSL_LineStateSimple_t nLineStateSimple)
{
    switch ( nLineStateSimple )
    {
    case DSL_LINESTATE_SIMPLE_NOTINITIALIZED:
        return "notinitialized";
    case DSL_LINESTATE_SIMPLE_RESET:
        return "reset";
    case DSL_LINESTATE_SIMPLE_READY:
        return "ready";
    case DSL_LINESTATE_SIMPLE_SYNCING:
        return "syncing";
    case DSL_LINESTATE_SIMPLE_SHOWTIME:
        return "showtime";
    case DSL_LINESTATE_SIMPLE_ERROR:
        return "error";
    case DSL_LINESTATE_SIMPLE_UNKNOWN:
    default:
        return "unknown";
    }
}
   ...
   DSL_EventStatus_t *pEvent)
{
   ...
   if (g_sRcScript != DSL_NULL)
   {
...
#ifdef SAS_EVENT_SCRIPT
    DSL_LineStateSimple_t nLineStateSimple = DSL_LINESTATE_SIMPLE_NOTINITIALIZED;
    switch ( nLineState )
    {
    case DSL_LINESTATE_NOT_INITIALIZED:
        nLineStateSimple = DSL_LINESTATE_SIMPLE_NOTINITIALIZED;
        break;
    case DSL_LINESTATE_EXCEPTION:
        nLineStateSimple = DSL_LINESTATE_SIMPLE_ERROR;
        break;
    case DSL_LINESTATE_NOT_UPDATED:
        nLineStateSimple = DSL_LINESTATE_SIMPLE_READY;
        break;
    case DSL_LINESTATE_IDLE_REQUEST:
    case DSL_LINESTATE_IDLE:
        nLineStateSimple = DSL_LINESTATE_SIMPLE_RESET;
        break;
    ...
    case DSL_LINESTATE_SHOWTIME_NO_SYNC:
        nLineStateSimple = DSL_LINESTATE_SIMPLE_SYNCING;
        break;
    case DSL_LINESTATE_SHOWTIME_TC_SYNC:
        nLineStateSimple = DSL_LINESTATE_SIMPLE_SHOWTIME;
        break;
    ...
    default:
        nLineStateSimple = DSL_LINESTATE_SIMPLE_UNKNOWN;
    }


   if ( nLineStateSimple != g_nPrevLineStateSimple )
   {
        char szCmd[128];
        DSL_CPE_snprintf(szCmd, sizeof(szCmd), "xdsl.event %s %s",
                         DSL_CPE_LineStateSimpleDescription(g_nPrevLineStateSimple),
                         DSL_CPE_LineStateSimpleDescription(nLineStateSimple));

        DSL_CPE_snprintf(CLI_EventText, sizeof(CLI_EventText),
                         "LineStateChange %08X -> %08X: calling '%s'" DSL_CPE_CRLF,
                         g_nPrevLineState, nLineState, szCmd);

        DSL_CPE_System(szCmd);

        g_nPrevLineStateSimple = nLineStateSimple;
   }
#endif
