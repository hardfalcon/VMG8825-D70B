...
//static void write_pppd_status_rp(int status)
//{
//    FILE *fp;
//
...
//    if (fp != NULL)
//    {
//        /* syslog(LOG_INFO,"--rp %s -- %s = %d \n ", __FUNCTION__,"/tmp/pppd_status" , status);   */
//        fprintf(fp, "%d", status);
//        fclose(fp);
//    }
//
//    return;
//}
/* boeo 24.08.2009: use funktion exported by pppd */
extern void write_pppd_status(int status);

/* schwierd 08.06.2006: added function for writing name of AC to status file */
/* boeo 24.08.2009: should be moved into pppd */
void write_ac_name(int len, const char *ac_name)
{
    FILE *fp;
    char filename[32];

// boeo 24.08.2009: use different files for different ppp<x> interfaces
//    fp = fopen("/tmp/pppd_ac_name", "w");
    snprintf(filename, sizeof(filename), "/tmp/pppd%d_ac_name", req_unit);
    fp = fopen(filename, "w");
    if (fp != NULL)
    {
        fprintf(fp, "%.*s\n", len, ac_name);
...
#ifdef PLUGIN
        /* schwierd 08.06.2006: write name of AC to status file */
        write_ac_name((int)len, data);
#endif
...
waitForPADO(PPPoEConnection *conn, int timeout)
{
	...
	if (BPF_BUFFER_IS_EMPTY) {
		...
		// when used as plugin: handle SIGTERM received by pppd
		if (errno == EINTR)
		{
...
#ifdef PLUGIN
		/* schwierd 05.05.2006 / boeo 24.08.2009: show received PADO */
		write_pppd_status(1);
#endif
		break;
	    }
...
#ifdef PLUGIN
    /* schwierd 05.05.2006 / boeo 24.08.2009: show session */
    write_pppd_status(2);
#endif
}
...
#ifdef PLUGIN
    /* schwierd 05.05.2006 / boeo 24.08.2009: show start of discovery */
    write_pppd_status(5);
#endif
...
restart_padi:
...
#ifdef PLUGIN
        /* boeo 24.08.2009: show fault on timeout */
        write_pppd_status(3);
#endif
	}
	sendPADI(conn);
	...
	// see waitForPADO(): when pppd receives SIGTERM ...
	if( conn->discoveryState == STATE_TERMINATED) return;
	...
	if (padrAttempts > MAX_PADI_ATTEMPTS) {
        ...
        write_pppd_status(3);
#endif
	    /* RFC 2516: restart with PADI after failed PADS */
	    if (is_persist())
		goto restart_padi;
	}
	sendPADR(conn);
	...
	// see waitForPADS(): when pppd receives SIGTERM ...
	if( conn->discoveryState == STATE_TERMINATED) return;
	timeout *= 2;
