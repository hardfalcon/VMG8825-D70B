...
* hwaddr -- buffer for ethernet address
*%RETURNS:
* Nothing
*%DESCRIPTION:
...
* priority -- set the socket's skb->priority
*%RETURNS:
...
***********************************************************************/
int
openInterface(char const *ifname, UINT16_t type, unsigned char *hwaddr, unsigned int priority)
{
    static int fd = -1;
    char bpfName[32];
    u_int optval;
	...
	fatalSys("socket DGRAM failed");
    }

	if (setsockopt(fd, SOL_SOCKET, SO_PRIORITY, &priority, sizeof(unsigned int) )) {
	    fatalSys("setsockopt Priority failed");
	}
...
openInterface(char const *ifname, UINT16_t type, unsigned char *hwaddr, unsigned int priority)
{
    ...
    syslog(LOG_INFO, "%s: Interface=%.16s,Socket=%d, Socket priority=0x%x (%x:%x)\n", __FUNCTION__, ifname, fd, priority, (priority >> 16), (priority & 0xffff));

    if (setsockopt(fd, SOL_SOCKET, SO_BROADCAST, &optval, sizeof(optval)) < 0) {
	fatalSys("setsockopt Broadcast failed");
    }
	...
	if (NOT_UNICAST(hwaddr)) {
        ...
        /**
         * S.Mahr, 24.04.06:
         * MAC address modification for Multiple PPPoE.
         */
        if (usemac) {
            memcpy(hwaddr, usemac, ETH_ALEN);
        }

        syslog( LOG_INFO, "%s: Interface=%.16s HWaddr=%02X:%02X:%02X:%02X:%02X:%02X",
                __FUNCTION__,
                ifname,
                hwaddr[0], hwaddr[1], hwaddr[2],
                hwaddr[3], hwaddr[4], hwaddr[5] );
...
* type -- Ethernet frame type
* hwaddr -- if non-NULL, set to the hardware address
* priority -- set the socket's skb->priority, unsupported on aix
*%RETURNS:
* A raw socket for talking to the Ethernet card.  Exits on error.
*%DESCRIPTION:
* Opens a raw Ethernet socket
