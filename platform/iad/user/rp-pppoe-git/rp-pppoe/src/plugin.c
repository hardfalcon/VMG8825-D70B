...
unsigned int socket_priority = 0;

#ifdef SAS_PADT_PATCH
static void phase_changed __P((void *, int));
#endif

#define MAX_DEVICES 2

struct DevList {
    char list[MAX_DEVICES][MAXPATHLEN];
    int index;
};

static struct DevList dev_list;
...
static option_t Options[] = {
    ...
    { "socket_priority",  o_uint32, &socket_priority,
      "Desired socket priority to given value for PPP Discovery Packet priorization"},
    { NULL }
};
...
static int
findDevice(const char* devnam)
{
    int i;
    for (i = 0; i < MAX_DEVICES; i++) {
        if (!strcmp(dev_list.list[i], devnam))
            return i;
    }
...
static void
insertDevice(const char* devnam)
{
    int i;
    for (i = 0; i < MAX_DEVICES; i++) {
        if (dev_list.list[i][0] == '\0') {
            strncpy(dev_list.list[i], devnam, sizeof(dev_list.list[i]));
            info("insert device %s on index %d", dev_list.list[i], i);
            return;
        }
    }
}
...
getNextDevice(void)
{
    if (dev_list.index + 1 >= MAX_DEVICES) {
        dev_list.index = 0;
    } else {
        if (dev_list.list[dev_list.index + 1][0] != '\0')
            dev_list.index++;
    }

    return dev_list.list[dev_list.index];
}
...
{
    static const char ZERO[ETH_ALEN] = {0,0,0,0,0,0};
    ...
    conn->socket_priority = socket_priority;

    if (memcmp(mac_to_use, ZERO, ETH_ALEN))
    {   
        if (usemac != NULL)
            free (usemac);
        ...
        usemac = (char*) malloc(ETH_ALEN * sizeof(char));
        memcpy(usemac, mac_to_use, ETH_ALEN);

        info( "%s: Use MAC-address %02X:%02X:%02X:%02X:%02X:%02X\n", 
             __FUNCTION__,
             (unsigned char) usemac[0],
             (unsigned char) usemac[1],
             (unsigned char) usemac[2],
             (unsigned char) usemac[3],
             (unsigned char) usemac[4],
             (unsigned char) usemac[5] );
    }

    info("%s: socket priority: %d\n", __FUNCTION__, conn->socket_priority);
    info("%s: dev name %s\n", __FUNCTION__, conn->ifName);
	...
	for (i=0; i<ETH_ALEN; i++) {
        ...
        do {
            conn->discoverySocket =
                openInterface(conn->ifName, Eth_PPPOE_Discovery, conn->myEth, conn->socket_priority);
            discovery(conn);

            if (conn->discoveryState == STATE_SENT_PADI) {
                error("Unable to complete PPPoE Discovery");
                close(conn->discoverySocket);
                char *dev = getNextDevice();
                info("Switch to interface %s", dev);
                SET_STRING(conn->ifName, dev);
                strlcpy(devnam, dev, sizeof(devnam));
                strlcpy(ppp_devnam, devnam, sizeof(ppp_devnam));
            } else if (conn->discoveryState == STATE_TERMINATED) {
                info("Abort PPPoE Discovery (EINTR)");
                return -1;
            } else if (conn->discoveryState != STATE_SESSION) {
                error("Unable to complete PPPoE Discovery");
                return -1;
            }
...
PPPoEDevnameHook(char *cmd, char **argv, int doit)
{
    ...
    close(fd);
    if (r) {
	strncpy(devnam, dev_list.list[0], sizeof(devnam));
    ...
    if (OldDevnameHook) 
      r = OldDevnameHook(cmd, argv, doit);
...
#ifdef SAS_PADT_PATCH
    add_notifier(&phasechange, phase_changed, 0);
#endif

    memset(&dev_list, 0, sizeof(struct DevList));
...
#ifdef SAS_PADT_PATCH
#warning "compiled with PADT-Patch"
static void
phase_changed(void *arg, int phase)
{
  if (phase == PHASE_DEAD)
  {
    sendPADT(conn, "RP-PPPoE: Modem hangup");
  }
}
#endif
