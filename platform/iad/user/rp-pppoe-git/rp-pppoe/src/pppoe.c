...
usage(char const *argv0)
{
    ...
    fprintf(stderr,
	    ...
	    "   -e sess:mac    -- Skip discovery phase; use existing session.\n"
            "   -M macaddr     -- Use different MAC address.\n"
...
main(int argc, char *argv[])
{
    ...
    unsigned int ma[6];         /* MAC address in -M option */
...
#ifdef DEBUGGING_ENABLED
    options = "I:VAT:D:hS:C:Usm:np:e:M:kdf:F:t:";
#else
    options = "I:VAT:hS:C:Usm:np:e:M:kdf:F:t:";
#endif
    ...
    case 'M':
        n = sscanf( optarg, "%2x:%2x:%2x:%2x:%2x:%2x",
                    &ma[0], &ma[1], &ma[2], &ma[3], &ma[4], &ma[5]);

        if (n != ETH_ALEN) {
            fprintf(stderr, "Illegal argument to -M: Should be xx:yy:zz:aa:bb:cc\n");
            exit(EXIT_FAILURE);
        }
        usemac=(char*)malloc(ETH_ALEN * sizeof(char));
        /* Copy MAC address of peer */
        for (n = 0; n < ETH_ALEN; n++) {
                usemac[n] = (unsigned char) ma[n];
        }
    ...
    if (!optSkipSession)
        conn.sessionSocket = openInterface(conn.ifName, Eth_PPPOE_Session, conn.myEth, conn.socket_priority);
...
*/
    vec[0].iov_base = (void *) packet.payload;
    vec[0].iov_len  = plen;
