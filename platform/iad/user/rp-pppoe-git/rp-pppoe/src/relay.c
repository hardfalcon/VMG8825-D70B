...
/* ulrv TCD 27.04.2006 */
/* hack for don't replacing session ID and mac address */
int PassThroughtFlag; /* 0 replace session ID and mac */
                      /* 1 don't replace session ID and client mac address */
                      /* 2 don't replace session ID and client/server mac addresses */
...
usage(char const *argv0)
{
    ...
    /* ulrv TCD 04.05.2006 */
    fprintf(stderr, "   -R type        -- Don't replace session ID and mac address (type=1,2)\n");
...
main(int argc, char *argv[])
{
    ...
    while((opt = getopt(argc, argv, "hC:S:B:n:i:FR:")) != -1) {
	...
	case 'n':
        ...
        /* ulrv TCD 03.05.2006 */
        case 'R':
            if (sscanf(optarg, "%d", &PassThroughtFlag) != 1) {
                fprintf(stderr, "Illegal argument to -R: should be -R 1 or 2\n");
                exit(EXIT_FAILURE);
            }
            if (PassThroughtFlag < 0 || PassThroughtFlag > 2) {
                fprintf(stderr, "Illegal argument to -R: must range from 0 to 2\n");
                exit(EXIT_FAILURE);
            }
            break;
	default:
	     ...
	     int acOK)
{
    ...
    i->discoverySock = openInterface(ifname, Eth_PPPOE_Discovery, i->mac, 0);
    i->sessionSock   = openInterface(ifname, Eth_PPPOE_Session, 0, 0);
    i->clientOK = clientOK;
    i->acOK = acOK;
	      ...
	      UINT16_t acSes)
{
    ...
    /* ulrv 02.05.2006 */
    cliHash->sesNum = (PassThroughtFlag)?acSes:sess->sesNum;
    cliHash->ses = sess;
...
relayLoop()
{
	...
	for (i=0; i<NumInterfaces; i++) {
	    if (FD_ISSET(Interfaces[i].sessionSock, &readableCopy)) {
                ...
                if (PassThroughtFlag)
                {
                    relayNoReplaceGotSessionPacket(&Interfaces[i]);
                }
                else
                {
...
relayGotDiscoveryPacket(PPPoEInterface const *iface)
{
    ...
    case CODE_PADT:
        ...
        if (PassThroughtFlag)
        {
            relayNoReplaceHandlePADT(iface, &packet, size);
        }
        else
        {
	relayHandlePADT(iface, &packet, size);
    ...
    case CODE_PADO:
        ...
        if (PassThroughtFlag)
        {
            relayNoReplaceHandlePADO(iface, &packet, size);
        }
        else
        {
	relayHandlePADO(iface, &packet, size);
    ...
    case CODE_PADS:
        ...
        if (PassThroughtFlag)
        {
            relayNoReplaceHandlePADS(iface, &packet, size);
        }
        else
        {
	relayHandlePADS(iface, &packet, size);
...
/**********************************************************************
*%FUNCTION: relayNoReplaceGotSessionPacket
*%ARGUMENTS:
* iface -- interface on which packet is waiting
*%RETURNS:
* Nothing
*%DESCRIPTION:
* Receives and processes a session packet without replace session ID and
* mac-address.
***********************************************************************/
void
relayNoReplaceGotSessionPacket(PPPoEInterface const *iface)
{
    PPPoEPacket packet;
    int size;
    SessionHash *sh;
...
/*    if (memcmp(packet.ethHdr.h_dest, iface->mac, ETH_ALEN)) {
        syslog(LOG_ERR,
               "Wrong dest mac-addr %02x:%02x:%02x:%02x:%02x:%02x",
               packet.ethHdr.h_dest[0],
               packet.ethHdr.h_dest[1],
               packet.ethHdr.h_dest[2],
               packet.ethHdr.h_dest[3],
               packet.ethHdr.h_dest[4],
               packet.ethHdr.h_dest[5]);
        return;
    }*/
    ...
    if (!sh) {
        /* Don't log this.  Someone could be running the client and the
           relay on the same box. */
/* psi, removed
        syslog(LOG_INFO, "No Session found");
*/
        return;
    }
    ...
    sh = sh->peer;
        /* ulrv 04.05.2006 */
        /* don't replace session number */
    /* packet.session = sh->sesNum;*/
        /* don't replace sever/client mac addresses for PassThroughtFlag=2  */
        /* don't replace client mac addresses for PassThroughtFlag=1  */
        if (sh->interface->clientOK)
        {
                if (PassThroughtFlag == 1)
                {
                memcpy(packet.ethHdr.h_source, sh->interface->mac, ETH_ALEN);
                }
        }
...
#endif
#if 0
    syslog(LOG_ERR, "Relaying %02x:%02x:%02x:%02x:%02x:%02x(%s:%d) to %02x:%02x:%02x:%02x:%02x:%02x(%s:%d)\n",
            sh->peer->peerMac[0], sh->peer->peerMac[1], sh->peer->peerMac[2],
            sh->peer->peerMac[3], sh->peer->peerMac[4], sh->peer->peerMac[5],
            sh->peer->interface->name, ntohs(sh->peer->sesNum),
            sh->peerMac[0], sh->peerMac[1], sh->peerMac[2],
...
/**********************************************************************
*%FUNCTION: relayNoReplaceHandlePADT
*%ARGUMENTS:
* iface -- interface on which packet was received
* packet -- the PADT packet
*%RETURNS:
* Nothing
*%DESCRIPTION:
* Receives and processes a PADT packet without replace session ID and
* mac-address.
***********************************************************************/
void
relayNoReplaceHandlePADT( PPPoEInterface const *iface,
                          PPPoEPacket *packet,
                          int size )
{
    SessionHash *sh;
...
/**********************************************************************
*%FUNCTION: relayNoReplaceHandlePADO
*%ARGUMENTS:
* iface -- interface on which packet was received
* packet -- the PADO packet
*%RETURNS:
* Nothing
*%DESCRIPTION:
* Receives and processes a PADO packet without replace session ID and
* mac-address.
***********************************************************************/
void
relayNoReplaceHandlePADO(PPPoEInterface const *iface,
                PPPoEPacket *packet,
                int size)
{
    PPPoETag tag;
...
/**********************************************************************
*%FUNCTION: relayNoReplaceHandlePADS
*%ARGUMENTS:
* iface -- interface on which packet was received
* packet -- the PADS packet
*%RETURNS:
* Nothing
*%DESCRIPTION:
* Receives and processes a PADS packet without replace session ID and
* mac-address.
***********************************************************************/
void
relayNoReplaceHandlePADS(PPPoEInterface const *iface,
                PPPoEPacket *packet,
                int size)
{
    PPPoETag tag;
    ...
    if (packet->session != htons(0)) {
        ...
        /* Replace session number */
        /* ulrv 27.04.2006  don't replace session number */
        /* packet->session = ses->sesNum; */
        /*                                                */
    }
    ...
    /*
    syslog(LOG_ERR,
           "PADS packet send, source is %02x:%02x:%02x:%02x:%02x:%02x, session id %x ",
           packet->ethHdr.h_source[0],
           packet->ethHdr.h_source[1],
           packet->ethHdr.h_source[2],
           packet->ethHdr.h_source[3],
           ...
           packet->session);
    syslog(LOG_ERR,
           "PADS packet send, destination is to %02x:%02x:%02x:%02x:%02x:%02x, session id %x ",
           packet->ethHdr.h_dest[0],
           packet->ethHdr.h_dest[1],
           packet->ethHdr.h_dest[2],
           packet->ethHdr.h_dest[3],
    ...
    */
    ...
    sendPacket(NULL, Interfaces[ifIndex].discoverySock, packet, size);
        /*syslog(LOG_ERR, "PDAS sent, ifIndex %d", ifIndex);*/
}
