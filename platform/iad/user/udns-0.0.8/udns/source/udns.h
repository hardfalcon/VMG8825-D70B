/* $Id: udns.h,v 1.3.1.0 2006-06-19 10:18:07+02 titzec Exp $
 ...
 * <0 on failure. */
UDNS_API int
dns_init(int do_open,const char *interface,const char *resolvconf);

/* return new resolver context with the same settings as copy */
UDNS_API struct dns_ctx *
dns_new(const struct dns_ctx *copy, const char *resolvconf);
...
UDNS_API int
dns_open(struct dns_ctx *ctx, const char *interface);
...
enum dns_status {
  ...
  DNS_E_BADIF           = -7    /* bind to interface failed */
};
...
UDNS_API const char *dns_classname(enum dns_class a_class);
