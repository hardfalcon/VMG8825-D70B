/* $Id: udns_resolver.c,v 1.3.1.1 2006-10-17 10:45:03+02 svandar Exp $
...
struct dns_ctx {		/* resolver context */
  ...
  union _sockaddr_ns dnsc_serv[DNS_MAXSERV];
...
static int dns_init_internal(struct dns_ctx *ctx,const char *resolvconf) {
  char *v;
  char buf[2049];	/* this buffer is used to hold /etc/resolv.conf */

  /* read resolv.conf... */
  { int fd = open(resolvconf, O_RDONLY);
    if (fd >= 0) {
...
int dns_init(int do_open,const char *interface,const char *resolvconf) {
  ...
  if (dns_init_internal(ctx,resolvconf) != 0)
    return -1;
  dns_firstid(ctx);
  ctx->dnsc_flags |= DNS_INITED;
  return do_open ? dns_open(ctx,interface) : 0;
}

struct dns_ctx *dns_new(const struct dns_ctx *ctx, const char *resolvconf) {
  struct dns_ctx *n;
  SETCTXINITED(ctx);
  dns_assert_ctx(ctx);
  n = malloc(sizeof(*n));
  ...
  *n = *ctx;
  if(NULL != resolvconf)
  {
    n->dnsc_nserv = 0;
    n->dnsc_nsrch = 0;
    if (dns_init_internal(n,resolvconf) != 0)
    {
      free(n);
      return NULL;
    }
...
void dns_free(struct dns_ctx *ctx) {
  ...
  {
    dns_end_query(ctx, q, DNS_E_BADQUERY, 0);
  }
...
int dns_open(struct dns_ctx *ctx,const char *interface) {
  int sock;
  unsigned i;
  int port;
  union _sockaddr_ns *sns;
...
static void dns_send(struct dns_ctx *ctx, struct dns_query *q, time_t now) {
  ...
  DNS_DBGQ(ctx, q, 1,
           &ctx->dnsc_serv[q->dnsq_servi].sa, sizeof(union _sockaddr_ns),
