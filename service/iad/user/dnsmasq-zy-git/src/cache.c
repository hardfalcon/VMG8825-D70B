...
void dump_cache(time_t now)
{
  ...
  if (clear_cache_file() == -1)
      my_syslog(LOG_ERR, "Could not clear cache file: %s", strerror(errno));
  ...
  for (serv = daemon->servers; serv; serv = serv->next)
   ...
   /*
   * Always dump cache on SIGUSR1. To keep diff small we use this always true
   * condition.
   */
  if (1)
    {
      struct crec *cache ;
      int i;
      my_syslog(MS_DNS | LOG_DEBUG, "Host                                     Address                        Flags     Expires");
	...
	for (cache = hash_table[i]; cache; cache = cache->hash_next)
	  {
	    ...
	    *(p-1) = 0;
#endif
	    my_syslog(MS_DNS | LOG_INFO, "%s", daemon->namebuff);
	  }
    }
}
