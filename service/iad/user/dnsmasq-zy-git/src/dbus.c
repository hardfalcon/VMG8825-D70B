				  ...
				  void *user_data)
{
  ...
  else if (strcmp(method, "DumpCache") == 0)
  {
      DBusMessage *reply = dbus_message_new_method_return(message);
      int result = 1;

      dump_cache(dnsmasq_time());
...
void dbus_init(void)
{
  ...
  if (!(connection = dbus_bus_get (DBUS_BUS_SYSTEM, &dbus_error)))
    die(_("%s"), (char*)dbus_error.message, EC_MISC);
  ...
  if (!dbus_connection_register_object_path(connection,  DNSMASQ_PATH, 
					    &dnsmasq_vtable, NULL))
    die(_("could not register a DBus message handler"), NULL, EC_MISC);
...
void emit_dbus_signal(int action, struct dhcp_lease *lease, char *hostname)
{
  ...
  expires = lease->length;
#else
  expires = (unsigned int) lease->expires;
#endif

  if (dbus_message_iter_append_basic(&args, DBUS_TYPE_STRING, &daemon->addrbuff) &&
      dbus_message_iter_append_basic(&args, DBUS_TYPE_STRING, &mac) &&
      dbus_message_iter_append_basic(&args, DBUS_TYPE_STRING, &hostname) &&
      dbus_message_iter_append_basic(&args, DBUS_TYPE_UINT32, &expires))
    dbus_connection_send(connection, message, NULL);
