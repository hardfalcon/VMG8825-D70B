...
int log_start(struct passwd *ent_pw, int errfd)
{
  ...
  if (!log_reopen(daemon->log_file, &log_fd))
    {
      send_event(errfd, EVENT_LOG_ERR, errno, daemon->log_file ? daemon->log_file : "");
      _exit(0);
    }

  if (daemon->dns_cache_file)
  {
      if (!log_reopen(daemon->dns_cache_file, &log_fd_dns))
      {
          send_event(errfd, EVENT_LOG_ERR, errno, daemon->log_file ? daemon->log_file : "");
          _exit(0);
      }
...
int log_reopen(char *log_file, int *fd)
{
  if (!log_stderr)
    {      
        if (!fd)
      ...
      if (log_file)
	    {
          *fd = open(log_file, O_WRONLY|O_CREAT|O_APPEND, S_IRUSR|S_IWUSR|S_IRGRP);
          if (*fd == -1)
              return 0;
      }  
      else
...
#else
	   int flags;
    *fd = socket(AF_UNIX, connection_type, 0);

    /* if max_logs is zero, leave the socket blocking */
    if (*fd != -1 && max_logs != 0 && (flags = fcntl(*fd, F_GETFL)) != -1)
...
void my_syslog(int priority, const char *format, ...)
{
  ...
  pid_t pid = getpid();
  char *func = "";
  int old_log_to_file = log_to_file;
  ...
  else if ((LOG_FACMASK & priority) == MS_DNS)
  {
      func = "-dns";
      if (log_fd_dns)
      {
          old_log_to_file = log_to_file;
          log_to_file = 1;
      }
  }
...
/*
 * Clears the file specified by option --dns-cache-file
 */
int clear_cache_file()
{
    if (log_fd_dns != -1)
        return ftruncate(log_fd_dns, 0);
