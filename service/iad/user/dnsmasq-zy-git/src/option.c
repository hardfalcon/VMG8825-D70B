...
static const struct myoption opts[] = 
  ...
  { 
    ...
    { "dns-cache-file", 1, 0, LOPT_CACHEFILE },
...
static struct {
  ...
  { LOPT_CACHEFILE, ARG_ONE, "<file>", gettext_noop("Write DNS cache to this file."), NULL }, 
...
{      
  ...
  switch (option)
    ...
    case LOPT_CACHEFILE: /* --dns-cache-file */
      if (daemon->dns_cache_file)
        ret_err(_("only one dns-cache-file allowed"));
      else
        daemon->dns_cache_file = opt_string_alloc(arg);
      break;
