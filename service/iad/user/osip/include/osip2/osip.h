 ...
 */
  struct osip_naptr {
    ...
    int dns_result;                         /**< dns result */
    void *arg;                              /**< arg */
    int keep_in_cache;                      /**< keep in cache value */
    char *dns_if_name;                      /**< name of the interface which is used for dns-resolving */
...
/**
 * Structure for dns record handling.
 * @var srv_data_t
 */
  typedef struct srv_data srv_data_t;
 ...
 */
  ...
  {
    /* NAPTR result */
    char service[64];   /**< transport protocol*/
    int order;          /**< order */
    int preference;     /**< preference */
    long expires;       /**< ttl + now */
  };
/**
 * Structure for dns record handling.
 * @var a_data_t
 ...
 */
  struct a_data
  {
    ...
    int port;           /**< port */
    long expires;        /**< ttl + now */
    char address[40];   /**< A/AAA IP address */
  };
...
/**
 * Structure for dns record handling.
 * @var osip_dns_record_t
 */
  typedef struct osip_dns_record osip_dns_record_t;
/**
 * Structure for dns record handling.
 * @struct osip_dns_record
 */
  struct osip_dns_record
  {
    char *query;
    enum osip_dns_query_type query_type;
    enum osip_dns_query_state query_state;
    int dns_result;
    int ipv6_enable;
    union dns_data data;
    void *arg; /* c-ares */
    int param; /* for general use */
    int cache_record;
    osip_dns_record_t* parent;
    osip_dns_record_t* child;
    osip_dns_record_t* next;
  };  

/**
 * Release single dns record
 *
 * @param dns_record
 */
...
#if 0
    svr: srv record is not used, but takes 10KB memory, so removed
            additionally osip_transaction_set_srv_record is removed
            keeping member commented out as info
    osip_srv_record_t record;           /**< memory space for SRV record */
#endif

    osip_naptr_t *naptr_record;         /**< memory space for NAPTR record */
    osip_dns_record_t* dns_record;
    int dns_result;                     /**< Result of last DNS query */
