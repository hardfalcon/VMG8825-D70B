...
ict_snd_invite (osip_transaction_t * ict, osip_event_t * evt)
{
  ...
  if (i == OSIP_SUCCESS + 1) {
    ict->ict_context->timer_a_length = 10; /* give 10 ms for DNS answer */
...
osip_ict_timeout_a_event (osip_transaction_t * ict, osip_event_t * evt)
{
  ...
  /* reset timer */
  if (ict->state == ICT_CALLING && ict->ict_context->timer_a_length == 10)
      ; /* 10s are used as a marker, ict is in DNS processing, do not change timer a length until send returns OSIP_SUCCESS + 1 */
  else
  {
  ...
  if (i == 0)
    ...
    ict->ict_context->timer_a_length = 10;
    osip_gettimeofday (&ict->ict_context->timer_a_start, NULL);
  }
}
