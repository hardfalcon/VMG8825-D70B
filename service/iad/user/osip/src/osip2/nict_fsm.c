...
#endif
    if (i == OSIP_SUCCESS + 1) {
      nict->nict_context->timer_e_length = 10; /* give 10 ms for DNS answer */
      osip_gettimeofday (&nict->nict_context->timer_e_start, NULL); 
    }
    else if (nict->nict_context->timer_e_length > 0) {
      osip_gettimeofday (&nict->nict_context->timer_e_start, NULL);
...
osip_nict_timeout_e_event (osip_transaction_t * nict, osip_event_t * evt)
{
  ...
  if (nict->state == NICT_TRYING) {
    if (nict->nict_context->timer_e_length == 10)
      ; /* 10s are used as a marker, nict is in DNS processing, do not change timer e length until send returns OSIP_SUCCESS + 1 */
    else if (nict->nict_context->timer_e_length < DEFAULT_T1)
  ...
  if (i == 0) {                   /* but message was really sent */
    if (nict->state == NICT_TRYING)
      if (nict->nict_context->timer_e_length == 10)
        nict->nict_context->timer_e_length = DEFAULT_T1; /* finally message was send */
    __osip_message_callback (OSIP_NICT_REQUEST_SENT_AGAIN, nict, nict->orig_request);
  }
  else if (i == OSIP_SUCCESS +1) {
    /* still in DNS processing, give another 10 ms */
    nict->nict_context->timer_e_length = 10;
    osip_gettimeofday (&nict->nict_context->timer_e_start, NULL);
  }
}
