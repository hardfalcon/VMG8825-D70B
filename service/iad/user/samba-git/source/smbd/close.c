...
static NTSTATUS close_normal_file(files_struct *fsp, enum file_close_type close_type)
{
    ...
    if (lp_direct_printing(SNUM(conn)) && conn->printer) {
      /** Close print shortcut to lpr socket */
      sas_lpr_close();

      /**
       * Delete temporary file - not needed, Samba takes care
       * later... 
       */ 
    }
    /** END of modification */
    ...
    /**
     * thomas.haak@sphairon.com, note, 06.11.2007, SAS-specific 
     * customisation in order to avoid print-file spooling
     * 
     * in case direct printing has been enabled, printing was
     * already done at this point
     * nevertheless, we need to keep the print_file call,
     * since this will take care of discarding the temporary
     * print file
     */
