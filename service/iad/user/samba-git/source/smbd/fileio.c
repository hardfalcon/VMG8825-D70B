...
/**
 * thomas.haak@sphairon.com, modified, 12.11.2007, SAS-specific customisation
 * in order to avoid print-file spooling
 */
...
ssize_t write_file(files_struct *fsp, const char *data, SMB_OFF_T pos, size_t n)
{
    ...
    if (lp_direct_printing(SNUM(conn)) && conn->printer) {

      /**
       * Write_file is on a print file connection
       */

      DEBUG(3, ("write_file: file write request is on a print file connection...\n"));

      if (!sas_lpr_is_open()) {

        /** Shortcut to lpr not opened yet, need to open it */

        DEBUG(3, ("write_file: need to open lpr shortcut...\n"));

        char* hostname = (char*)lp_direct_printing_host(SNUM(conn));

        char* printername = lp_servicename(SNUM(conn));

        /** 
         * We have an write file request for printing thus, open a 
         * shortcut to the lpr 
         */ 
        if (sas_lpr_open(hostname, printername) < 0) {
          DEBUG(3, ("write_file: FAILED to open shortcut to lpr '%s' on host '%s'\n", printername, hostname));
        ...
        DEBUG(3, ("write_file: successfully opened shortcut to lpr '%s' on host '%s'.\n", printername, hostname));
      }

      DEBUG(3, ("write_file: writing data to lpr via shortcut...\n"));

      if (sas_lpr_write(data, n) == 0) {
        /** Writing was successfull */
        total_written = n;
        DEBUG(3, ("write_file: %d bytes successfully written to lpr\n", total_written));
      } else {
        DEBUG(3, ("write_file: FAILED writing to lpr via shortcut\n"));
        total_written = 0;
      }
