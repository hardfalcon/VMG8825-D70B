...
/*
 * thomas.haak@sphairon.com, modified, 06.11.2007, SAS-specific customisation
...
#if 0
  /** 
   * Code not activated, because handling of the sas lpr open is
   * done shortly before writing the data. This is because we only 
   * want to open a shortcut to the lpr in case there is also data 
   * to write. 
   */ 
  if (lp_direct_printing(SNUM(conn))) {
	  if (conn->printer) {
	    DEBUG(3, ("open_file: file open request is a print file connection...\n"));

        char* hostname = (char*)lp_direct_printing_host(SNUM(conn));

        /** 
         * We have an open file request for printing 
         * thus, open a shortcut to the lpr 
         */ 
	    if (sas_lpr_open(hostname) < 0) {
	      DEBUG(3, ("open_file: FAILED to open lpr shortcut to %s\n", hostname));
	    ...
	    DEBUG(3, ("open_file: successfully opened shortcut to lpr on %s.\n", hostname));

        /** 
         * Proceed... - let them create the file, we wont write into
         * it, so it will be deleted afterwards... 
         */ 
	  }
  }
#endif
